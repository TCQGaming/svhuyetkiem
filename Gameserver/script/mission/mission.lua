...
function tbBase:JoinPlayer(pPlayer, nGroupId)
	...
	if self.tbPlayers[nPlayerId] then
		-- print(debug.traceback("Mission Error: 玩家重复加入"));
		                print(debug.traceback("Mission Error: 玩家重复加入, PlayerId: " .. tostring(nPlayerId)));
	...
	local tbPlayerInfo	= {
		nGroupId		= nGroupId,
		nLogoutRegId	= PlayerEvent:Register("OnLogout", self._OnLogout, self),
	...
	if (tbDeathRevPos) then
		...
		if (tbPos) then
			...
			tbPlayerInfo.nRevMapId, tbPlayerInfo.nRevPointId	= pPlayer.GetRevivePos();
			pPlayer.SetTmpDeathPos(unpack(tbPos));
		end
	end
...
function tbBase:KickPlayer(pPlayer, szReason)
	...
	if (tbPlayerInfo.nLogoutRegId) then
		PlayerEvent:UnRegister("OnLogout", tbPlayerInfo.nLogoutRegId);
	end
	...
	if (tbPlayerInfo.nOnDeathRegId) then
		PlayerEvent:UnRegister("OnDeath", tbPlayerInfo.nOnDeathRegId);
		tbPlayerInfo.nOnDeathRegId	= nil;
	end
	
	if (tbPlayerInfo.nOnKillNpcRegId) then
		PlayerEvent:UnRegister("OnKillNpc", tbPlayerInfo.nOnKillNpcRegId);
		tbPlayerInfo.nOnKillNpcRegId	= nil;
	end
	
	self:_GoPos(tbMisCfg.tbLeavePos, pPlayer, nGroupId);
...
function tbBase:BroadcastMsg(nGroupId, szMsg, szName)
	...
	local tbPlayer, nCount	= self:GetPlayerList(nGroupId);
	if (nCount > 0) then
		KDialog.Msg2PlayerList(tbPlayer, szMsg, szName or "Hệ thống");
	end
end
...
function tbBase:GetPlayerList(nGroupId)
	...
	for nIndex, nPlayerId in ipairs(tbGroup) do
		...
		if pPlayer then
			...
			print(debug.traceback("Mission Error: 玩家不存在, PlayerId: " .. tostring(nPlayerId)));
...
function tbBase:GetPlayerGroupId(pPlayer)
	...
	local tbPlayerInfo	= self.tbPlayers[pPlayer.nId];
...
function tbBase:_GoPos(tbGroupPos, pPlayer, nGroupId)
	...
	local tbPos	= tbGroupPos[nGroupId] or tbGroupPos[0];
