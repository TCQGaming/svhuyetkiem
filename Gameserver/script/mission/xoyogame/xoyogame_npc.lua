...
function XoyoGame:ToBaoMingDian(nMapId)
  	...
  	if me.GetTask(9179,1) ~= nNgay then
		me.SetTask(9179,1, nNgay);
		me.SetTask(9180,1, 0);
	end 
...
XoyoGame.HONOR_CLASS_MONEY		= 8;	
XoyoGame.tbHonorLevelInfo	= {
	["money"]	= {
		nHonorId	= XoyoGame.HONOR_CLASS_MONEY,
	},
};
function XoyoGame:ApplyJoinGame(nGameId)
	...
	if not self.__debug_test and (nCurTime < self.START_TIME1 or nCurTime >= self.END_TIME1) and
		...
		Dialog:Say("Hãy đến gặp lão phu từ <color=yellow>8 giờ 00 đến 11 giờ tối, 12 giờ tối đến 2 giờ sáng<color> mỗi ngày!")
		return 0;
	end
	local tbOpt = {};
	if not nGameId then
...
for i = 1, #tbMember do
	...
	local nGioiHan = pPlayer.GetTask(9180,1)
    local nHonorValue = PlayerHonor:GetPlayerHonor(nPlayerId, XoyoGame.HONOR_CLASS_MONEY, 0);
    if nHonorValue < ConfigAll.GioiHanTaiPhuTDC then
        Dialog:Say("Thành viên " .. playerName .. " Tài phú hiện nay là :<color=yellow> "..nHonorValue.."<color> không đạt đủ<color=yellow> "..ConfigAll.GioiHanTaiPhuTDC.." <color>điểm Tài Phú để tham gia");
        return 0;
    end
	-- if nGioiHan > 0 then 
        -- Dialog:Say("Thành viên " .. playerName .. " Hôm nay đã đi Tiêu Dao Cốc 1 lần rồi ngày mai hãy quay lại");
        -- return 0;
    -- end
    local nRet = self:CheckPlayer(nPlayerId, nManagerId);
    if nRet ~= 1 then
        return 0;
    end
end
		...
		if nTeamId == 0 then --chưa có tổ đội
			Dialog:Say("Hãy lập tổ đội rồi quay lại tìm ta!")
			return 0;
		end
		local tbMember, nMemberCount = KTeam.GetTeamMemberList(nTeamId);
		if not tbMember or nMemberCount < self.MIN_TEAM_PLAYERS then
			Dialog:Say("Tổ đội phải có ít nhất (1) người, mau đi tìm đủ rồi quay lại đây sau!")
			return 0;
		end
	...
	local nDateNVngay    = tonumber(GetLocalDate("%d%m%y"));
	if pPlayer.GetTask(9055,1) < nDateNVngay then
		pPlayer.SetTask(9055,1, nDateNVngay);
		pPlayer.SetTask(9055,2, 0);
		pPlayer.SetTask(9055,3, 0);
		pPlayer.SetTask(9055,4, 0);
		pPlayer.SetTask(9055,5, 0);
		pPlayer.SetTask(9055,6, 0);
		pPlayer.SetTask(9055,7, 0);
	end 
	
	  	if pPlayer.GetTask(9180,3) ~= nDateNVngay then
		pPlayer.SetTask(9180,3, nDateNVngay);
		pPlayer.SetTask(9180,2, 0);
	end 
	
			pPlayer.SetTask(Cf.MuaLaiHD_nGROUP[2], Cf.MuaLaiHD_nTASK_ID,pPlayer.GetTask(Cf.MuaLaiHD_nGROUP[2], Cf.MuaLaiHD_nTASK_ID)+1);
