...
-- Giải tán đội ngủ khi hoàn thành (có thể thêm phần thưởng hoàn thành ở cụm này)
	...
	for i=1, nCount do
		...
		if pPlayer then
			if bAward and bAward == 1 then
				if pPlayer.AddStackItem(18,1,80,1,nil,5) ~= 2 then
					pPlayer.Msg("Hành trang không đủ ô trống!");
				end
				-- if pPlayer.AddItem(18,1,20350,1) ~= 2 then
					-- pPlayer.Msg("hé hé");
				-- end
				-- if pPlayer.AddStackItem(18,10,11,2,nil,150) ~= 2 then
					-- pPlayer.Msg("..");
				-- end
				--
			end
