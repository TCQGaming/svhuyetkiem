...
function BaseRoom:TeamAward(tbTeam, nMinuteExp, nRepute, nPrestige, nOffer, bWinner)
	...
	local fnExcute = function (pPlayer)
		...
		--==============================================================--
			--Phần thưởng qua ải
			--pPlayer.AddStackItem(18,10,11,2,nil,50);
		local nDiemHoatDong = pPlayer.GetTask(9177,1) --DAY
		local nGioiHanNgay = pPlayer.GetTask(9180,1) --DAY
		local nGioiHanNgayUyDanhTDC = pPlayer.GetTask(9180,2) --DAY
		local tbNhiemVu = SpecialEvent.NhiemVuHangNgay;
		local nCountNV  = pPlayer.GetTask(tbNhiemVu.TaskGroup, tbNhiemVu.TaskTDC);			
		local nSoTran = KGblTask.SCGetDbTaskInt(DBTASD_UPDATE_MUALAIHOATDONGTDC);
		local nSoCho = nSoTran;
		local nCount = pPlayer.GetTask(9042,1)
		 	local nEvent = ConfigAll.KhoiDong_NL_Event_TongKim
			local NguyenLieu_Event = ConfigAll.NguyenLieu_Event
				local NhiemVuTuanTDC = me.GetTask(9192,2) --DAY
		--========================TCQGaming======================================--
		if nCount >= nSoCho then
		pPlayer.Msg("<color=pink>Bạn Đã Mua Full Lần TDC Hoặc đã tham gia full lượt Tham Gia này Sẽ Ko Có Thưởng\n<color><color=yellow>Tổng số trận TDC Hiện nay "..nCount.."/"..nSoTran.." Toàn Máy Chủ")
		else
			--Phần thưởng qua ải	
			
			 pPlayer.AddExp(1000000);
			 pPlayer.AddBindCoin(20000);-----5 vạn đồng khóa
			 pPlayer.AddBindMoney(50000);--30 vạn bạc khóa
			 -- pPlayer.AddStackItem(18,1,114,6,nil,1);
			 pPlayer.AddStackItem(18,1,114,5,{bForceBind=1},1);
			 -- pPlayer.AddStackItem(18,1,200,1,nil,1)------Huyễn Ảnh Thương
			  			local nMocNap = me.GetTask(3028,1)
			me.SetTask(3028,1, nMocNap + 5);
			-- pPlayer.SetTask(9177,1,nDiemHoatDong +1);

		me.SetTask(9192,2,NhiemVuTuanTDC +1);
			if nGioiHanNgay < 1 then 
			pPlayer.SetTask(9180,1,nGioiHanNgay +1);
			end 
			if nGioiHanNgayUyDanhTDC < 10 then 
						me.AddKinReputeEntry(2)
						pPlayer.SetTask(9180,2,nGioiHanNgayUyDanhTDC +2);
						pPlayer.Msg("Chúc mừng bạn qua ải TDC nhận 2 Uy Danh")
			else 
			pPlayer.Msg("Ngày hôm nay đã đạt tối đa 10 uy danh ngày mai mới có thể nhận Uy danh lại")
			end 
			pPlayer.Msg("<color=gold>Chúc mừng bạn qua ải TDC nhận được :\n1 tr exp\n2 vạn đồng khóa\n5 vạn bạc khóa\n1 Huyền Tinh 5\n5 vạn tích nạp thẻ")
			
					if nEvent == 1 then 
						pPlayer.AddStackItem(NguyenLieu_Event[1], NguyenLieu_Event[2], NguyenLieu_Event[3], NguyenLieu_Event[4],nil,5);----50 Giỏ Event
					end 
			-- pPlayer.Msg("<color=yellow>Nhận thành công (+1 điểm hoạt động)<color>");
			pPlayer.SetTask(tbNhiemVu.TaskGroup, tbNhiemVu.TaskTDC,nCountNV + 1);
	--==============================================================--
			-- Tăng quỹ xây dựng băng đảng và các tộc trưởng tương ứng và cổ phần cá nhân
			local nStockBaseCount = 15; -- Số lượng cổ phiếu cơ bản		
...
function BaseRoom:ThemVatPhamQuaAi(nRepute, pPlayer)
if pPlayer.nLevel < 110 then
pPlayer.AddJbCoin(500);
else
pPlayer.AddJbCoin(100);
end
end
--==============================================================================================--
