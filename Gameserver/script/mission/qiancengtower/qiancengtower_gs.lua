...
local szMsg = string.format("Bạn có thể chọn chi tiêu <color=yellow>%s %s<color> hoặc <color=green>%s Jinghuo<color> để mua cơ hội bốc thăm may mắn, vui lòng chọn", tbConsumeItem. nNeedCount, szItemName, nJingHuo);
	local tbOpt = {
		--	{string.format("<color=yellow>是，我要消耗%s个%s继续抽奖<color>", tbConsumeItem.nNeedCount, szItemName), self.OnSureBuyFanPaiCount, self, nRound, self.DEF_BUY_TYPE_FANPAI_COUNT_CONSUMEITEM, nRoomNum},
			{string.format("<color=yellow>Có, tôi muốn sử dụng %s %s để tiếp tục vẽ <color>", tbConsumeItem.nNeedCount, szItemName), self.OnSureBuyFanPaiCount, self, nRound, self.DEF_BUY_TYPE_FANPAI_COUNT_CONSUMEITEM, nRoomNum} ,
		--	{string.format("<color=green>是，我要花费%s点精活继续抽奖<color>", nJingHuo), self.OnSureBuyFanPaiCount, self, nRound, self.DEF_BUY_TYPE_FANPAI_COUNT_JINGHUO, nRoomNum},			
			{string.format("<color=green>Có, tôi muốn dùng %s điểm Jinghuo để tiếp tục rút thưởng<color>", nJingHuo), self.OnSureBuyFanPaiCount, self, nRound, self.DEF_BUY_TYPE_FANPAI_COUNT_JINGHUO, nRoomNum},
			{"Ta muốn nghĩ lại"},
		};
...
function QianCengTower:OnSureBuyFanPaiCount(nRound, nBuyType, nRoomNum, nFlag)
	...
	if (nRound == 2 and nIsInMission == 0) then
		...
		if (nCount <= 0) then
		Dialog:Say("Hiện tại không có đợt bốc thăm bổ sung nào");
			return 0;
		end
	end
	...
	if (self.DEF_BUY_TYPE_FANPAI_COUNT_JINGHUO == nBuyType) then
		if me.dwCurGTP < nNeedJingHuo or me.dwCurMKP < nNeedJingHuo then
			Dialog:Say(string.format("Thể lực của bạn không đủ, bạn cần tiêu <color=yellow>%s điểm năng lượng và sức sống cho mỗi <color=yellow>%s điểm<color> để mua cơ hội xổ số.",nNeedJingHuo ));
			return 0;
		end		
	else
		...
		if (tbConsumeItem.nNeedCount > nHaveCount) then
			...
			Dialog:Say(string.format("<color=pink> %s quay xổ số <color> yêu cầu <color=green>%s %s<color>, số lượng bạn mang theo là <color=red>%s < màu>! Cần mua ngay?", nRound, tbConsumeItem.nNeedCount, szItemName, nHaveCount),
			--Dialog:Say(string.format("<color=pink>第%s轮<color>抽奖需要<color=green>%s个%s<color>，您所携带的数量为<color=red>%s个<color>！需要现在购买吗？", nRound, tbConsumeItem.nNeedCount, szItemName, nHaveCount),
				{
					{string.format("Mua<color=yellow>%s %s<color>", nDet, szItemName), self.BuyConsumeItem, self, nDet, tbAwardList.nWareId},
					{"Ta muốn nghĩ lại"},
				});
			return 0;
		end
	...
	if (self.DEF_BUY_TYPE_FANPAI_COUNT_JINGHUO == nBuyType) then
		...
		if (nRet ~= 0) then
me.Msg("Không thể khấu trừ đạo cụ!");
			return 0;
		end
...
function QianCengTower:BuyConsumeItem(nCount, nWareId)
	if (me.CountFreeBagCell() < nCount) then
		Dialog:Say(string.format("Không gian trong hành trang của bạn không đủ cho %s chỗ trống và không thể mua được!", nCount));
		return 0;
	end
	local nRet = me.ApplyAutoBuyAndUse(nWareId, nCount, 0);
	if (nRet ~= 1) then
Dialog:Say("Không thể mua cơ hội xổ số!");
		return 0;
	end
	return 1;
end
...
function QianCengTower:ApplyOpenQianCengFanPai()
	local nIsInMission = self:IsInGameMission(me);
	if (nIsInMission == 1) then
me.Msg("Giao diện chỉ mở được ở ngoài bản!");
		return 0;
	end
...
function QianCengTower:ApplyGetQianFanPaiAward(pPlayer)
	...
	if (1 == nRet) then
		...
		Dialog:SendBlackBoardMsg(pPlayer, "Tiếp tục quay xổ số tiếp theo và nhận phần thưởng hậu hĩnh hơn!");
		else
...
pPlayer.Msg(string.format("Hành trang của bạn không đủ chỗ trống cho %s, vì vậy bạn không thể nhận phần thưởng!", nNeedBag));
			return 0;
		end
	end
	...
	elseif (tbItem[1] == "item") then		
		...
		pPlayer.Msg(string.format("Bạn đã nhận được %s phần thưởng vật phẩm!", tbItem[2][5]));
...
function QianCengTower:ShowRandomResult(pPlayer, nRandomRet, nRound, nSelectIndex)
	...
	local szNormalMsg = string.format("Trong <color=gold>【Đường Baqianli】Hầm ngục ngàn tầng <color>, tôi đã nhận được [%s] qua vòng rút thăm %s. Tôi rất vui!", nRound, tbResultAward [4]);	
...
pPlayer.SendMsgToFriend(string.format("Bạn của bạn[<color=yellow>%s<color>]%s", pPlayer.szName, szNormalMsg));
	end
	
	-- 世界公告
	if (tbResultAward[7] == 1) then
		KDialog.NewsMsg(1, Env.NEWSMSG_NORMAL, string.format("Đại hiệp <color=gold>%s<color>%s", pPlayer.szName, szNormalMsg));
	end
...
function QianCengTower:CloseFanPaiGame(pPlayer)
	...
	if (nRandomRet > 0) then
		pPlayer.Msg("Không có phần thưởng, vui lòng quay lại sau");
	...
	Dialog:SendBlackBoardMsg(pPlayer, "Chúc bạn may mắn lần sau");	
end
...
function QianCengTower:ApplyGetQianCengFinishAward(pPlayer)
	local nAward = self:GetPlayerFinishAward(pPlayer);
	if (nAward <= 0) then
		Dialog:SayByPlayer(pPlayer, "Không có phần thưởng");
		return 0;
	end
	
	if (pPlayer.CountFreeBagCell() < 1) then
		Dialog:SayByPlayer(pPlayer, "Hành trang cần 1 ô trống");
		return 0;
	end
