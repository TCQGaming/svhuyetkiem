...
--script\mission\qiancengtower\qiancengtower_npc.lua
local tbSignNpc = Npc:GetClass("qiancengtower_sign_npc");
--
function tbSignNpc:OnDialog()
	QianCengTower:UpdateDaliyFreeCount(me);
	local nTongLuot = me.GetTask(2239,1)
	DoScript("\\script\\\mission\\\qiancengtower\\qiancengtower_savefile.lua");-- reload lai bang xep hang
	local tbOpt = {
		...
		--	{"Bảng Xếp Hạng", QianCengTower.OnDialog_Top, QianCengTower},
			{"Để ta suy nghĩ lại"},
		};
			local nTime = GetTime();
			local nCurTime = tonumber(os.date("%H%M", nTime))
			if (nCurTime >= QianCengTower.nStart_Time and nCurTime <= QianCengTower.nEnd_Time) and  nTongLuot > 0 then
				table.insert(tbOpt,1, {"<color=pink>Ta muốn báo danh", QianCengTower.OnApplyOpen, QianCengTower});
				table.insert(tbOpt,2, {"<color=pink>Hãy đưa ta vào Tháp", QianCengTower.OnEnterTower, QianCengTower});
				table.insert(tbOpt,3, {"<color=pink>Xem Xếp Hạng", QianCengTower.OnDialog_Top, QianCengTower});
			else
				table.insert(tbOpt,1, {"<color=pink>Ta muốn nhận thưởng", QianCengTower.OnDialog_Top, QianCengTower});
			end	
	local szMsg = string.format("Thiên Tầng Tháp chứa nhiều báu vật giá trị, mỗi Tầng Tháp là một độ khó khác nhau hãy cẩn thận\n<color=red> Số lượt đi còn lại : "..nTongLuot.." lượt<color> \n<color=green>Thời gian diễn ra sự kiện<color>\nBắt đầu: <color=yellow>%sh00<color> Kết thúc <color=yellow>%sh<color>\n<color=green>Thời gian nhận thưởng<color>\nBắt đầu: <color=yellow>%sh<color> Kết thúc  <color=yellow>%sh<color>", QianCengTower.nStart_Time,QianCengTower.nEnd_Time,QianCengTower.nStart_Time_NT,QianCengTower.nEnd_Time_NT);
...
function tbFinishNpc:OnDialog()
	...
	tbOpt[#tbOpt + 1] = {"Đưa ta ra khỏi đây", QianCengTower.ApplyLeaveQianCengRoom, QianCengTower, me};
