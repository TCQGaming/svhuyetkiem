...
function tbCampBase:OnEnd(nResult, szWinMsg)	
	...
	for _, pPlayer in pairs(tbPlayerList) do
		...
		pPlayer.Msg(string.format("Bảng xếp hạng :<color=green>%d<color>, Điểm tích lũy: <color=yellow>%d<color>", tbBattleInfo.nListRank, tbBattleInfo.nBouns));
	end
end
...
function tbCampBase:OnJoin(pPlayer)
	...
	-- self:SetPlayerCount(self.nPlayerCount + 2);
	if pPlayer.nLevel < 200 then
	pPlayer.TeamDisable(0); -- To Doi
	end
...
function tbCampBase:OnProtectFlag(tbFlagPlayer)
	...
	local szSecMsg = string.format("Tích lũy hộ kỳ hiện tại của quân %s là: <color=red>%d<color>", Battle.NAME_CAMP[tbFlagPlayer.tbCamp.nCampId], tbFlagPlayer.tbCamp.nFlags);
