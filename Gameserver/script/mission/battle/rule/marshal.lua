...
-- 杀不同Npc获得的积分
tbRuleBase.BOUNS_KILL_NPC	= { 1, 1, 2, 2, 10, 10, 20, 20, 100, 150 };

-- 杀大将、元帅为本阵营所有玩家加的积分
tbRuleBase.BOUNS_KILL_BOSS	= {
	[tbRuleBase.RANKID_DAJIANG]		= 200,
	[tbRuleBase.RANKID_YUANSHUAI]	= 200,
};

-- 护卫大将、元帅的奖励积分
tbRuleBase.BOUNS_PROTECT_BOSS	= {
	[tbRuleBase.RANKID_DAJIANG]		= 1,
	[tbRuleBase.RANKID_YUANSHUAI]	= 1,
...
function tbRuleBase:GetKillNpcBoardMsg(nRankId, nNpcBouns, pNpc)
	...
	if (1 < nRankId and 9 > nRankId) then
		szMsg = string.format("Đã đánh bại %s", Battle.NAME_RANK[nRankId]);
	elseif (9 <= nRankId) then
		local szNpcName = pNpc.szName;
		nMidMsg = 2;
		szMsg 	= string.format("Ngươi hùng dũng xông phá đánh bại <color=yellow>%s<color> quân địch, nhận được <color=yellow>%d<color> điểm tích lũy.", szNpcName, nNpcBouns);
		if (9 == nRankId) then
			nMidMsg = 1;
		end
	end
...
function tbRuleBase:OnDeath_Yuanshuai(nCampId, pNpc)
	...
	if (szKillerName) then
		...
		local szMsg = string.format("<color=yellow>%s<color> phe <color=yellow>%s<color> dũng cảm đánh bại <color=yellow>%s Nguyên Soái<color>, toàn quân %s nhận được 10.000 điểm tích lũy.", Battle.NAME_CAMP[nWinCampId], szKillerName, Battle.NAME_CAMP[nLoseCampId], Battle.NAME_CAMP[nWinCampId]);
...
function tbRuleBase:TryAddMarshal(nCampId, bForce)
	...
	local szMsg	= string.format("%s Nguyên Soái đích thân ra trận giết giặc %s quân mau tới hộ vệ Nguyên Soái.", szCampName, szCampName);
...
function tbRuleBase:OnTimer_AddDaJiang()
	for nCampId, tbMapInfo in pairs(self.tbMapInfoCamp) do		
		...
		local szCampName	= Battle.NAME_CAMP[nCampId];
		local szMsg			= string.format("%s đại tướng ra trận giết giặc %s quân mau tới hộ vệ đại tướng!", szCampName, szCampName);
...
function tbRuleBase:AddProtectBouns(tbCamp, tbBoss, nRankId, nCampId, nDistanceSqure)
	...
	local szMsg		= string.format("Ngươi có công bảo vệ %s quân %s, nhận được <color=yellow>%d<color> điểm tích lũy.",
									tbCamp.szCampName, Battle.NAME_RANK[nRankId], nAddBouns);
	local tbBossPos	= {};
...
function tbRuleBase:GiveKillBossCampBouns(nCampId, nRankId)
	...
	local szMsg	= string.format("%s quân %s đã bị tướng sỹ quân ta đẩy lùi, nhận được <color=yellow>%d<color> điểm tích lũy.",
						tbOppCamp.szCampName, Battle.NAME_RANK[nRankId], nAddBouns);
	if (10 == nRankId) then
		szMsg = szMsg .. "Điểm tích lũy toàn quân tăng 10000 điểm.";
	end
	self.tbMission:BroadcastMsg(nCampId, szMsg);
