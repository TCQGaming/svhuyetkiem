...
local tbRuleBase	= Battle:GetRuleClass(3, "Đoạt kỳ");

-- 杀不同Npc获得的积分
tbRuleBase.BOUNS_KILL_NPC				= { 1, 1, 2, 2, 10, 10, 20, 20, 0, 0 };
-- 玩家护旗成功奖励积分
tbRuleBase.BOUNS_PROTECT_FLAG_PLAYER	= 500;
-- 本方护旗成功所有玩家奖励
tbRuleBase.BOUNS_PROTECT_FLAG_CAMP		= 100;
...
tbRuleBase.FLAG_PROTECT_BOUNS			= 1;
...
function tbRuleBase:AddFlag(nCampId)
	...
	local szMsg		= string.format("<color=yellow>%s<color> chiến kỳ xuất hiện tại <color=green>(%d, %d)<color>.", Battle.NAME_CAMP[nCampId], tbPos[2] / 8, tbPos[3] / 16);
...
function tbRuleBase:DeletePlayerFlag(pPlayer)
	...
	if ((2 == self.tbMission.nState) and (1 == tbBattleInfo.bHaveFlag) and (tbBattleInfo.pPlayer.nId == tbBattleInfo.tbCamp.nPlayerIsFlag)) then
		...
		local szMsg		= string.format("%s chiến kỳ lạc mất tại (%d, %d).", Battle.NAME_CAMP[nCampId], nPosX / 8, nPosY / 16);
		self.tbMission:BroadcastMsg(szMsg);
...
function tbRuleBase:ReachToFlagDes(nCampId)
	...
	GeneralProcess:StartProcess("Đang cắm cờ ....", 3 * Env.GAME_FPS, {self.ReachToFlagDesSuc, self, tbCamp, pPlayer.nId}, 
			...
			if (pPlayer.IsDead() == 0) then
				...
				if (1 == self.nDuringProtect_TimeFlag) then
					...
					if (tbPos and tbPos[1]) then
						...
						if (nDx*nDx + nDy*nDy <= nDistanceSqure) then
							...
							szMsg		= string.format("Ngươi có công hộ kỳ, nhận được <color=yellow>%d<color> điểm thưởng.", nAddBouns);
							pPlayer.Msg(szMsg);
						end
					end
				end
				...
				if (0 == nAddBouns) then
					...
					if (tbPos and tbPos[1]) then
						...
						if (nDx*nDx + nDy*nDy <= nDistanceSqure) then
							nAddBouns	= self.FLAG_PROTECT_BOUNS;
							szMsg		= string.format("Ngươi có công hộ kỳ, nhận được <color=yellow>%d<color> điểm tích lũy.", nAddBouns);
							pPlayer.Msg(szMsg);
						end
					end
				end
...
function tbRuleBase:GiveFlagCampBouns(nCampId, tbBattleInfo)
	...
	local szMsg	= string.format("%s %s hộ kỳ thành công nhận được <color=yellow>%d<color> điểm tích lũy.", Battle.NAME_RANK[tbBattleInfo.nRank], tbBattleInfo.pPlayer.szName, nAddBouns);
	...
	for _, tbPlayer in pairs(tbPlayerInfoList) do
		if (tbPlayer and tbPlayer.pPlayer) then
			...
			if (((nDetX * nDetX + nDetY * nDetY) <= nDistance * nDistance) and tbPlayer.pPlayer.nId ~= nProtectPlayerId) then
				...
				local szMsg	= string.format("Có công trợ giúp hộ kỳ, nhận được <color=yellow>%d<color> điểm tích lũy.", nAddBouns);
...
function tbRuleBase:ChangePlayerState(pPlayer)
	Battle:ChangeFeature(pPlayer);
	pPlayer.CallClientScript({"Battle:ChangeRightSkill"});
end

function tbRuleBase:RestorePlayerState(pPlayer)
	Battle:RestoreFeature(pPlayer);
