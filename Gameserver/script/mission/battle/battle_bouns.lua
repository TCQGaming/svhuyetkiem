...
function Battle:GiveKillerBouns(tbKillerBattleInfo, tbDeathBattleInfo)
	...
	local nCount = tbKillerBattleInfo.pPlayer.GetTask(3145,1);
	if nCount >= 3 then
		tbKillerBattleInfo.pPlayer.Msg("<color=white>Hạ gục cùng 1 người chơi quá nhiều lần, bạn không nhận được tích lũy<color>");
	else
		...
		-- tbKillerBattleInfo.nKillPlayerBouns = tbKillerBattleInfo.nKillPlayerBouns + 10000; 
		end
	end
end
...
function Battle:AwardJbCoin(tbPlayerList)
	...
	for i = 1, #tbPlayerList do
		...
		local nTamTam_TuongAnhPhu = ConfigAll.KhoiDong_NL_TamTamTuongAnhPhu
				local NL_TamTamTuongAnhPhu = ConfigAll.NguyenLieu_TamTamTuongAnhPhu
		...
		local VatPham_DuaTOP = ConfigAll.VatPham_DuaTOP
			local nDiemHoatDong = tbBattleInfo.pPlayer.GetTask(9177,1) --DAY
...
function Battle:AwardWeiWang(tbBattleInfo, nWeiWang, nGongXian, nFlag)
	...
	if tbBattleInfo.tbMission and tbBattleInfo.tbMission.nBattleLevel then
		...
		-- Chức năng uy danh mặc định Tống Kim
	end
end
...
function Battle:AwardFinalXinDe(tbPlayerList)
	...
	for i = 1, #tbPlayerList do
		if (1 == i) then
			self:AwardXinDe(tbPlayerList[i].pPlayer, 100000);	-- 冠军由6点威望
		elseif (2 <= i and 10 >= i) then
			self:AwardXinDe(tbPlayerList[i].pPlayer, 80000);
		else
			local nBouns = tbPlayerList[i].nBouns;
			if (3000 < nBouns) then
				self:AwardXinDe(tbPlayerList[i].pPlayer, 10000);
			end
		end
	end	
end
...
function Battle:AwardGood(pPlayer, nItemId, nPaiCount, nFuCount, nBouns, nBattleLevel)
	...
	for i = 1, Battle.nTimes do
		...
		if (nBouns > 0) then
			...
			if me.CountFreeBagCell() < 10 then
				Dialog:Say("Chuẩn bị hành trang có 10 ô trống!");
				return 0;
			end
	...
	local nCurTime = tonumber(os.date("%H%M", nTime))
			...
			local nCountNV  = me.GetTask(tbNhiemVu.TaskGroup, tbNhiemVu.TaskTK);
			 			local nMocNap = me.GetTask(3028,1)
	local nTuan    = tonumber(GetLocalDate("%W%y"));----Task Tuần Hiện Tại
  	if me.GetTask(9155,2) ~= nTuan then
		me.SetTask(9155,2, nTuan);
		me.SetTask(9155,3, 0);
	end 
	if (nCurTime > 1049 and nCurTime < 1649) or (nCurTime > 1649 and nCurTime < 2049) or (nCurTime > 2249 and nCurTime < 2359) or (nCurTime > 0000 and nCurTime < 1049) then 
if nBouns >= 3000 then 
me.SetTask(9192,4,me.GetTask(9192,4) + 1)-----Nhiệm vụ tuần TK
end 

			if nBouns >= 1000 and nBouns < 2999 then
			me.AddBindCoin(3*10000);----đồng khóa
			me.AddBindMoney(5*10000);
			me.AddStackItem(18,1,114,5,{bForceBind=1},2);
			me.AddExp(3000000);--3 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 20)-----Mốc nạp
						
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0	
						-----Cộng task nhiệm vụ ngày hoàng kim tống kim -------
			me.SetTask(tbNhiemVu.TaskGroup, tbNhiemVu.TaskTK,nCountNV + 1);
			----------------------------------------------------------------------------------------
			elseif nBouns >= 3000 and nBouns < 4999 then
			me.AddBindCoin(50000);----đồng khóa
			me.AddBindMoney(15*10000);
			me.AddStackItem(18,1,114,6,{bForceBind=1},1);
			 me.AddJbCoin(20000);-- đồng thường
			me.AddExp(5000000);--5 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 30)-----Mốc nạp
					me.AddStackItem(18,1,2028,7,nil,2)-- sao chiến tích 
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0
			me.AddKinReputeEntry(5);

			----------------------------------------------------------------------------------------
			elseif nBouns >= 5000 and nBouns < 6999 then
			   -- me.AddStackItem(18,1,2024,1,nil,5);------Voucher Thần Long
			me.AddBindCoin(7*10000);----đồng khóa
			me.AddBindMoney(25*10000);
			me.AddStackItem(18,1,114,6,{bForceBind=1},2);-----2 viên 6 khóa
			me.AddJbCoin(20000);
			me.AddExp(5000000);--5 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 40)
						me.AddStackItem(18,1,2028,7,nil,2)-- sao chiến tích 
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0
					me.AddKinReputeEntry(6);
			----------------------------------------------------------------------------------------
			elseif nBouns >= 7000 and nBouns < 9999 then
		   -- me.AddStackItem(18,1,2024,1,nil,10);------Voucher Thần Long
			me.AddBindCoin(10*10000);----đồng khóa
			me.AddBindMoney(35*10000);
			me.AddStackItem(18,1,114,7,{bForceBind=1},1);
					me.AddStackItem(18,1,2028,7,nil,3)-- sao chiến tích 
			me.AddJbCoin(3*10000);
			me.AddExp(5000000);--5 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 40)
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0	
    me.AddTitle(2,3,1,9);----Danh hiệu
	me.AddKinReputeEntry(7);
			----------------------------------------------------------------------------------------			
			elseif nBouns >= 10000 and nBouns <= 14999 then
			me.AddBindCoin(15*10000);----đồng khóa
			me.AddBindMoney(50*10000);
			me.AddStackItem(18,1,114,7,{bForceBind=1},2);-----3 viên HT 7
			me.AddStackItem(18,1,2028,7,nil,4)-- sao chiến tích 
			me.AddJbCoin(5*10000);
			me.AddExp(5000000);--5 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 50)
			me.AddTitle(2,3,1,9);----Danh hiệu
			me.AddKinReputeEntry(10);
			
			elseif nBouns >= 15000 and nBouns <= 29999 then
			
			me.AddBindCoin(15*10000);----đồng khóa
			me.AddBindMoney(30*10000);
			me.AddStackItem(18,1,114,7,{bForceBind=1},3);-----3 viên HT 7
					me.AddStackItem(18,1,2028,7,nil,5)-- sao chiến tích 
			me.AddJbCoin(10*10000);
			me.AddExp(10000000);--10 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 50)
			me.AddTitle(2,3,1,9);----Danh hiệu
			me.AddKinReputeEntry(15);
			
			elseif nBouns >= 30000 then
		  -- me.AddStackItem(18,1,2024,1,nil,10);------Voucher Thần Long
			me.AddBindCoin(15*10000);----đồng khóa
			me.AddBindMoney(50*10000);
			me.AddStackItem(18,1,114,7,{bForceBind=1},4);-----2 viên HT 7
					me.AddStackItem(18,1,2028,7,nil,6)-- sao chiến tích 
			me.AddJbCoin(15*10000);
			me.AddExp(15000000);--50 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 50)
            me.AddTitle(2,3,1,9);----Danh hiệu
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0	
			me.AddTitle(2,3,1,9);----Danh hiệu
	...
	if (nCurTime > 2130 and nCurTime < 2249) then 
	if nBouns >= 3000 then 
me.SetTask(9192,4,me.GetTask(9192,4) + 1)-----Nhiệm vụ tuần TK
end 
					if nBouns >= 1000 and nBouns < 2999 then
			me.AddBindCoin(6*10000);----đồng khóa
			me.AddBindMoney(10*10000);
			me.AddStackItem(18,1,114,5,{bForceBind=1},2);
			me.AddExp(6000000);--3 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 40)-----Mốc nạp		
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0	
			...
			me.AddBindMoney(30*10000);
			me.AddStackItem(18,1,114,6,{bForceBind=1},1);
			 me.AddJbCoin(4*10000);-- đồng thường
			me.AddExp(10000000);--5 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 60)-----Mốc nạp
					me.AddStackItem(18,1,2028,7,nil,4)-- sao chiến tích 
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0
			me.AddKinReputeEntry(10);
			...
			elseif nBouns >= 5000 and nBouns < 6999 then
			   -- me.AddStackItem(18,1,2024,1,nil,5);------Voucher Thần Long
			me.AddBindCoin(14*10000);----đồng khóa
			me.AddBindMoney(50*10000);
			me.AddStackItem(18,1,114,6,{bForceBind=1},2);-----2 viên 6 khóa
			 me.AddJbCoin(4*10000);-- đồng thường
			me.AddExp(10000000);--5 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 80)
						me.AddStackItem(18,1,2028,7,nil,4)-- sao chiến tích 
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0
					me.AddKinReputeEntry(12);
			----------------------------------------------------------------------------------------
			elseif nBouns >= 7000 and nBouns < 9999 then
			me.AddBindCoin(20*10000);----đồng khóa
			me.AddBindMoney(70*10000);
			me.AddStackItem(18,1,114,7,{bForceBind=1},1);
					me.AddStackItem(18,1,2028,7,nil,6)-- sao chiến tích 
			me.AddJbCoin(6*10000);
			me.AddExp(10000000);--5 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 80)
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0	
    me.AddTitle(2,3,1,9);----Danh hiệu
	me.AddKinReputeEntry(14);
			----------------------------------------------------------------------------------------			
			elseif nBouns >= 10000 and nBouns <= 14999 then
			me.AddBindCoin(30*10000);----đồng khóa
			me.AddBindMoney(100*10000);
			me.AddStackItem(18,1,114,7,{bForceBind=1},2);-----3 viên HT 7
			me.AddStackItem(18,1,2028,7,nil,8)-- sao chiến tích 
			me.AddJbCoin(10*10000);
			me.AddExp(10000000);--5 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 100)
			me.AddTitle(2,3,1,9);----Danh hiệu
			me.AddKinReputeEntry(20);
			...
			me.AddBindMoney(100*10000);
			me.AddStackItem(18,1,114,7,{bForceBind=1},3);-----3 viên HT 7
					me.AddStackItem(18,1,2028,7,nil,10)-- sao chiến tích 
			me.AddJbCoin(20*10000);
			me.AddExp(20000000);--10 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 100)
			me.AddTitle(2,3,1,9);----Danh hiệu
		  ...
		  -- me.AddStackItem(18,1,2024,1,nil,10);------Voucher Thần Long
			...
			me.AddExp(30000000);--50 triệu exp
			me.SetTask(3028,1,me.GetTask(3028,1) + 100)
            me.AddTitle(2,3,1,9);----Danh hiệu
			me.SetTask(9152,5,0);--------Set Task Lưu Điểm TK Tuần Về 0	
			me.AddTitle(2,3,1,9);----Danh hiệu
	...
	if ConfigAll.GioiHanKhongTrungPhai_TongKim  == 1 then 
			...
			-- Cộng Task mua lại phần thưởng ---
				pPlayer.SetTask(9152,6,0);
				pPlayer.SetTask(9152,7,0);
				pPlayer.SetTask(9152,8,0);
				pPlayer.SetTask(9152,10,0);
			me.SetTask(Cf.MuaLaiHD_nGROUP[3], Cf.MuaLaiHD_nTASK_ID,me.GetTask(Cf.MuaLaiHD_nGROUP[3], Cf.MuaLaiHD_nTASK_ID)+1);
			--------- End ---------
		end
