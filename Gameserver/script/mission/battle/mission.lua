...
function tbMSBase:OnDeath(pKillerNpc)
    ...
    if nCurTime >= 2100 and nCurTime < 2155 then
	local nRandCoin = MathRandom(50, 200);
    pKillerPlayer.AddJbCoin(nRandCoin);
        local szMsg = string.format("%s %s <color=yellow>%s<color> đẩy lùi %s %s <color=yellow>%s<color> và nhận được <color=yellow>%d<color> đồng",
            Battle.NAME_CAMP[nKillerGroupId], Battle.NAME_RANK[tbKillerBattleInfo.nRank], tbKillerBattleInfo.pPlayer.szName,
            Battle.NAME_CAMP[nGroupId], Battle.NAME_RANK[tbDeathBattleInfo.nRank], tbDeathBattleInfo.pPlayer.szName, nRandCoin);
...
function tbMSBase:UpdateBattleInfo(nRemainTime)
	...
	for _, tbBattleInfo in pairs(tbPlayerList) do
		...
		if ((0 == tbBattleInfo.pPlayer.nFightState) and (2 == self.nState)) then
			...
			tbBattleInfo.pPlayer.Msg("Bạn đã ở quá lâu trong doanh trại");
			-----------------------------------------------------------------------------
			local nCurTime = tonumber(GetLocalDate("%H%M"));
			if (nCurTime >= 2300 and nCurTime < 2355) then
			tbBattleInfo.tbCamp:TransTo(tbBattleInfo.pPlayer, "OuterCamp1");
			tbBattleInfo.pPlayer.SetFightState(1);
			end
			--
			if (nCurTime >= 1100 and nCurTime < 2200) then
			tbBattleInfo.tbCamp:TransTo(tbBattleInfo.pPlayer, "OuterCamp1");
			tbBattleInfo.pPlayer.SetFightState(1);
