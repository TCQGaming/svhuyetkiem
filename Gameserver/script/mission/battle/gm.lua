...
function Battle:GM()
	...
	if (tbMission) then
		if (tbMission.nState == 1) then
			tbOpt[#tbOpt + 1]	= {"Bắt đầu báo danh tống kim", "Battle:GM_Start"};
		end
		...
		-- end
		for nRuleId, tbRule in ipairs(self.tbRuleBases) do
			tbOpt[#tbOpt + 1]	= {"Khai chiến-Gia Dụ Quang Chiến "..tbRule.szRuleName, "Battle:GM_OpenSp_JiaYuGuan", nRuleId};
		end		
		-- for nRuleId, tbRule in ipairs(self.tbRuleBases) do
