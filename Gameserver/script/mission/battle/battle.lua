...
function Battle:Init()
	...
	if (MODULE_GAMESERVER) then
		...
		PlayerSchemeEvent:RegisterGlobalDailyEvent({self.OnLogin, self});
	end
end
...
--	assert(not self.tbMissions[nBattleLevel][nBattleSeq]);	-- 级别重复	
	...
	if (not self.tbRuleList[nBattleLevel]) then
		Battle:DbgOut("OpenBattle", string.fromat("Cấp %d không có quy tắc này", nBattleLevel));
		return;
	end
	local tbRuleData	= self.tbRuleList[nBattleLevel][nRuleType];
	if (not self.tbRuleList[nBattleLevel][nRuleType]) then
		Battle:DbgOut("OpenBattle", string.format("Cấp %d, mô thức %d không có quy tắc này", nBattleLevel, nRuleType));
		return;
	end
	...
	if (not self.tbAllMapData[nMapId]) then
		Battle:DbgOut("OpenBattle", "Thông tin bản đồ không tồn tại!");
		return;
	end
...
-- 登陆加军需
function Battle:OnLogin(nCount)
...
function Battle:ChangeFeature(pPlayer)
	-- pPlayer.ChangeFeature(0, 0, 2524, {});
	local nLevel = pPlayer.nLevel;
	local nSkillLevel = math.floor(nLevel / 10) - 7;
	pPlayer.AddSkillState(self.RULE_PROTECTFLAG_CHANGESKILL, nSkillLevel, 0, 3600 * 18, 1, 1);
	if (9 == pPlayer.nFaction) then -- 如果玩家是武当派加个坐忘
		pPlayer.AddSkillState(161, 5, 1, 3600 * 18, 1, 1);
	end
end

function Battle:RestoreFeature(pPlayer)
	-- pPlayer.RestoreFeature();
	local nSkillLevel = pPlayer.GetSkillState(self.RULE_PROTECTFLAG_CHANGESKILL);
	if (nSkillLevel > 0) then
		pPlayer.RemoveSkillState(self.RULE_PROTECTFLAG_CHANGESKILL);
	end

	nSkillLevel = pPlayer.GetSkillState(161);
	if (nSkillLevel > 0) then
		pPlayer.RemoveSkillState(161);
	end	
end

-- 客户端脚本
if (MODULE_GAMECLIENT) then
function Battle:ChangeRightSkill()
	self.nOrgSkillId	= me.nRightSkill;
	me.nRightSkill		= self.RULE_PROTECTFLAG_CHANGERIGHTSKILL;
end
...
-- giới hạn ip tống kim

function Battle:SendBTOpenInfo()
	Battle.JieAddKey = 0;
	local szMsg = string.format("Đại chiến Tống Kim sắp bắt đầu, hiện đang tiến hành báo danh, người muốn tham chiến hãy nhanh chóng tìm Quan Mộ Binh (chiến trường) hoặc dùng Chiêu Thư Tống Kim, thời gian báo danh còn: %d phút. Điều kiện: Đẳng cấp không nhỏ hơn cấp %d.", self.TIMER_SIGNUP / (Env.GAME_FPS * 60), self.LEVEL_LIMIT[1])
	KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, szMsg);
	--if Battle.Mission_Battle_CheckNumAccount == 1 then
		--self:resetHWfile();
	--end
	---------------------reset bxh tong kim tuan-------------------------
 	local nTime = GetTime();
	local nWeekDay = tonumber(os.date("%w", nTime))
	if nWeekDay == 1 then
	local nTime = GetTime();
	local nCurTime = tonumber(os.date("%H%M", nTime))
	if nCurTime < 1052 then 
    Battle:ResetFile_15tongkim()
	end
	end
	------------------------------------------------------------------
end
