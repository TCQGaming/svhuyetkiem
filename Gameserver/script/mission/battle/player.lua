...
function tbPLBase:ShowRightBattleInfo()
    local szMsg = "";
    local nLevelId = Battle:GetJoinLevel(self.pPlayer);
    local nOpenCount = Battle:GetOpenCount(nLevelId, #Battle.MAPID_LEVEL_CAMP[nLevelId]);
    local nSongnCampNumTotal, nJinCampNumTotal = 0, 0;

    for i=1, nOpenCount do
        local nSongnCampNum, nJinCampNum = Battle:GetPlayerCount(nLevelId, i);
        nSongnCampNumTotal = nSongnCampNumTotal + nSongnCampNum;
        nJinCampNumTotal = nJinCampNumTotal + nJinCampNum;
    end
		local nTongDiemPheTong		= self.tbMission.tbCampSong.nBouns
		local nTongDiemPheKim		= self.tbMission.tbCampJin.nBouns;
		local nPheThang = ""
		if nTongDiemPheTong > nTongDiemPheKim then 
		nPheThang = "Tống"
		elseif  nTongDiemPheKim > nTongDiemPheTong then
		nPheThang = "Kim"
		else 
		nPheThang = "Hòa"
		end 
...
szMsg = string.format(
    "<color=green>Tống:<color> <color=white>%d người<color>" ..
    "  <color=green> Kim:<color> <color=white>%d người<color>\n" ..
    "<color=green>%s<color> <color=green>: <color=yellow><bclr=red>Phe %s<bclr><color>\n" ..
    "<color=green>Xếp hạng:<color> <color=yellow><bclr=red>%d<bclr><color>\n" ..
    "<color=green>Điểm cá nhân:<color> <color=red>%d<color>\n" ..
    "<color=green>Giết địch:<color><color=red>%d<color>\n" ..
    "<color=green>Liên trảm hiện tại:<color><color=red>%d<color>\n" ..
    "<color=red>Bên thắng hiện nay:<color> <color=yellow>"..nPheThang.."<color>\n" ..
    "<color=red>Điểm Tống:<color> <color=yellow>%d Điểm<color>\n" ..
    "<color=red>Điểm Kim:<color> <color=yellow>%d Điểm<color>",
    nSongnCampNumTotal, nJinCampNumTotal,
    self.pPlayer.szName, Battle.NAME_CAMP[self.tbCamp.nCampId], self.nListRank, self.nBouns, 
    self.nKillPlayerNum, self.nSeriesKillNum,nTongDiemPheTong,nTongDiemPheKim
);
...
function tbPLBase:GiveKillNpcBouns(pNpc)
	...
	if (1 == nMidMsg) then
		...
		local szAllMsg = string.format("<color=yellow>%s<color> <color=yellow>%s<color> tiêu diệt <color=yellow>%s<color>.", Battle.NAME_CAMP[self.tbCamp.nCampId], self.pPlayer.szName, Battle.NAME_RANK[nRankId]);
		local tbPlayerList = self.tbMission:GetPlayerList();
		szMsg = string.format("<color=yellow>%s<color> <color=yellow>%s<color> hãy tiêu diệt <color=yellow>%s<color>.", Battle.NAME_CAMP[self.tbCamp.tbOppCamp.nCampId], Battle.NAME_RANK[nRankId], Battle.NAME_CAMP[self.tbCamp.nCampId]);
	...
	elseif (0 == nMidMsg) then
		if (nRankId > 1) then
			local szAllMsg = string.format("%s %s %s đánh bại %s %s", Battle.NAME_CAMP[self.tbCamp.nCampId], Battle.NAME_RANK[self.nRank], self.pPlayer.szName, Battle.NAME_CAMP[self.tbCamp.tbOppCamp.nCampId], Battle.NAME_RANK[nRankId]);
