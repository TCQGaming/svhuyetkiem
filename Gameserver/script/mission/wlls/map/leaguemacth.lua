...
function tbMap:OnEnter()
	...
	if not szLeagueName then
		Wlls:KickPlayer(me, "Bạn không có đội。", 1);
		return
	end
	...
	if Wlls.ReadyTimerId <= 0 or nGameLevel == 0 then
		...
		Wlls:KickPlayer(me, "Trò chơi đã bắt đầu", nGameLevel)
		return
	end
	...
	Dialog:SendBlackBoardMsg(me, "Tiến vào sân chuẩn bị của võ lâm, sau khi hết thời gian chuẩn bị, trò chơi sẽ tự động bắt đầu.")
	me.Msg("Vào sân chuẩn bị của liên đoàn võ lâm, <color=yellow> sau khi hết thời gian chuẩn bị <color>, bạn sẽ <color=yellow> tự động vào sân thi đấu <color>, hãy chuẩn bị sẵn sàng.");
