...
--[[
	if (GLOBAL_AGENT) then
		...
		elseif (MODULE_GC_SERVER) then
			...
			print("*****************1111111111111111111111 - "..nResult.."*****************");
			return nResult;
		end		
	else
...
function Wlls:AddHonor(szName, nHonor)
	...
	if MODULE_GAMESERVER then
		...
		if nPlayerId and nPlayerId > 0 then
			...
			if pPlayer then
				pPlayer.Msg(string.format("Xin chúc mừng bạn đã có <color=yellow>%s<color> Điểm danh dự", nHonor));
			end
		end
		return 0;
	end
...
function Wlls:SendGbWllsFinalMsg_Gb(tb8FinalResult)
	...
	if nNowType == Wlls.MAP_LINK_TYPE_FACTION then
		...
		for nFaction, tbInfo in pairs(tbAdv) do
			...
			local szMatchName = string.format("Phái %s", szFaction);
...
function Wlls:SendGbWllsFinalOneMsg_Gb(tbData, nRank, szMatchName)
	...
	if (tbInfo) then
		local szFinalHonor = tbFinalHonor[nRank] or string.format("%s", nRank);
