...
function Wlls:UpdateWllsHonorLadder()
	print("Bang Xep Hang Vo Lam Lien Dau Bat Dau");
	local nType = 0;
	...
	local szContext		= string.format("%s Bảng danh dự (cập nhật sau khi giải đấu kết thúc)", self.LADDER_FACTIONNAME[0]);
	local szLadderName	= string.format("%s Danh dự", self.LADDER_FACTIONNAME[0]);
	...
	for i=1, 12 do
		...
		local szSubContext	= string.format("%s Bảng danh dự (cập nhật sau khi giải đấu kết thúc)", self.LADDER_FACTIONNAME[i]);
		local szLadderName	= string.format("%s Danh dự", self.LADDER_FACTIONNAME[i]);
	...
	print("Bang Xep Hang Vo Lam Lien Dau Ket Thuc");
end
