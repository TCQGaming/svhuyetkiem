...
function MissionBase:OnJoin(nGroupId)
	...
	Wlls:WriteLog(string.format("Người chơi vào sân chơi: %s", me.nMapId), me.nId)
end;
...
function MissionBase:OnLeave(nGroupId, nState)
	...
	if self.nGameState == 1 or self.nGameState == 2 then
		if nState ~= 1 and self.tbLeagueList[szLeagueName] and self.tbLeagueList[szMacthName] then
			...
			if Wlls:CountTableLeng(self.tbLeagueList[szLeagueName].tbAtGameList) == 0 then
				...
				Dialog:SendBlackBoardMsg(me, "Hãy đợi phần thi của đội bạn kết thúc để nhận được phần thưởng kinh nghiệm và vật phẩm tương ứng.");
				me.Msg("Hãy đợi phần thi của đội bạn kết thúc để nhận được phần thưởng kinh nghiệm và vật phẩm tương ứng.Trong quá trình này, vui lòng không thoát game hoặc rời khỏi địa điểm tổ chức giải đấu, vì điều này có thể dẫn đến việc không thể nhận được phần thưởng.");
			end
		end
	end
	...
	me.Msg(string.format("\n<color=green>Tổng sát thương của đối thủ： <color=red>%s\n<color=green>Tổng thiệt hại bên mình： <color=blue>%s", nMaxDamageB, nMaxDamageA));
	Wlls:LeaveGame()
	Wlls:CloseSingleUi(me)
end;

-- 开启活动
function MissionBase:StartGame(nReadyId, nGameLevel)
	-- 设定可选配置项
...
function MissionBase:OnGamePk()
	...
	for _, pPlayer in pairs(self:GetPlayerList()) do
		...
		Dialog:SendBlackBoardMsg(pPlayer, "Trò chơi chính thức bắt đầu");
	...
	for szLeagueName, tbLeague in pairs(self.tbLeagueList) do
		...
		if Wlls:CountTableLeng(tbLeague.tbAtGameList) > 0 then
			...
			if nTie == 1 then
				...
				self:KickLooker(szWin, string.format("Kết thúc，<color=yellow>%s<color> với <color=yellow>%s<color> giành chiến thắng。", szWin, szLoss));
				self:KickLooker(szLoss, string.format("Kết thúc，<color=yellow>%s<color> với <color=yellow>%s<color> bại trận", szWin, szLoss));
			else
				...
				self:KickLooker(szWin, string.format("Kết thúc，<color=yellow>%s<color> đánh bại <color=yellow>%s<color>。", szWin, szLoss));
				self:KickLooker(szLoss, string.format("Kết thúc，<color=yellow>%s<color> thua <color=yellow>%s<color>。", szLoss, szWin));
			end
			local nReadyId	= League:GetLeagueTask(Wlls.LGTYPE, szLeagueName, Wlls.LGTASK_ATTEND);
			if Wlls.AdvMatchState == 5 then
				Wlls:SetAdvMacthResult(nReadyId);
...
function MissionBase:LookerResult(szLeagueName, nResult)
	if self.tbLooker[szLeagueName] then
		...
		if szMsg2 then
			if nResult == 1 then
				szMsg2 = szMsg2 .. string.format("\n<color=green>Chiến thắng: <color><color=gold>%s<color>", szLeagueName);
			else
				szMsg2 = szMsg2 .. string.format("\n<color=green>Người chiến thắng: <color><color=gold>%s<color>", szMacthName);
			end
			for nLookId in pairs(self.tbLooker[szLeagueName]) do
				local pPlayer = KPlayer.GetPlayerObjById(nLookId);
				if pPlayer then	
