...
function Wlls:OnLookDialog(nReadyId, nSure)
	if Wlls.GameState <= 0 and Wlls.GameState > 2 then
		Dialog:Say("Bạn chỉ có thể đăng ký để xem trận đấu trong giai đoạn chuẩn bị đăng ký, và bạn không thể xem trận đấu bây giờ.");
		return 0;			
	end
	
	if me.nLevel < 100 then
		Dialog:Say("Đẳng cấp yêu cầu 100");
		return 0;				
	end
	
	if Wlls.GameState == 1 and Wlls.ReadyTimerId > 0 and Timer:GetRestTime(Wlls.ReadyTimerId) <= Wlls.MACTH_TIME_READY_LASTENTER then
		Dialog:Say("Đội thi đấu đang vào đấu trường. Vui lòng đợi vài giây và đợi người chơi vào đấu trường trước khi vào trò chơi.");
		return 0;
	end
	...
	if not nSure and Wlls:GetMacthLevelCfgType() == Wlls.MAP_LINK_TYPE_SERIES then
		local szMsg = "Vòng tứ kết của cuộc thi võ thuật đang diễn ra! \nHãy chọn trường bạn muốn xem?";
		local tbOpt = {
			{"Tứ kết vàng", self.OnLookDialog, self, 1, 1},
			{"Tứ kết gỗ", self.OnLookDialog, self, 2, 1},
			{"Tứ kết thủy hệ", self.OnLookDialog, self, 3, 1},
			{"Tứ kết lửa", self.OnLookDialog, self, 4, 1},
			{"Tứ kết đất", self.OnLookDialog, self, 5, 1},
			{"Không xem nữa"},
			};
	...
	if not nSure and Wlls:GetMacthLevelCfgType() == Wlls.MAP_LINK_TYPE_FACTION then
		local szMsg = "Vòng tứ kết của cuộc thi võ thuật đang diễn ra! \nHãy chọn trường bạn muốn theo dõi？";
		local tbOpt = {};
		for i=1, 12  do
			table.insert(tbOpt, {Player:GetFactionRouteName(i).." Tứ kết", self.OnLookDialog, self, i, 1});
		end
		table.insert(tbOpt, {"Không xem nữa"});
		Dialog:Say(szMsg, tbOpt);
		return 0;
	end
	...
	if #tbPlayerList <= 0 then
		Dialog:Say("Không có đội tham gia hoặc trò chơi chưa bắt đầu, và không thể xem trận đấu bây giờ.");
		return 0;
	end
	local nLevel = Wlls.MACTH_STATE_ADV_TASK[Wlls.AdvMatchState];
	if not nLevel or nLevel <= 0 then
		Dialog:Say("Trò chơi vẫn chưa bắt đầu, và không thể xem trận đấu bây giờ.");
		return 0;		
	end
	local szMsg = string.format("<color=yellow>%s đang diễn ra<color>。\nHãy chọn trường bạn muốn theo dõi？", nLevel);
	local tbOpt = {};
	if nLevel == 8 then
		for i=1, 4 do
			if tbPlayerList[i] or tbPlayerList[9-i] then
				local szLeagueA = "<color=gray>Không có đội<color>";
				local szLeagueB = "<color=gray>Không có đội<color>";
				local szName1 = "";
				local szName2 = "";
...
function Wlls:OnLookInto(szLeagueName, szLeagueName2, nReadyId)
	...
	if Wlls.GameState == 2 then
		if not Wlls.LookerLeagueMap[szLeagueName] or not Wlls.LookerLeagueMap[szLeagueName2] then
			Dialog:Say("Đội khán giả bạn đã chọn không có trên sân chơi hoặc trận đấu của họ đã kết thúc.");		
			return 0;
		end
 	...
 	if Wlls.LookPlayerCount[szLeagueName] and Wlls.LookPlayerCount[szLeagueName] >= self.LOOK_PLAYER_MAX then
 		Dialog:Say("Số lượng khán giả của trận đấu này đã chật kín.");
		return 0;		
 	end	
...
function Wlls:LookOnEnterReady()
	...
	local szUiName1 = "<color=gray>Không có đội<color>";
	local szUiName2 = "<color=gray>Không có đội<color>";
	...
	Wlls:UpdateMsgUi(me, string.format("\n<color=green>Xem đội chiến đấu：\n<color=gold>%s<color> Vs <color=gold>%s<color>\n\n<color=green>Chờ trò chơi bắt đầu<color>", szUiName1, szUiName2));
	Dialog:SendBlackBoardMsg(me, "Bạn sắp xem một trận đấu trong giải đấu. Sau khi hết thời gian chuẩn bị, trận đấu sẽ tự động bắt đầu.")
	me.Msg("Vào sân chuẩn bị cho giải đấu，<color=yellow>Sau khi thời gian chuẩn bị kết thúc<color>，Bạn có thể <color=yellow>Tự động vào sân chơi<color> Xem trò chơi.");
...
function Wlls:LookOnEnterPk()
	...
	tbMis.tbLooker[szLookLeagueName] = tbMis.tbLooker[szLookLeagueName] or {};
	tbMis.tbLooker[szLookLeagueName][me.nId] = 1;
	Dialog:SendBlackBoardMsg(me, "Vào sân, trò chơi sắp bắt đầu");
...
function Wlls:LookOnLeavePk()
	...
	local tbMis = self.LookerLeagueMap[szLookLeagueName][4];
	if not tbMis.tbLooker[szLookLeagueName] then
		Wlls:CloseSingleUi(me);
		return 0;
	end
	tbMis.tbLooker[szLookLeagueName][me.nId] = nil;
	Wlls:CloseSingleUi(me);
	GlobalExcute({"Wlls:MinusLooker", szLookLeagueName});
end
