	...
	for nReadyId, nMapId in pairs(tbMacthLevelCfg1.tbReadyMap) do
		...
		for _, tbLeague in pairs(self.GroupList[Wlls.MACTH_PRIM][nReadyId]) do
			...
			for _, nPlayerId in pairs(tbLeague.tbPlayerList) do
				...
				if pPlayer then
					nLeaveId = self:KickPlayer(pPlayer, "Lượt mới đã diễn ra, bắt buộc phải khởi động từ quá trình chuẩn bị", Wlls.MACTH_PRIM, nLeaveId);
				end
			end
		end
		self.GroupList[Wlls.MACTH_PRIM][nReadyId] = {};
	...
	for nReadyId, nMapId in pairs(tbMacthLevelCfg2.tbReadyMap) do
		...
		for _, tbLeague in pairs(self.GroupList[Wlls.MACTH_ADV][nReadyId]) do
			...
			for _, nPlayerId in pairs(tbLeague.tbPlayerList) do
				local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
				if pPlayer then
					nLeaveId = self:KickPlayer(pPlayer, "Lượt mới đã diễn ra, bắt buộc phải khởi động từ quá trình chuẩn bị", Wlls.MACTH_ADV, nLeaveId);
				end
			end
		end
		self.GroupList[Wlls.MACTH_ADV][nReadyId] = {};
...
function Wlls:GameStart()
	if (not GLOBAL_AGENT) then
		KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, "Võ lâm liên đấu đã bắt đầu nhận đăng ký , vui lòng đến NPC : Võ Lâm Liên Đấu Sơ hoặc Cao tại các thành thị ghi danh");		
	end
...
function Wlls:GamePkStart(nGameLevel)
	self.GameState = 2;
	KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, "Giải đấu đã chính thức bắt đầu");
	...
	for nReadyId, nMapId in pairs(tbMacthLevelCfg.tbReadyMap) do
		if SubWorldID2Idx(nMapId) >= 0 then
			if not self.MissionList[nGameLevel][nReadyId] then
				self.MissionList[nGameLevel][nReadyId] = Lib:NewClass(self.GameMission);
			end
			self.MissionList[nGameLevel][nReadyId]:StartGame(nReadyId, nGameLevel);
		end
	end
end
...
function Wlls:UpdateAllMsgUi(nGameLevel, nReadyId, szLeagueName)
	...
	for _, nPlayerId in pairs(self.GroupList[nGameLevel][nReadyId][szLeagueName].tbPlayerList) do
		local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
		Wlls:UpdateMsgUi(pPlayer, string.format("\n\n<color=green>Số lượng người chơi vào sân sẵn sàng： <color><color=white>%s<color>\n\n<color=green>Thời gian chờ <color>", #self.GroupList[nGameLevel][nReadyId][szLeagueName].tbPlayerList));
	end
end
...
function Wlls:MapStateFull(nPlayerId)
	local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
	if pPlayer then
		pPlayer.Msg("Số lượng đội tham gia đã đủ, Quay lại sau");
		Dialog:SendBlackBoardMsg(pPlayer, "Số lượng đội tham gia đã đủ, Quay lại sau.");
	end
end
...
--	for nReadyId, tbList in pairs(Wlls.AdvMatchLists) do
	if Wlls.AdvMatchLists[nReadyId]	then
		if Wlls.AdvMatchLists[nReadyId][2] then
			...
			if nRankAdvA == 1 then
			   	Wlls.AdvMatchLists[nReadyId][1][1]	= Wlls.AdvMatchLists[nReadyId][2][1];
			elseif nRankAdvB == 1 then
				Wlls.AdvMatchLists[nReadyId][1][1]	= Wlls.AdvMatchLists[nReadyId][2][2];
			end
			if Wlls.AdvMatchLists[nReadyId][2][1] then
