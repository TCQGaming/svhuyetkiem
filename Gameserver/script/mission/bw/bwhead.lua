...
BiWu.TB_CITY_BIWU_INFO = {		--这里指定 每个城市都用哪个地图,所以:有同城市的不要用同样的擂台ID, 不同城市可以用但不利于调配服务器
				--city	= {bwmap1,bwmap2,....}
					[23] = {167,174},--Biện Kinh
					[24] = {168,175},--Phượng tường
					[25] = {169,176},--Tương dương
					[26] = {170,177},--Dương Châu
					[27] = {171,178},--Thành Đô
					[28] = {172,179},--Đại Lý
					[29] = {173,180},--Lâm An
						};
...
BiWu.TB_MAPTYPE_MAPDEC = {
								--地图名，		白色			紫色			观众
				[1] = {szName = "Lôi Đài 1", tbCampPos = {{1833,3511},{1833,3511},{1833,3511}},},
				[2] = {szName = "Lôi Đài 2", tbCampPos = {{1833,3511},{1833,3511},{1833,3511}},},
			};
...
BiWu.TIMER_2 = 6 * 60 * Env.GAME_FPS ; --Tổng thời gian trận đấu là 6 phút, 1 phút đăng ký, 1 phút chuẩn bị thi đấu và 10 phút chiến đấu
...
--		me.TeamDisableChangeCamp(1)
		...
		me.Msg("<color=cyan>Thông báo hệ thống: Bạn tạm thời ẩn mặt");
	else
...
function BiWu:OnShowKey(nMapId)
	...
	if (nMapId == 0) then
		Dialog:Say(string.format("%s：Rất tiếc, bạn không phải là một trong những thí sinh ở đấu trường này nên tôi không thể cho bạn biết số dự thi.", him.szName), {{"Kết thúc đối thoại",self.OnCancel}});
		return 0;
	end;
	...
	if (self.tbMission[nMapId].nType <= 1) then
		Dialog:Say(string.format("%s：Đấu trường mà bạn đăng ký là cuộc thi 1 chọi 1, bạn không cần biết mã số dự thi, có thể trực tiếp tham gia thi đấu.", him.szName), {{"Kết thúc đối thoại",self.OnCancel}});
		return 0;
	end
	...
	Dialog:Say(string.format("%s：Mã số dự thi của người chơi của bạn là: <color=cyan>[%d]<color>. Vui lòng thông báo cho các thành viên trong nhóm tham gia của bạn về mã số này. Các thành viên trong nhóm chỉ có thể tham gia cuộc thi nếu họ nhập đúng số. Cảm ơn bạn", him.szName, nKey), {{"Kết thúc đối thoại",self.OnCancel}});
end
...
function BiWu:AddWinState(tbCurBiWuMapDec, nGroupId)
	...
	local tbPlayerLose = GetPlayerList:GetPlayerList(nGroupIdLose);
	for i = 1, #tbPlayerLose do
		PlayerIndex = tbPlayerLose[i];
		--me = tbPlayerLose[i];
