...
function tbMission:SetRestContext(pPlayer, nTime)
	...
	pPlayer.SetFightState(0);
	Dialog:SetBattleTimer(pPlayer, "<color=green>Tổng thời gian:<color=white> %s <color>\n\n<color=green>Thời gian vòng này:<color=white> %s<color>", self.tbGameSumTimer:GetRestTime(), nTime);
end
...
function tbMission:__join(__pPlayer)
	...
	if pPlayer.nTeamId == 0 then
		...
		if pPlayer.nId ~= tbPlayerList[1] then
			pPlayer.Msg("Gọi đội trưởng đến để đăng ký ~ ~ ~");
			return;
		end
...
function tbMission:BlizzardControl()
	if self.nNpcBlizzardId then
		...
		if pNpc then
			for _, pPlayer in pairs(self:GetPlayerList()) do
				Dialog:SendBlackBoardMsg(pPlayer, "Có vẻ như có mưa đá, cẩn thận vỡ đầu!");
			end
...
function tbMission:NianShouControl()
	...
	if(state == 1)then
		...
		self:DelRefreshNpc()
		for _, pPlayer in pairs(self:GetPlayerList()) do
			Dialog:SendBlackBoardMsg(pPlayer, "Con thú: Ai trong gây ồn ào vậy, phá vỡ giấc mơ của tôi, muốn chết ak!");
		end
	...
	elseif (state == 2) then
		for _, pPlayer in pairs(self:GetPlayerList()) do
			Dialog:SendBlackBoardMsg(pPlayer, "Con thú: Tất cả trốn đi đâu rồi, cho tôi xem nào!");
	...
	elseif (state == 4) then
		...
		for _, pPlayer in pairs(self:GetPlayerList()) do
			Dialog:SendBlackBoardMsg(pPlayer, "Con thú: Bạn có một số màu sắc cung cấp để tôi xem không, nếu không tôi tiếp tục đi ngủ!");
		end
	end
