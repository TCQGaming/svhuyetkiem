...
function Esport:SignUpFail(tbPlayerList)
	for _, nPlayerId in pairs(tbPlayerList) do
		...
		if pPlayer then
			pPlayer.Msg("Để tham gia ném tuyết. Người chơi có thể đến Án Nhược Tuyết ở Tân Thủ Thôn để đăng ký");
			Dialog:SendBlackBoardMsg(pPlayer, "Để tham gia ném tuyết. Người chơi có thể đến Án Nhược Tuyết ở Tân Thủ Thôn để đăng ký")
			return 0;
		end
	end
end
		...
		if SubWorldID2Idx(nMapId) > 0 then
			...
			if #tbGroupMatchList[nLastMembers] <=0 or #tbGroupMatchList[nCurMembers] <= 0 then
				for _, nId in pairs(tbGroupMatchList[nLastMembers]) do
					...
					if pPlayer then
						pPlayer.Msg("Thật không may, trong hoạt động lần này bạn mất một quả cầu tuyết");
						Dialog:SendBlackBoardMsg(pPlayer, "Thật không may, trong hoạt động lần này bạn mất một quả cầu tuyết");
				...
				for _, nId in pairs(tbGroupMatchList[nKey]) do
					...
					if pPlayer then
						if nDyMapId then
							...
							pPlayer.Msg("Không thể Loading map sân chơi  ném tuyết, hoạt động không thể mở, xin vui lòng liên hệ với GM.");
							pPlayer.NewWorld(nLeaveMapId, nLeavePosX, nLeavePosY);					
						end
					end
				end
