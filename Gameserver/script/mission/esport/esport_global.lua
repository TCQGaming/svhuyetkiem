...
--			end
--		end
		KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, "Hoạt động ném tuyết bắt đầu báo danh, người chơi trên cấp 50 đến Án Nhược Tuyết đăng kí");
	end	
	self:OnInit();
	self.nReadyState = 0;
...
function Esport:AddHonor(szName, nHonor)
	...
	if MODULE_GAMESERVER then
		...
		if nPlayerId and nPlayerId > 0 then
			local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
			if pPlayer then
				pPlayer.Msg(string.format("Bạn nhận được: <color=yellow>%s điểm<color>", nHonor));
			end
		end
		return 0;
	end
