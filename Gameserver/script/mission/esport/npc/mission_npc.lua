...
local TIME_COLLECT_BAOXIANG		 = 5; -- 采集宝箱需要用的时间，单位秒
...
local SkillSnow = Npc:GetClass("esport_skillsnow");		-- 神奇雪堆/刷技能
local TrapSnow	= Npc:GetClass("esport_trapsnow");		-- 冰原陷阱
local BuffSnow	= Npc:GetClass("esport_buffsnow");		-- 雪原神符
local BaoXiang  = Npc:GetClass("esport_baoxiang");		-- 宝箱
...
function SkillSnow:GiveSkill(nPlayerId, dwNpcId)
	...
	local nSkillId = Esport.tbTemplateId2Skill[pNpc.nTemplateId];
	local nSkillLevel = Esport.tbSkill2Level[nSkillId];
	local nTimeOut = 30 * Env.GAME_FPS;
	if (Esport.tbSkill2Time[nSkillId] and Esport.tbSkill2Time[nSkillId] > 0) then
		nTimeOut = Esport.tbSkill2Time[nSkillId] * Env.GAME_FPS;
	end
	tbFun:GiveSkill(pPlayer, nSkillId, nSkillLevel, nTimeOut);
	FightSkill:SaveLeftSkillEx(pPlayer, Esport.SKILL_ID_SNOWBALL_ORIGINAL);
...
function TrapSnow:UseTrap(nPlayerId, dwNpcId)
	...
	local nTrapId = Esport.tbTemplateId2Trap[pNpc.nTemplateId];
	local nTrapLevel = Esport.tbSkill2Level[nTrapId];
...
function BuffSnow:GiveBuff(nPlayerId, dwNpcId)
	...
	local nBuffId = Esport.tbTemplateId2Buff[pNpc.nTemplateId];
	local nBuffLevel = Esport.tbSkill2Level[nBuffId];
	local nTime = 15 * Env.GAME_FPS;
	if (Esport.tbSkill2Time[nBuffId] and Esport.tbSkill2Time[nBuffId] > 0) then
		nTime = Esport.tbSkill2Time[nBuffId] * Env.GAME_FPS;
	end
...
function tbFun:GiveSkill(pPlayer, nSkillId, nSkillLevel, nTimeoutFPS)
	...
	local nOriginal = Esport.tbSkill2Original[nSkillId] or Esport.SKILL_ID_SNOWBALL_ORIGINAL;
...
function BaoXiang:OnDialog()
	if me.CountFreeBagCell() < 1 then
		me.Msg("Hành trang khô đủ 1 ô trống");
		return 0;
	end	
	GeneralProcess:StartProcess("Đang lấy...", TIME_COLLECT_BAOXIANG * Env.GAME_FPS, {self.OnSurePickUp, self, me.nId, him.dwId}, nil, tbEvent);
...
-- ?pl DoScript("\\script\\mission\\esport\\npc\\mission_npc.lua")