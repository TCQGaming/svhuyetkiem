...
tbNpc.tbChangeItemList = {
	[1] = {
		szContext = "1个月影之石换取一艘破浪（2次攻击性改造）",
		tbGiftParam = {
	...
	[2] = {
		szContext = "1个月影之石换取一艘乘风（1次攻击性改造及一次防御性改造）",
		tbGiftParam = {
	...
	[3] = {
		szContext = "1个月影之石换取一艘离水（2次防御性改造）",
		tbGiftParam = {
	...
	[4] = {
		szContext = "5个月影之石换取一艘霸舟（2次攻击性改造及1次防御性改造）",
		tbGiftParam = {
...
function tbNpc:ProductBoat()
	local szMsg = "在这里，你只需要花少量的钱就可以对心爱的龙舟进行改造，从而使其具有特殊的能力，帮助你取得比赛的胜利！";
	local tbOpt = {
		{"了解龙舟改造", self.AboutBoat, self},
		{"进攻性改造", self.OpenProductBoatUi, self, 1},
		{"防御性改造", self.OpenProductBoatUi, self, 2},
		{"随便看看"},
	};
	Dialog:Say(szMsg, tbOpt);
end
...
function tbNpc:ChangeItem(nLevel)
	...
	if (not nLevel) then
		...
		tbOpt[#tbOpt + 1] = {"我来看看"};
		Dialog:Say("1个月影之石换取一艘破浪或者乘风或者离水，5个月影之石换取一艘霸舟。月影之石可以去杂货店买月影原石利用生活技能加工制作得到。你要兑换哪种道具？", tbOpt);
		return 0;
	end
...
function tbNpc:AboutBoat()
	...
	Dialog:Say(szMSg,{{"我知道了", self.ProductBoat, self}});
end

function tbNpc:OpenProductBoatUi(nType)
	Dialog:OpenGift("放入需要改造的龙舟", nil, {self.OnProductBoat, self, nType});
end

function tbNpc:OnProductBoat(nType, tbItem)
	if #tbItem <= 0 or #tbItem >= 2 then
		Dialog:Say("请放入需要改造的龙舟，只需要给我一艘龙舟。");
		return 0;
	end
	...
	if szKey ~= string.format("%s,%s,%s", unpack(Esport.DragonBoat.ITEM_BOAT_ID)) then
		Dialog:Say("请放入需要改造的龙舟，你放入的不是龙舟。");
		return 0;
	end
	local nGenId1 = Esport.DragonBoat:GetBoatRestGenId(nType, pItem)
	if nGenId1 <= 0 then
		Dialog:Say("你的龙舟性能已改造完成了，不能再进行改造。");
		return 0;
	end
...
function tbNpc:OnProductBoat1(nSel, nItemId)
	...
	if nGenId <= 0 then
		Dialog:Say("你的龙舟本类型技能改造已完成，不能再进行改造。", {{"返回上层", self.OnProductBoatSel, self, nItemId},{"Kết thúc đối thoại"}});
		return 0;
	end
	...
	--table.insert(tbOpt, {"返回上层", self.OnProductBoat1, self, nSel, nItemId});
...
function tbNpc:OnProductBoat2(nSel, nSelSkill, nItemId) 
	...
	if Esport.DragonBoat:CheckSkill(pItem, nSkillId) > 0 then
		Dialog:Say("你的龙舟已经改造了这个技能,不能同时改造同一技能", {{"继续改造这只龙舟", self.OnProductBoat1, self, nSel, nItemId},{"Kết thúc đối thoại"}});
		return 0;
	end
	...
	local tbOpt = {
		{"我要进行改造", self.OnProductBoat3, self, nSel, nSelSkill, nItemId},
		{"返回上层", self.OnProductBoat1, self, nSel, nItemId},
		{"我再考虑考虑"},
	}
	Dialog:Say(szMsg, tbOpt);
end
...
function tbNpc:OnProductBoat3(nSel, nSelSkill, nItemId) 
	...
	Dialog:Say("你成功改造这只龙舟。", {{"继续改造",self.ProductBoat, self},{"Kết thúc đối thoại"}});
end
...
{"如何获得专属龙舟",[[
    龙舟的获取方式：去杂货店购买月影原石，利用生活技能制作出月影之石，再去秦洼处换取。1个月影之石换取一艘普通龙舟，5个月影之石可换取龙舟·霸舟。
    ]]
},
{"了解龙舟改造",[[
