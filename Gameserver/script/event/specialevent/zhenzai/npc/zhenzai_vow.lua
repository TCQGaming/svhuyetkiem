...
function tbNpc:GetPingAn()
	...
	local pItem = me.AddItem(18,1,934,1);   --平安香
	if pItem then
...
function tbNpc:GetAward(tbItem)
	...
	if (nCount + 1) == ZhenZai.nTrapNumber then
		Dialog:GlobalNewsMsg_GS(string.format("%s许下了平安佛的第%s次祝愿，成为今日的平安大使，灾区人民一定能平安！", me.szName, ZhenZai.nTrapNumber));
		...
		if pItemEx then
			EventManager:WriteLog(string.format("[赈灾]获得物品:%s",pItemEx.szName), me);
			me.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, string.format("[赈灾]获得物品:%s",pItemEx.szName));
		end
	end
end
