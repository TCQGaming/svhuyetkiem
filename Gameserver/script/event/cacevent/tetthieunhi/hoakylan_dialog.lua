local tbHoaKyLanDoiThoai = Npc:GetClass("satthu95_hoakylan");
tbHoaKyLanDoiThoai.tbItemInfo = {
        bForceBind=1,
};
function tbHoaKyLanDoiThoai:OnDialog()
...
if nCount1 < 1 then
Dialog:SendBlackBoardMsg(me, string.format("Để thuần phục <color=red>Hỏa Kỳ Lân<color> cần <color=yellow> 1 Quyển 300 Bài Hát Thiếu Nhi<color> !"));
return
end
local tbEvent = 
	{
...
function tbHoaKyLanDoiThoai:GetQuest(nPlayerId, nNpcId)
...
KDialog.MsgToGlobal("<color=yellow>Người chơi <color=green>["..me.szName.."]<color> thuần phục <color=red>Hỏa Kỳ Lân<color> nhận được <color=green>50tr EXP<color>!<color>");	
end
