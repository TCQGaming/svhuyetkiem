local tbHacKyLanDoiThoai = Npc:GetClass("satthu95_hackylan");
tbHacKyLanDoiThoai.tbItemInfo = {
        bForceBind=1,
};
function tbHacKyLanDoiThoai:OnDialog()
...
if nCount1 < 1 then
Dialog:SendBlackBoardMsg(me, string.format("Để thuần phục <color=black>Hắc Kỳ Lân<color> cần <color=yellow> 1 Quyển 300 Bài Hát Thiếu Nhi<color> !"));
return
end
local tbEvent = 
	{
...
function tbHacKyLanDoiThoai:GetQuest(nPlayerId, nNpcId)
...
me.AddExp(1); -- Nhận được 50tr EXP
Task:DelItem(me, tbItemId1, 1); -- Xóa 1 Item tbItemId1 được khai báo ở trên là 300 Bài Hát Thiếu Nhi
KDialog.MsgToGlobal("<color=yellow>Người chơi <color=green>["..me.szName.."]<color> thuần phục <color=black>Hắc Kỳ Lân<color> nhận được <color=green>50tr EXP<color>!<color>");	
end
if (tbAward[nIndex]==2) then -- Xác xuất ra số 1 là 30/100 (30%)
me.AddExp(1); -- Nhận được 70tr EXP
Task:DelItem(me, tbItemId1, 1); -- Xóa 1 Item tbItemId1 được khai báo ở trên là 300 Bài Hát Thiếu Nhi
KDialog.MsgToGlobal("<color=yellow>Người chơi <color=green>["..me.szName.."]<color> thuần phục <color=black>Hắc Kỳ Lân<color> nhận được <color=green>50tr EXP<color>!<color>");	
end
if (tbAward[nIndex]==3) then -- Xác xuất ra số 1 là 15/100 (15%)
me.AddExp(1); -- Nhận được 100tr EXP
Task:DelItem(me, tbItemId1, 1); -- Xóa 1 Item tbItemId1 được khai báo ở trên là 300 Bài Hát Thiếu Nhi
KDialog.MsgToGlobal("<color=yellow>Người chơi <color=green>["..me.szName.."]<color> thuần phục <color=black>Hắc Kỳ Lân<color> nhận được <color=green>50tr EXP<color>!<color>");	
end
if (tbAward[nIndex]==4) then -- Xác xuất ra số 1 là 5/100 (5%)
me.AddExp(1); -- Nhận được 150tr EXP
Task:DelItem(me, tbItemId1, 1); -- Xóa 1 Item tbItemId1 được khai báo ở trên là 300 Bài Hát Thiếu Nhi
KDialog.MsgToGlobal("<color=yellow>Người chơi <color=green>["..me.szName.."]<color> thuần phục <color=black>Hắc Kỳ Lân<color> nhận được <color=green>50tr EXP<color>!<color>");	
end
-------
