...
function EPlatForm:MacthAward(szLeagueName, szMatchLeagueName, tbMisPlayerList, nResult)
	local tbMsg = 
	{
[1] = {string.format("<color=yellow>Đội của bạn đã đánh bại đối thủ trong trò chơi, chúc mừng chiến thắng.<color>")},
[2] = {string.format("<color=green>Đội của bạn đã hòa với đối thủ trong trận đấu, hãy tiếp tục cố gắng trong lần tới.<color>")},
[3] = {string.format("<color=blue>Đội của bạn đã thua đối thủ trong trò chơi, lần sau hãy cố gắng hơn.<color>")},
[4] = {string.format("<color=yellow>Đội của bạn tạm biệt cuộc thi này và bất ngờ giành chiến thắng.<color>")}
	}
...
--	local nGameLevel 	= League:GetLeagueTask(self.LGTYPE, szLeagueName, self.LGTASK_MLEVEL);
	...
	for _, szName in pairs(tbMemberList) do
		...
		if pPlayer then
			...
			if nResult == 1 or nResult == 4 then
				...
				pPlayer.SendMsgToFriend(string.format("Đội của bạn [%s] của bạn đã thắng sự kiện %s vừa kết thúc!", pPlayer.szName, szMatchName));
			end
		...
		EPlatForm:WriteLog(string.format("Phần thưởng cho thành viên nhóm: %s, %s so với %s, kết quả: %s", szName, szLeagueName, (szMatchLeagueName or ""), nResult))
	end
	
	local szKinName = self:GetKinNameFromLeagueName(szLeagueName);
	if nResult == 1 or nResult == 4 then
		local szKinMsg = string.format("Đội %s đã thắng sự kiện %s!", szLeagueName, szMatchName);
	...
	if EPlatForm:GetMacthState() == EPlatForm.DEF_STATE_ADVMATCH then
		...
		if nResult == 1 or nResult == 4 then
			...
			EPlatForm:WriteLog(string.format("Chiến thắng làm tăng số trận chiến thắng: %s", szLeagueName));
...
function EPlatForm:OnCheckAward(pPlayer, nPart)
	...
	if not szLeagueName then
		return 0, 0, "Bạn chưa có đội nào";
	end
	...
	if EPlatForm:GetMacthState() ~= EPlatForm.DEF_STATE_REST or KGblTask.SCGetDbTaskInt(EPlatForm.GTASK_MACTH_RANK) < EPlatForm:GetMacthSession() then
		return 0, 0, string.format("Thời gian thi đấu vẫn chưa kết thúc hoặc bảng xếp hạng cuối cùng của cuộc thi vẫn chưa được công bố, hãy kiên nhẫn chờ đợi.");		
	end
	
	if League:GetLeagueTask(EPlatForm.LGTYPE, szLeagueName, EPlatForm.LGTASK_TOTAL) <= 0 then
		return 0, 0, string.format("Đội của bạn chưa tham gia thi đấu, là đội mới nên không thể nhận phần thưởng.");	
	end
	local nRank = League:GetLeagueTask(EPlatForm.LGTYPE, szLeagueName, EPlatForm.LGTASK_RANK);
	...
	if pPlayer.GetTask(self.TASKID_GROUP, self.TASKID_MATCH_FINISH) >= nSession then
		return 0, 0 , string.format("Bạn đã nhận được phần thưởng của mình.");
	end	
	...
	if nLevelSep <= 0 then
		return 0, 0, string.format("Bang hội của bạn không có phần thưởng nào để yêu cầu.");
	end
	if nMaxRank >= 10000 and nTotle < math.floor(nMaxRank/10000) then
		return 0, 0 , string.format("Nhóm của bạn không nhận được phần thưởng nào trong cuộc thi này, hãy tiếp tục cố gắng trong lần tới.");
	end
...
function EPlatForm:OnGetAward_Final(nPart, nFlag)
	...
	if not nFlag then
local szMsg = string.format("Đội của bạn đã giành được vị trí <color=yellow>%s<color> trong sự kiện thi đấu gia đình lần trước. Sau khi nhận được phần thưởng, bạn sẽ rời khỏi đội. Nếu không còn thành viên nào trong đội , đội sẽ bị giải tán.", nRank);
		local tbOpt = 

{
{"Tôi chắc chắn nhận giải thưởng", self.OnGetAward_Final, self, nPart, 1},
{"Tôi sẽ nghĩ về nó"},
}
	...
	if me.CountFreeBagCell() < nFree then
		Dialog:Say(string.format("Hành trang không đủ chỗ trống hãy dọn trống %s.", nFree));
		return 0;
	end
...
Dialog:Say("Bạn đã nhận thưởng thành công và rời đội, hoan nghênh tiếp tục tham gia sự kiện tiếp theo.");
EPlatForm:WriteLog(string.format("Xếp hạng của sự kiện này: %s", nRank), me.nId, me.szName)
end
...
function EPlatForm:GetKinAward(nFlag)
	...
	szMsg = string.format("%s，Bạn có chắc chắn muốn nhận phần thưởng gia tộc không?？", szMsg or "");
	
	Dialog:Say(szMsg, 
		{
			{"Nhận thưởng", self.OnApplyGetKinAward, self, 1},
			{"Ta suy nghĩ lại"}	
		});
	return 1;
end
...
function EPlatForm:OnGetKinAward(nPlayerId)
	...
	if (nLevel <= 0) then
		self:WriteLog("GetKinAward", string.format("%s Thu thập phần thưởng không thành công", pPlayer.szName), nSession, nType);
		return 0;
	end
	...
	self:WriteLog("OnGetKinAward", string.format("%sNgười chơi đã nhận thành công phần thưởng gia tộc", pPlayer.szName), nSession, nLevel, nKinAwardType);
	return 1;	
end

function EPlatForm:OnCheckKinAward(pPlayer)
	if EPlatForm:GetMacthState() ~= EPlatForm.DEF_STATE_REST or KGblTask.SCGetDbTaskInt(EPlatForm.GTASK_MACTH_RANK) < EPlatForm:GetMacthSession() then
		return 0, string.format("Thời gian thi đấu vẫn chưa kết thúc hoặc bảng xếp hạng cuối cùng của cuộc thi vẫn chưa được công bố, hãy kiên nhẫn chờ đợi.");		
	end		
	
	local dwKinId, nMemberId = KKin.GetPlayerKinMember(pPlayer.nId);
	if (dwKinId <= 0) then
return 0, "Bạn không thể nhận phần thưởng gia tộc nếu bạn không có gia tộc";
	end
	
	local pKin = KKin.GetKin(dwKinId);
	if (not pKin) then
		return 0, "Bạn chưa tham gia gia tộc nào";
	end
	...
	local nFigure = pMember.GetFigure();
	if (nFigure <= 0 or nFigure > 3) then
return 0, string.format("Bạn không phải là thành viên <color=yellow> <color> chính thức của gia tộc <color=yellow>%s<color> <color>, và không thể nhận phần thưởng gia tộc!", szKinName);
	end
	
	if (self:GetPlayerMonthScore(pPlayer.szName) < self.DEF_MIN_KINAWARD_SCORE) then
		-- return 0, string.format("你的月活动积分没有达到%d分，不能领取奖励！", self.DEF_MIN_KINAWARD_SCORE);	
		return 0, string.format("Điểm hoạt động hàng tháng của bạn chưa đạt %d điểm nên bạn không thể nhận phần thưởng!", self.DEF_MIN_KINAWARD_SCORE);
	end

	local nKinAwardType = pKin.GetPlatformKinAward() or 0;
	if (nKinAwardType <= 0) then
		-- return 0, string.format("你所在家族没有奖励可以领取！");	
		return 0, string.format("Gia tộc bạn không có phần thưởng nào để nhận!");
	end
	local nSession = math.floor(nKinAwardType / 10000);
	local nType = math.fmod(nKinAwardType, 10000);
	...
	if (nSession == nMySession) then
	return 0, "Bạn đã nhận được phần thưởng gia tộc của gia tộc này";
	end
	
	local nKinCount = pKin.GetPlatformAwardCount();
	if (nKinCount >= self.DEF_MAX_KINAWARDCOUNT) then
	return 0, string.format("Đã nhận được %s số tiền thưởng gia tộc, bạn không thể nhận nó!", szKinName);
	end

	local nLevel = self:GetAwardLevelSep(self.MATCH_KINAWARD, nSession, nType);
	
	if (nLevel <= 0) then
self:WriteLog("GetKinAward", string.format("%s mức nhận phần thưởng không đúng", pPlayer.szName), nSession, nType);
return 0, "Bạn không đủ điều kiện nhận phần thưởng gia tộc";
	end
	
	if (not self.AWARD_KIN_LIST[nSession][nLevel]) then
	return 0, "Phần thưởng không tồn tại, vui lòng liên hệ với quản trị viên";
	end
	
	local nFree = EPlatForm.Fun:GetNeedFree(self.AWARD_KIN_LIST[nSession][nLevel]);
	if pPlayer.CountFreeBagCell() < nFree then
		return 0, string.format("Không gian ba lô của bạn không đủ, vui lòng sắp xếp %s không gian ba lô.", nFree);
	end
...
return 1, string.format("Gia tộc bạn đã nhận được phần thưởng <color=yellow>%s<color> trong sự kiện %d", nSession, tbResultName[nLevel]);
end
...
Dialog:Say("Không có phần thưởng để nhận, nếu bạn muốn nhận phần thưởng, vui lòng tham gia sự kiện!");
		return 0;
	end
...
function EPlatForm:OnGetAwardSingle(nAwardType, nSession, nAwardID)
	...
	if (self.MATCH_WELEE == nAwardType) then
		...
		if (not EPlatForm.AWARD_WELEE_LIST[nSession]) then
			Dialog:Say("không có phần thưởng");
			return 0;
		end
		...
		if me.CountFreeBagCell() < nFree then
			Dialog:Say(string.format("Không gian ba lô của bạn không đủ, vui lòng dọn dẹp không gian ba lô %s.", nFree));
			return 0;
		end
...
EPlatForm:WriteLog("OnGetAwardSingle", string.format("%s nhận phần thưởng vật phẩm cận chiến nSession, nAwardID", me.szName), nSession, nAwardID);
	-- 战队奖励
...
local szMsg = "Chúc mừng gia tộc <color=yellow>%s<color> của gia tộc <color=yellow>%s<color>, dưới sự lãnh đạo của đội trưởng <color=yellow>%s<color>, đã giành chiến thắng cuộc thi dành cho gia đình tháng này <color=yellow>%s<color> của <color=yellow>%s<color>";
local szMyMsg = "Chúc mừng đội của bạn đã chiến thắng %s"
