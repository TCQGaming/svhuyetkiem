...
function Partner:SendGift(nPartnerIndex)
	...
	local szMsg = "Tặng quà cho đồng hành có thể giúp người đó tăng lĩnh ngộ.\nQuà được tặng: <color=yellow>Huyền Tinh Cấp 1-9, Hồn Thạch, Thỏi vàng<color>";
...
function Partner:OnSendGiftOk(nPartnerIndex, tbItemObj)
	...
	for _, tbItem in pairs(tbItemObj) do
		...
		if self.tbItemTalentValue[szIndex] then
			...
			KDialog.MsgToGlobal("<color=pink>Đồng Hành Chỉ Ăn Được Thỏi Vàng, NHHT và Huyền Tinh <color>");	   
...
-----------------------------------Call_KichHoat_SkillPET--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function Partner:Call_KichHoat_SkillPET()

	local szMsg = string.format("Kích hoạt hiệu ứng Pet <color=red><color>？");
	local tbOpt = 
	{
		{"Kích Hoạt Hiệu Ứng", SpecialEvent.HieuUng_DongHanh.OnDialog, SpecialEvent.HieuUng_DongHanh, self, nPartnerIndex},
		{"Thoát"},
	}
	Dialog:Say(szMsg, tbOpt);
...
RegC2SFun("Call_KichHoat_SkillPET", Partner.Call_KichHoat_SkillPET);

------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function Partner:Call_DongHanh_NgungTu()
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- function Partner:Call_CheckAutoTCQ()
	-- Dialog:AskString("Nhập Mật Khẩu", 15, self.CheckMatNa, self);
-- end
function Partner:Call_KichHoat_Auto()
local check = me.GetTask(3028,1)
if check < 5000 then
		Dialog:Say("Plugin thuộc sever Tuyệt Đỉnh KT2009.Com bạn nhé!\nBạn cần đủ mốc nạp 5tr VNĐ mới có thể sử dụng đầy đủ tính năng");
	return
end
RegC2SFun("Call_KichHoat_Auto", Partner.Call_KichHoat_Auto);
end

------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

function Partner:InsertAPartners(nParam)
	...
	if (nPartnerId == 241) or (nPartnerId == 262) then -- Diệp tịnh
		me.AddStackItem(1,24,1,1,{bForceBind=1},1);
		me.DeletePartner(pPartner.nPartnerIndex);
	elseif(nPartnerId == 193) or (nPartnerId == 249) or (nPartnerId == 251) or (nPartnerId == 263) then
		me.AddStackItem(1,24,2,1,{bForceBind=1},1); -- Bảo Ngọc
		me.DeletePartner(pPartner.nPartnerIndex);
	elseif(nPartnerId == 182) or (nPartnerId == 247) or (nPartnerId == 261) then
		me.AddStackItem(1,24,3,1,{bForceBind=1},1); -- Hạ Tiểu Sảnh
		me.DeletePartner(pPartner.nPartnerIndex);
	elseif(nPartnerId == 194)  or (nPartnerId == 264) then
		me.AddStackItem(1,24,4,1,{bForceBind=1},1); -- Oanh Oanh
		me.DeletePartner(pPartner.nPartnerIndex);
	elseif(nPartnerId == 181) or (nPartnerId == 256) or (nPartnerId == 258) then
		me.AddStackItem(1,24,5,1,{bForceBind=1},1); -- Mộc Siêu
		me.DeletePartner(pPartner.nPartnerIndex);
	elseif(nPartnerId == 177) or (nPartnerId == 252) or (nPartnerId == 253) or (nPartnerId == 259) then
		me.AddStackItem(1,24,6,1,{bForceBind=1},1); -- Tử Uyển
		me.DeletePartner(pPartner.nPartnerIndex);
	elseif(nPartnerId == 178)  or (nPartnerId == 260) then
		me.AddStackItem(1,24,7,1,{bForceBind=1},1); -- Trần Trọng
		me.DeletePartner(pPartner.nPartnerIndex);
	else
		Dialog:Say("<color=red>Chú ý : Đồng hành này không thể ngưng tụ rồi <color>\nCó thể bạn chưa biết Đồng Hành Có thể ngưng tụ là: <color=yellow>\nBảo Ngọc\nHạ Tiểu Sảnh\nOanh Oanh\nMộc Siêu\nTừ Uyển\nTần Trọng\nDiệp Tịnh");
	return 0;
	end
end
RegC2SFun("Call_DongHanh_NgungTu", Partner.Call_DongHanh_NgungTu);
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
