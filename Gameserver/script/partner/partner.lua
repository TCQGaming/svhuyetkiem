...
function Partner:UpgradeSkill(pPartner)
	local nRes, varRes = self:CalToUpgradeSkill(pPartner);
	local szResult;
	if nRes == 0 then
		if string.find(varRes, "升满了") ~= nil then
			szResult = ", tất cả kỹ năng đã đạt tối đa";
		elseif string.find(varRes, "很遗憾") ~= nil then
			szResult = "Thất bại";
		end
	else
		szResult = "Thành công";
	...
	--提升技能的log
	local szLog = string.format("Log đồng hành: Đồng hành của %s nâng cấp skill lên 1 mức mới",me.szName );
	Dbg:WriteLog(szLog);
...
function Partner:CalToUpgradeSkill(pPartner)
	...
	return 1, tbSkillIndex[nGenIndex];
end
...
function Partner:TryToPersuade(pPlayer, pNpc, nItemLevel)
	...
	-- 指定的NPC是否被开放为可说服NPC
	if not self.tbPartnerAttrib[pNpc.nTemplateId] then
		self:SendClientMsg("Anh chàng này là nham hiểm hoặc không được gõ. Thay đổi hình ảnh tiếp theo cung cấp cho nó một thử.");
		return 0, "Anh chàng này là nham hiểm hoặc không được gõ. Thay đổi hình ảnh tiếp theo cung cấp cho nó một thử.";
	end
...
local tbTamTamTuongAnhPhu	= {18,1,1327,15,0,0}; 
	Task:DelItem(me, tbTamTamTuongAnhPhu, 2);
	...
	if MathRandom(1, 150) > nRate then
		...
		pPlayer.Msg("Thuyết phục thất bại!");
...
-- 相关的可叠加物品消耗都可以通过这个接口完成扣除操作，目前只有甜言蜜语丸和精魄使用到这里。
