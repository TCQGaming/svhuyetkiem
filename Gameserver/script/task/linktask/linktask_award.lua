...
function LinkTask:ShowAwardDialog(tbAward)
	...
	local nIDHuyenTinh = MathRandom(3,4)
	tbGeneralAward.tbFix	= {};
----------Phần thưởng bao vạn đồng------------------		
	if nDailyTaskNum == 9 and nDailyAward ~= tonumber(GetLocalDate("%Y%m%d")) then
		...
		table.insert(tbGeneralAward.tbFix,{szStatLogName="Nhiệm vụ Bao Vạn Đồng", szType="exp",varValue=nFixExp,nSprIdx=0,szDesc="Kinh nghiệm"});
		table.insert(tbGeneralAward.tbFix,{szStatLogName="Nhiệm vụ Bao Vạn Đồng",szType="bindmoney",varValue=nFixMoney * 1.5,nSprIdx=1,szDesc="額外銀兩"});
		...
		elseif me.nLevel >= 80 then
			...
			table.insert(tbGeneralAward.tbFix,{szStatLogName="Nhiệm vụ Bao Vạn Đồng", szType="item",varValue={18,1,114,nIDHuyenTinh,0,0,0,nil,0,0},nSprIdx=0,szDesc="藏寶圖"});
			table.insert(tbGeneralAward.tbFix,{szStatLogName="Nhiệm vụ Bao Vạn Đồng", szType="item",varValue={18,1,9,3,0,0,0,{bForceBind = 1},0,0},nSprIdx=0,szDesc="藏寶圖"});
		szAwardTalk = szAwardTalk.."\n\nTrong ngày bạn hoàn thành <color=green>10 Nhiệm Vụ<color> liên tiếp nên được nhận phần thưởng sau:";
		
	-- elseif nDailyTaskNum == 19 then
		-- if self:GetTask(self.tbExMoneyAward[nDailyTaskNum + 1]) == 0 then
		
			-- local nFixMoney		= math.floor(30000 * self:_CountLevelProductivity() / 2) * Task.IVER_nLinkTaskAward;	-- 額外金錢	
				-- table.insert(tbGeneralAward.tbFix,{szStatLogName="Nhiệm vụ Bao Vạn Đồng", szType="item",varValue={18,1,114,nIDHuyenTinh,0,0,0,nil,0,0},nSprIdx=0,szDesc="藏寶圖"});
				-- table.insert(tbGeneralAward.tbFix,{szStatLogName="Nhiệm vụ Bao Vạn Đồng", szType="item",varValue={18,1,114,5,0,0,0,nil,0,0},nSprIdx=0,szDesc="藏寶圖"});
			-- szAwardTalk = szAwardTalk.."\n\nTrong ngày bạn hoàn thành <color=green>"..nDailyTaskNum+1 .." Nhiệm Vụ <color> liên tiếp sẽ được nhận phần thưởng sau:"; -- nDailyTaskNum khi hoàn thành nó sẽ thông báo số mức hoàn thành
	-- end;
			-- elseif nDailyTaskNum == 29 then
		-- if self:GetTask(self.tbExMoneyAward[nDailyTaskNum + 1]) == 0 then
