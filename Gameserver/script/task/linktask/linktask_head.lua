...
LinkTask.PERDAY_NUM_MAX	= 10;	-- So nhiem vu BVD
...
function LinkTask:AwardFinish()
	...
	self:SetAwardState(2);
			local nDemBVD = me.GetTask(2024,80);
	...
	---------------- Phần Thưởng --------------------------------
	if nTaskNum == 9 then
			me.AddBindCoin(5*10000);-- 3v dong khoa
			me.AddBindMoney(5*10000); --5v bạc khóa
			me.AddStackItem(18,1,114,6,{bForceBind=1},1);
			me.AddKinReputeEntry(10);---Uy Danh 
			me.AddStackItem(18,1,553,1,{bForceBind=1},10)--10-Tiền Du Long
			me.AddExp(200000);--- 200k exp
			 			local nMocNap = me.GetTask(3028,1)
			me.SetTask(3028,1, nMocNap + 15);
			 me.Msg("Chúc mừng "..me.szName.." đã nhận được phần thưởng Hoàn thành 10 nhiệm vụ Bao Vạn Đồng nhận được :\n<color=gold>1 Huyền tinh 6\n15 Vạn tích lũy mốc nạp\n5 vạn đồng khóa\n5 vạn bạc khóa\n10 tiền du long\n10 uy danh\n 200k EXP!");

-- KDialog.MsgToGlobal("Chúc mừng "..me.szName.." đã nhận được phần thưởng Hoàn thành 10 nhiệm vụ Bao Vạn Đồng nhận được :\n<color=gold>1 Huyền tinh 6\n15 Vạn tích lũy mốc nạp\n5 vạn đồng khóa\n5 vạn bạc khóa\n10 tiền du long\n10 uy danh\n 200k EXP!")
				me.SetTask(2024,80,nDemBVD + 1);
			--------- End ---------
	local nMoKhamNam = ConfigAll.OpenKham
	local nSoLuongRuongDa = ConfigAll.nSoRuong_NV_BVD	
	...
	local nCountNV  = me.GetTask(tbNhiemVu.TaskGroup, tbNhiemVu.TaskBVD);
	me.SetTask(tbNhiemVu.TaskGroup, tbNhiemVu.TaskBVD,nCountNV + 1);
end;
