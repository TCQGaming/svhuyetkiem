...
-- kiếm hiệp thế giới môn phái nhiệm vụ liên đối thoại văn kiện 
-- Edited by peres
-- 2007/12/18 PM 03:05

-- nàng là thấu triệt đích 。
-- chỉ thị một dễ cảm giác cô gia độc người của ，hội muốn dùng có chút ảo giác lai gây tê chính 。
-- một trong tay khẩn trảo trứ chỗ trống đích nữ tử ，cuối cùng luôn hội nhượng chính thất vọng 。
...
-- ở đăng khan nhiệm vụ thì tựu điều dùng 
-- cây cứ truyện tiến lai đích nhiệm vụ loại hình và nhiệm vụ tham sổ lai thu hoạch một đoạn nhiệm vụ đích miêu tả ，hơn nữa ký lục đáo nhiệm vụ biến lượng trung 
...
-- cây cứ nhiệm vụ loại hình và nhiệm vụ Idlai thu hoạch một đoạn văn tự 
function LinkTask:GetTaskText(nType, nSubTaskId)
	...
	local szTargetName	= tbTaget.szTargetName; -- xong này mục tiêu đích tên 
	...
	if szTargetName =="SearchItemWithDesc"then
		...
		szMainText			= string.gsub(szOrgText,"<Item>","<color=green>"..nItemNum.."cá"..szItemName.."<color>");
	...
	elseif szTargetName =="SearchItemBySuffix"then
		...
		local nItemNum		= tbTaget.nNeedCount;
		szMainText			= string.gsub(szOrgText,"<Item>","<color=green>"..nItemNum.."món"..szItemName.."﹒"..szSuffix.."<color>");
		szMainText			= szMainText.."\n<color=yellow>Loại vật phẩm này có thể tạo ra bởi Kỹ năng sống.<color>";
