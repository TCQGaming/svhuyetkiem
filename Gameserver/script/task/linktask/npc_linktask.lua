	...
	if nContain >= LinkTask.CONTAIN_LIMIT then
		if me.nOnlineTime - nPauseTime >= LinkTask.PAUSE_TIME then
			...
			Dialog:Say("Bạn thiếu nhẫn nại vậy sao? Hãy nghỉ ngơi lát rồi quay lại!", {	
			          --{"<color=yellow>Nhận thưởng mốc 20,30,40,50<color>",SpecialEvent.MocBVD.OnDialog,SpecialEvent.MocBVD};			
					});
			return;
		end;
	end;	
...
----------------------reset task mua lai----------------
	...
	if me.GetTask(2024,77) < nDate then
		me.SetTask(2024,77, nDate);
		me.SetTask(2024,78, 0);
	end 
	...
	if nOpen==0 then
		if me.nLevel >=20 then
			Dialog:Say("Tại hạ Bao Vạn Đồng có chuyện thỉnh cầu, không biết đại hiệp có thể nghe ta nói không ?", {
			          ...
			          --{"<color=yellow>Mua phần thưởng 50 nhiệm vụ<color>",SpecialEvent.ReBuyBVD.OnDialog,SpecialEvent.ReBuyBVD};			
			          --{"<color=yellow>Nhận thưởng mốc 20,30,40,50<color>",SpecialEvent.MocBVD.OnDialog,SpecialEvent.MocBVD};			
					});
			return;
		else
...
function tbNpc:OpeningTalk()
	...
	local nCheck = me.GetTask(2024,78)
	if nCheck >= 1 then
		Dialog:Say("Hôm nay bạn đã hoàn thành nhanh bao vạn đồng, không được phép nhận nhiệm vụ nữa");
		return 0;
	end
	
	-- local nTask = me.GetTask(2024,80)
	-- me.SetTask(2024,80, nTask + 1);
...
function tbNpc:NextTask(nIsTeam)
	...
	if nMainTaskId <= 0 or nSubTaskId <= 0 then
		Dialog:Say("Hôm nay đại hiệp đã giúp Nghĩa quân hoàn thành nhiệm vụ, Lão Bao ta rất cảm kích, hãy nghỉ ngơi đi, <color=yellow>ngày mai trở lại<color>, ta còn nhiệm vụ giao cho người.");		
		return;
	end;
