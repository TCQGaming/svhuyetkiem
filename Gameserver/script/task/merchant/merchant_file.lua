...
function Merchant:InitTask()
	...
	tbAttribute["Context"]		= Lib:Str2Val(szTaskDesc);	-- 任務描述
...
function Merchant:NewRefer(nReferId)
		local tbReferData	= {};	
		local nReferIdx = 1;
		...
		tbReferData.tbDesc			= {szMainDesc=string.format("Giúp thương hội hoàn thành %s bước nhiệm vụ sẽ nhận được nhiều bạc và Huyền Tinh", self.TASKDATA_MAXCOUNT),tbStepsDesc={""}}
		
		tbReferData.tbVisable	= self.TaskContent.tbReferData.tbVisable;	    -- 可見條件
		tbReferData.tbAccept	= self.TaskContent.tbReferData.tbAccept; 		-- 可接條件
		...
		tbReferData.nReplyNpcId	= self.NPC_ID;		-- 回復 NPC
		tbReferData.szReplyDesc	= "Tìm Chủ Thương Hội nhận thưởng";		-- 回復文字
		...
		tbReferData.nBagSpaceCount, tbReferData.tbAwards.tbFix = self:GetAwardFix();
		return tbReferData;
end
...
function Merchant:LoadFile()
	...
	self.TaskFile = {};			--各小事件表
	self.TaskAwardFile = {};	--獎勵表
	self.TaskContent = {};		--固定內容表：
...
function Merchant:GetAwardFix()
	local tbFix = {};
	local nItemFree = 0; --物品空間
...
function Merchant:InitStep(nTypeId, nStepType, nLevelType, nTaskId, tbTargetFile)
	...
	if Task:GetPlayerTask(me).tbTasks[self.TASKDATA_ID] then
		Task:GetPlayerTask(me).tbTasks[self.TASKDATA_ID].tbReferData = self:NewRefer(self.TASKDATA_ID);
		...
		if nTypeId == Merchant.TYPE_DELIVERITEM or nTypeId == Merchant.TYPE_DELIVERITEM_NEW then
			...
			if #tbFind1 <= 0 and #tbFind2 <= 0 then
				if me.CountFreeBagCell() >= 1 then
					...
					me.Msg("Túi không còn chỗ trống, không thể lấy được Thư Thương Hội, hãy đến thương nhân nhận.");
				end
			end
		end
	end
	...
	--描述：
	--TBReferTask.tbDesc.szMainDesc = "主任務描述內容";
