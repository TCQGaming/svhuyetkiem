...
function tbItem_Plate:OnUse()
	...
	if (me.GetTask(2066, 287)>=3) then
		Dialog:SendInfoBoardMsg(me, "Mỗi Tuần Chỉ Tham Gia  <color=yellow>3<color> lần thôi !");
		return;
	end;
	...
	Dialog:Say("Giờ ngươi muốn tiến vào ?<enter><enter><color=yellow>Ta khuyên ngươi nêu có đủ 6 chiến hữu cùng tham gia cấp 85 trở lên<color>\n Lưu ý : <color=yellow> Mở Phó Bản Tốn 20 Vạn Đồng Thường , Khi tiến vào tốn 5 vạn đồng<color>.", {
			  {"Vâng ta muốn tiến vào",		self.OpenInstancing, self, me, it},
			  {"Khoan đã"},
			});
...
function tbItem_Plate:OpenInstancing(pPlayer, pItem)
	...
	-- 臨時寫法
	if (pPlayer.GetTask(2066, 287)>=3) then
		-- Dialog:SendInfoBoardMsg(me, "một tuần <color=yellow>3<color> lần !");
		Dialog:SendInfoBoardMsg(me, "Có thành viên đã đi đủ <color=yellow>3<color> lần !");
		return;
	end;
...
--	pPlayer.ConsumeItemInBags(1, 18, 1, 186, 1);
	me.AddJbCoin(-200000);--Trừ 20 Vạn Đồng
	 -- me.AddJbCoin(-50000);--Trừ 5 Vạn Đồng
	pItem.Delete(me);
