-------------------------------------------------------
-- văn kiện danh 　：kuozhanqunpc.lua
-- văn kiện miêu tả ：Hải Lăng Vương Mộ mở rộng khu 
-- sang xây người 　：ZhangDeheng
-- sang xây thời gian ：2009-04-14 10:43:10
-------------------------------------------------------
...
function tbMiDao:OnDialog()
	Dialog:Say("có hay không tiến vào", 
			{
				{"Tiến vào", self.Enter, self, him.dwId, me.nId},
				{"Tạm không tiến vào"},
			}
		);	
end;
...
tbQiHun.szDesc	="Kỳ Hồn";
tbQiHun.szText 	="kỳ bàn biến hóa ，áo hay vô cùng ，thế gian vạn vật ，túi la trong đó ，lai lai lai ，chúng ta hạ bàn kỳ 。";

function tbQiHun:OnDialog()
	Dialog:Say(self.szText, 
			{
				{"bắt đầu", self.FightStart, self, him.dwId, me.nId},
				{"kết thúc đối thoại"},
			}
		);
end;
...
tbYingHun.szDesc	="Ảnh Hồn";
tbYingHun.szText 	="có lẽ trên thế giới tối nan chuyện tình ，hay chiến thắng chính ，ngươi làm tốt chuẩn bị liễu mạ ？";
tbYingHun.tbCapyNpc	= {
		[1] = 4251,
		[2] = 4252,
...
function tbYingHun:OnDialog()
	Dialog:Say(self.szText, 
			{
				{"bắt đầu chiến đấu", self.FightStart, self, him.dwId, me.nId},
				{"kết thúc đối thoại"},
			}
		);
end;
...
function tbYingHun:AddCopyNpc(nId, nNo)
	...
	if (nIndex <1 or nIndex> 24) then
		print("[Hải Lăng Vương Mộ] ERROR: nFaction[".. nFaction.."], nRouteId[".. nRouteId.."].");
		nIndex = 1;
	end;
...
tbLiuYiBan_Dialog.szText ="<npc=4276>：chẩm ma là ngươi ，chẩm ma hoàn là ngươi ？ta thiểm tiên 。"
