...
local tbItem = Item:GetClass("army_hisbook")
tbItem.nTime = 5;
...
function tbItem:OnUse()
	...
	if nYear> 0 then
		...
		if nSec1 <nSec2 then
			me.Msg(string.format("Sau khi đọc 1 trang phải đợi 10 phút mới được đọc trang kế tiếp, bạn phải đợi <color=yellow>%s giây<color> nữa mới đọc được trang kế.", (nSec2 - nSec1)))
			return 0;
		end
	end
	local tbEvent = 
