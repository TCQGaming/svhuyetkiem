...
tbItem.nTime = 5;
tbItem.nRelayTime = 10*60 --cần diên thì 10phút hậu tài năng tái độc 
tbItem.nBookMax = 10;
tbItem.nSkillId = 798;
tbItem.tbArmyPart = {20,1,484,1}; --ki quan linh kiện ;
tbItem.nRate =	20; --mỗi độc nhất trang thu được linh kiện đích xác suất,tỉ lệ phần trăm 
tbItem.nExp	= 30000;

function tbItem:OnUse()
	if me.CountFreeBagCell() <1 then
		me.Msg("Hành trang của bạn không đủ chỗ trống, cần có 1 ô trống mới có thể đọc.");
		return 0;
	end
	...
	if nYear> 0 then
		...
		if nSec1 <nSec2 then
			me.Msg(string.format("Mỗi lần đọc xong 1 trang phải đợi 10 phút sau mới có thể chuyển trang. Bạn còn <color=yellow>%s giây<color> nữa mới có thể chuyển trang.", (nSec2 - nSec1)))
			return 0;
		end
	end
	local tbEvent = 
