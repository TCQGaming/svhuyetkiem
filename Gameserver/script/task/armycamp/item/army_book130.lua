...
local tbItem = Item:GetClass("army_book130")
tbItem.nTime = 5;
tbItem.nRelayTime = 1*60 --cần diên thì 10phút hậu tài năng tái độc 
tbItem.nBookMax = 10;
tbItem.nRepute = 3;
tbItem.nSkillId = 798;
tbItem.nExp	= 30000;
tbItem.nMoney = 100;
tbItem.nTaskGroupId = 2044;	--nhiệm vụ biến lượng Group
...
function tbItem:OnUse()
	...
	if nYear> 0 then
		...
		if nSec1 <nSec2 then
			me.Msg(string.format("Mỗi lần đọc xong 1 trang phải đợi 1 phút sau mới có thể chuyển trang. Bạn còn <color=yellow>%s giây<color> nữa mới có thể chuyển trang.", (nSec2 - nSec1)))
			return 0;
		end
	end	
	local tbEvent = 
...
function tbItem:SuccessUse(nItemId, nPlayerId)
	...
	if self.nBookMax - nUseCount <= 1 then
		pPlayer.SetTask(1022,182,1,1)
		if (pPlayer.DelItem(pItem, Player.emKLOSEITEM_USE) ~= 1) then
			assert(false);
		--	pPlayer.Msg("bỏ đi binh thư thất bại ！");
			return 0;
		end
		pPlayer.AddExp(self.nExp);
		pPlayer.Earn(self.nMoney, Player.emKEARN_TASK_ARMYCAMP);
		KStatLog.ModifyAdd("jxb","[Nơi] Nhiệm vụ Quân doanh","Tổng", self.nMoney);
...
function tbItem:InitGenInfo()
	-- thiết định thời hạn có hiệu lực hạn 
