...
tbBase.HONOR_CLASS_MONEY		= 8;	-- 荣誉大类，财富荣誉
tbBase.tbHonorLevelInfo	= {
	["money"]	= {
		nHonorId	= tbBase.HONOR_CLASS_MONEY,
	},
};
...
-----------------------giới hạn tp tranh đoạt lãnh thổ-----------
		...
		Dialog:SendBlackBoardMsg(pPlayer, ConfigAll.ThongBaoGioiHan);
		return 0;
		end
		end
-----------------------------------------------------------------
		...
		Dialog:SendBlackBoardMsg(pPlayer, "Bạn đã bước vào bản đồ chinh chiến bang hội, tự động chuyển trạng thái chiến đấu sang Bang hội");		-- 黑条
		
		-- 成就：参加领土争夺
		Achievement:FinishAchievement(pPlayer.nId, Achievement.DOMAINBATTLE);
	...
	if self.nState == Domain.BATTLE_STATE then
		if self.tbPlayer and self.tbPlayer[pPlayer.nId] then
			...
			if nScore > 0 and self.tbPlayer[pPlayer.nId].bSyncGC == 1 and Domain.CLOSE_SYNC_TO_GC ~= 1 then
				GCExcute{"Domain:SetTongPlayerScore_GC", pPlayer.dwUnionId, pPlayer.dwTongId, pPlayer.nId, nScore}
...
function tbBase:AddPlayerScore(pPlayer, nSelfScore, nTeamScore, szMsg)
	...
	pPlayer.Msg(string.format("Bạn%snhận được<color=yellow>%d điểm<color> công trạng lãnh thổ!", szMsg, nSelfScore));
