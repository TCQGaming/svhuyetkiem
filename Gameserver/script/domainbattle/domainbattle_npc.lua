...
Domain.HONOR_CLASS_MONEY		= 8;	-- 荣誉大类，财富荣誉
Domain.tbHonorLevelInfo	= {
	["money"]	= {
		nHonorId	= Domain.HONOR_CLASS_MONEY,
	},
};
-- 奖励Npc响应对话
...
-----------------------giới hạn tp tranh đoạt lãnh thổ-----------
		...
		Dialog:SendBlackBoardMsg(me, "Bạn không đạt đủ điểm Tài Phú để nhận thưởng Tranh Đoạt Lãnh Thổ");		-- 黑条
		return 0;
		end
		end
-----------------------------------------------------------------
...
function Domain:DeclareWar_Confirm(nDomainId, nTongId)	
	local tbDlg = {
		{"Xác định", self.DeclareWar_GS1, self, nDomainId, nTongId},
		{"Để ta suy nghĩ đã"},
	}
	local szDomainName = self:GetDeclareTongNames(nDomainId);
	if  szDomainName then
		Dialog:Say("Bạn có chắc là bạn chọn<color=yellow> "..self:GetDomainName(nDomainId)..
			"<color> Là mục tiêu được tuyên bố của bang hội trong cuộc chiến tranh lãnh thổ này?\n<color=yellow>Bang Hội hiện đang tuyên chiến trên lãnh thổ ：\n<color><color=green>"..
			szDomainName.."<color>", tbDlg);
	else
		Dialog:Say("Bạn có chắc là bạn chọn <color=yellow>"..self:GetDomainName(nDomainId).."<color> Là mục tiêu được tuyên bố của bang hội trong cuộc chiến tranh lãnh thổ này?", tbDlg);
	end
end
...
function Domain:SetJunXu(nType, nLevel, nNum, nConfirm)
	...
	if not nConfirm or nConfirm ~= 1 then
		...
		{
			{"Xác định", self.SetJunXu, self, nType, nLevel, nNum, 1},
			{"Kết thúc đối thoại"},
		})
		return 0;
	end
