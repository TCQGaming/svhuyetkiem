...
function Domain:EndAllGame()
	...
	local szMsg = "ranh đoạt lãnh thổ kết thúc!"
	KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, szMsg);
end
...
function Domain:_HasBattleRight(nTongId,nMapId)
	...
	if self:GetDomainType(nDomainId) == "village" then
		return 0; 		-- 只有宣战才能攻打Tân thủ thôn 
	end
	
	-- 如果占有Tân thủ thôn 可供打任意有归属的领土
...
-- 开始计算奖励
	...
	local nPersonalExp = nExpFactor * self.DOMAINBATTLE_SCORE_RATE_TABLE[nScoreLevel][4] * me.GetBaseAwardExp();
	-- 取整
	nPersonalExp = math.floor(nPersonalExp);
	
	local nDirectRepute = math.floor(nPersonalRepute * self.REPUTE_PERCENT);
	...
	local nBoxAward = math.floor(nBoxRepute / self.BOX_REPUTE_PARAM);
...
-- 确认认领
	...
	local nFreeCount, tbFunExecute = SpecialEvent.ExtendAward:DoCheck("DomainbattleTask", me, nScore);
...
-- 给予奖励
	...
	me.AddRepute(self.CAMP_DOMAINBATTLE, self.CLASS_DOMAIN, nDirectRepute);
	...
	local nRemain = nRemain + self.BOX_REPUTE_PARAM * (nBoxAward - nGiven)
...
function Domain:SetTongAward(nLevel, bConfirme)
	...
	if nRet ~= 1 or nCurTime < self.TONG_AWARD_START_TIME or nCurTime > self.TONG_AWARD_END_TIME then 
		Dialog:Say("Cuối tuần thứ 7, chủ nhật vào lúc 21:30 đến 22:00 trong lúc đó đến để thiết lập ngạch quân thưởng.");
		return 0;
	end
	...
	if (bConfirme == 0) then	
		...
		Dialog:Say("    Thủ lĩnh khả dĩ đi qua thiết trí quân lương ngạch độ, tiêu hao bộ phận bang hội kiến thiết tài chính lai thưởng cho tại lãnh thổ chiến trung thành lập trác việt công huân đích bang hội thành viên. Bất đồng đích quân lương ngạch độ hạ, bang hội thành viên có thể lĩnh đích thưởng cho cũng không đồng, ngạch hơn đại thành viên thu được đích thưởng cho cũng sẽ càng nhiều. \n  <color=yellow> nâm đích bang hội trước mặt còn không có thiết trí thưởng cho ngạch độ. <color>\n\n ta yếu thiết trí thưởng cho ngạch độ:", 
			tbOpt); 

		return 0;		
	end
	
	-- 如果选择了帮会Hình thức thưởng 而帮会建设资金不足
...
function Domain:GetScoreLevel(cPlayer, nMinLimit)
	...
	return nScoreLevel, math.max(cTong.GetDomainAwardLimit(5), nMinLimit);
end
...
function Domain:FatchJunXu_GS2(nTongId, nPlayerId, nType, nParticular, nLevel, nSucceed, nCostedBuildFund)
	...
	if nSucceed == 1 then	
		if nType == self.JUNXU_MEDICINE then 
			nJunXuLevel = nLevel + 1;
		elseif nType == self.JUNXU_HELPFUL then
			nJunXuLevel = nLevel;
		end
		local pItem = pPlayer.AddItem(nGenre, nDetail, nParticular, nJunXuLevel);
		if not pItem then
