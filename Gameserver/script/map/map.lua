...
-- 动态地图类型（与gc_core/kgc_mapmanager_def.h的枚举对应）
...
function Map:OnEnter(nMapId, szParam)
	...
	if GetMapType(me.nMapId) == "taskfight" or GetMapType(me.nMapId) == "tasksafe" then-- class map bản đồ nhiệm vụ
	me.Msg("Nơi này không thể đến")
	me.NewWorld(24,1750,3544);
	return 0;
	end
	   local nMapId, nPosX, nPosY = me.GetWorldPos()
	if nMapId == 124 then
	me.Msg("Nơi này không thể đến Bản đồ này ADM đã đóng vĩnh viễn")
	me.NewWorld(24,1762,3517);
	return 0;
	end
	
		local nCurTime = tonumber(GetLocalDate("%H%M"));
	if (nCurTime >= 2200 and nCurTime < 2240) then
	if nMapId == 132 then
		Map:RegisterMapForbidReviveType(132,0,0, "Bản đồ hái quả cấm hồi sinh tại chỗ và hồi sinh kỹ năng");
		me.nPkModel = Player.emKPK_STATE_TONG;
	end 
	end

	----------------------------------------------------------Nhiệm vụ ngày hoàng kim TCQGaming------------------------------------------------------------------------------------	
	...
	----------------------------------------------------------Check Ban Vi Phạm TCQGaming------------------------------------------------------------------------------------
local tbBanTimers_ViPham = {} -- Bảng lưu trữ đối tượng đếm thời gian của người chơi bị ban
local szBanFile_ViPham = "\\script\\global\\tcqgaming\\npc\\ban\\BanList.txt"
local szData_ViPham = KFile.ReadTxtFile(szBanFile_ViPham)

if szData_ViPham then
    local banList_ViPham = {}
    for line in szData_ViPham:gmatch("[^\r\n]+") do
        local id, name = line:match("(%S+)\t(%S+)")
        if id and name then
            banList_ViPham[id] = true
        end
    end
    local banId_ViPham = tostring(me.GetTask(2406, 1))
    if banList_ViPham[banId_ViPham] then
        local banTimer_ViPham = tbBanTimers_ViPham[me]
        if banTimer_ViPham then
            if banTimer_ViPham:GetTime() <= 0 then
                me.KickOut()
                tbBanTimers_ViPham[me] = nil
                return
            else
                me.Msg("Bạn đã bị ban. Vui lòng thử lại sau.")
                return
            end
        end
        local interval_ViPham = 1 -- Khoảng thời gian cập nhật (số giây)
        local countdown_ViPham -- Thời gian đếm ngược (số giây)
        local timerId_ViPham -- ID của timer
        local function CountdownTimer_ViPham()
            countdown_ViPham = countdown_ViPham - interval_ViPham
            if countdown_ViPham > 0 then
                me.CallClientScript({"UiManager:OpenWindow", "UI_INFOBOARD", string.format("<color=yellow>Máy tính bị cấm truy cập. Thời gian còn lại: %d giây", countdown_ViPham)})
                me.Msg("<color=cyan>Máy tính này của bạn đã bị ban. Tất cả các tài khoản trên máy tính này đều bị cấm truy cập. Vui lòng liên hệ GM nếu cảm thấy đây là một lỗi.")
            else
                me.CallClientScript({"UiManager:CloseWindow", "UI_INFOBOARD"})
                me.KickOut()
                tbBanTimers_ViPham[me] = nil
                Timer:Close(timerId_ViPham) -- Đóng timer khi đếm ngược kết thúc
                return
            end
        end
        countdown_ViPham = 3 -- Thiết lập thời gian đếm ngược ban đầu
        timerId_ViPham = Timer:Register(Env.GAME_FPS * interval_ViPham, CountdownTimer_ViPham) -- Đăng ký hàm CountdownTimer để tiếp tục đếm ngược sau khoảng thời gian interval
        tbBanTimers_ViPham[me] = timerId_ViPham
        me.Msg("Bạn đã bị ban. Vui lòng thử lại sau.")
        return
    end
end
	----------------------------------------------------------Check Ban Treo Tống Kim TCQGaming------------------------------------------------------------------------------------
local tbBanTimers_TreoTongKim = {} -- Bảng lưu trữ đối tượng đếm thời gian của người chơi bị ban
local szBanFile_TreoTongKim = "\\script\\global\\tcqgaming\\npc\\ban\\BanTongKimTreo.txt"
local szData_TreoTongKim = KFile.ReadTxtFile(szBanFile_TreoTongKim)

if szData_TreoTongKim then
    local banList_TreoTongKim = {}
    for line in szData_TreoTongKim:gmatch("[^\r\n]+") do
        local id, name = line:match("(%S+)\t(%S+)")
        if id and name then
            banList_TreoTongKim[id] = true
        end
    end
    local banId_TreoTongKim = tostring(me.GetTask(2406, 1))
    if banList_TreoTongKim[banId_TreoTongKim] then
        local banTimer_TreoTongKim = tbBanTimers_TreoTongKim[me]
        if banTimer_TreoTongKim then
            if banTimer_TreoTongKim:GetTime() <= 0 then
                me.KickOut()
                tbBanTimers_TreoTongKim[me] = nil
                return
            else
                me.Msg("Bạn đã bị ban. Vui lòng thử lại sau.")
                return
        ...
        local interval_TreoTongKim = 1 -- Khoảng thời gian cập nhật (số giây)
        local countdown_TreoTongKim -- Thời gian đếm ngược (số giây)
        local timerId_TreoTongKim -- ID của timer
        local function CountdownTimer_TreoTongKim()
            countdown_TreoTongKim = countdown_TreoTongKim - interval_TreoTongKim
            if countdown_TreoTongKim > 0 then
                me.CallClientScript({"UiManager:OpenWindow", "UI_INFOBOARD", string.format("<color=yellow>Máy tính bị cấm truy cập. Thời gian còn lại: %d giây", countdown_TreoTongKim)})
                me.Msg("<color=cyan>Máy tính này của bạn đã bị ban do treo clone trong Trận Tống Kim 21h00. Lệnh cấm sẽ được xóa khi trận Tống Kim kết thúc đây là hệ thống tự động.")
            else
                me.CallClientScript({"UiManager:CloseWindow", "UI_INFOBOARD"})
                me.KickOut()
                tbBanTimers_TreoTongKim[me] = nil
                Timer:Close(timerId_TreoTongKim) -- Đóng timer khi đếm ngược kết thúc
                return
            end
        end
        countdown_TreoTongKim = 10 -- Thiết lập thời gian đếm ngược ban đầu
        timerId_TreoTongKim = Timer:Register(Env.GAME_FPS * interval_TreoTongKim, CountdownTimer_TreoTongKim) -- Đăng ký hàm CountdownTimer để tiếp tục đếm ngược sau khoảng thời gian interval
        tbBanTimers_TreoTongKim[me] = timerId_TreoTongKim
        me.Msg("Bạn đã bị ban. Vui lòng thử lại sau.")
        return
    end
end
