...
function ServerEvent:OnClientCall(tbCall)
	...
	if not tbCall or type(tbCall) ~= "table" then-- kiểm tra callsv tào lao
		return;
	end
	local Version = 343158
	--if me.GetTask(2405,1) ~= Version then
		--me.CallClientScript({"Ui:ServerCall", "UI_TASKTIPS", "Begin", "Client không hợp lệ nếu bạn là người chơi vui lòng liên hệ GM"});			 
	--	me.CallClientScript({"UiManager:OpenWindow", "UI_DISCONNECT"});
	--end
	if tostring(tbCall[1]) == "ProCmd"  then
	 local t = math.floor(os.clock());
	local currentTime = me.GetTask(9155,1);
	if t < currentTime then
		me.SetTask(9155,1,0);
	end
	if( t - currentTime ) < 0.8 then --- cho hẳn 10s
