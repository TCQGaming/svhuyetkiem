...
function c2s:ProCmd(szFun,...)-- chổ này có thể spam bay luôn gc bằng cái lưu thông tin nhân vật
	if type(szFun) ~= "string" then
		return
	end
...
-- 师徒传送
function c2s:ShiTuChaunSongCmd(szFun, ...)-- này là phù đồng tâm của đám cưới nó có thể phá qua đây
if me.nLevel < 150 then
return;
end
	if (type(szFun) ~= "string") then
...
-- 夫妻传送
function c2s:tbFuQiChuanSongCmd(szFun, ...)--này là hệ thống phù liên sv có thể call phá từ client qua
if me.nLevel < 150 then
return;
end
	if (type(szFun) ~= "string") then
...
function c2s:UseUnlimitedTrans(nMapId)
	...
	if tonumber(nMapId) < 1 then
		return;
	end
	if tonumber(nMapId) > 2013 then-- chổ này xem thử world set em là map bao nhiêu cao nhất làm điều kiện chổ này, lớn hơn 2013 nghĩa là ko có map  anh có thể cho lại bỏ cái cột đi đc ko =)) nhìn hoa mắt ghê
	Dialog:Say("bản đồ không tồn tại")
		return;
	end
