...
local tbNpcBai = Npc:GetClass("trangbihotro");
		...
		{	-- Thiếu lâm
			{	-- Dao Thiếu Lâm
				[0] = {	--nam
					...
					{2, 10, 408-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},	--护腕cuff
					{2,  5, 206-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},	--项链necklace
					{2,  4, 206-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},	--戒指ring
				...
				[1] = {	--nu
					...
					{2, 10, 418-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},
					{2,  5, 206-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},
					{2,  4, 206-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},
			...
			{	-- 	con thieu lam
				[0]	= {
					...
					{2, 10, 408-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},
					{2,  5, 206-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},
					{2,  4, 206-6+tbNpcBai.nArmor_Level, tbNpcBai.nArmor_Level, -1},
...
function tbNpcBai:LevelUpPlayer(nPosStartIdx)
	...
	for i = nPosStartIdx, Player.FACTION_NUM do
		if (nCount <= 0) then
			tbOpt[#tbOpt]	= {"下一页", self.LevelUpPlayer, self, i - 1};
			break;
		end
...
-- dieu kien nhan roi
	...
	for i = nPosStartIdx, Player.FACTION_NUM do
		if (nCount <= 0) then
			tbOpt[#tbOpt]	= {"Trang tiếp theo", self.GetEquipFaction, self, i - 1, nQiangHua};
			break;
		end
...
function tbNpcBai:GetEquipRoute(nFactionId, nPosStartIdx, nQiangHua)
	...
	for i = nPosStartIdx, #Player.tbFactions[nFactionId].tbRoutes do
		if (nCount <= 0) then
			tbOpt[#tbOpt]	= {"下一页", self.GetEquipRoute, self, nFactionId, i - 1, nQiangHua};
			break;
		end
		tbOpt[#tbOpt+1]	= {Player:GetFactionRouteName(nFactionId, i).."60", self.GetEquip, self, nFactionId, i, nQiangHua};
		nCount	= nCount - 1;
	end;
	tbOpt[#tbOpt+1]	= {"Kết thúc đối thoại"};
...
function tbNpcBai:GetHorse_60Level()
	...
	if me.CountFreeBagCell() < 5 then
		Dialog:Say("Túi của bạn không đủ chỗ trống需要5格背包空间。");
		return 0;
	end
	for ni, tbItem in pairs(self.tbHorse_60Level) do
		local pItem = me.AddItem(unpack(tbItem));
