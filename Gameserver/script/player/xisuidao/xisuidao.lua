...
function Xisuidao:CanDuoxiu(pPlayer)
	...
	if (pPlayer.nLevel < 50) then
		Setting:SetGlobalObj(pPlayer);
		Dialog:Say("Đẳng cấp phải đạt <color=red>cấp 50<color> mới có thể phụ tu sang môn phái khác.");
		Setting:RestoreGlobalObj();
		return 0;
	end
...
function Xisuidao:OnEnterXisuidao_Duoxiu(pPlayer)
	...
	local tbOpt = {
		{"Ta đã hiểu rõ, hãy đưa ta vào.", self.EnterXisuidao_Duoxiu, self, pPlayer},
		{"Kết thúc đối thoại"},
	};
...
function Xisuidao:OnEnter(pPlayer, nCount)
	...
	if (0 < nCount) then
		...
		if (nPaiCount > nLingpaiCount) then
			Setting:SetGlobalObj(pPlayer);
			Dialog:Say("Bạn đã vào Tẩy Tủy Đảo.");
			Setting:RestoreGlobalObj();
			return;
		end
