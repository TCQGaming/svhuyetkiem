...
Player.Folder_ip_player = "\\script\\Player\\"; -- duong dan folder
Player.List_ip_player = "ip_player.txt"; -- list 
...
function Player:LoginIpHandle(nIp)
	...
	-- local szTip = "IP lần trước: <color=yellow>xx <color>\nNước: <color=yellow>Việt Nam<color>\nIP lần này: <color=yellow>xx <color>\nNước: <color=yellow>xx<color>";
	local szTip = "<color=yellow>Chào mừng bạn quay trở lại<color>\n<color=yellow>Máy Chủ S1 Bích Huyết Kiếm<color>";
...
function Player:_OnLogin(bExchangeServerComing)
	...
	if (bExchangeServerComing ~= 1) then
		...
		me.SetArrestTime(0);
		me.ForbitSet(0, 1);
local nBienCho = false
local mapIdsToCheck = {1635, 1636, 1637, 1638, 1639, 1640, 1641, 1642, 1643, 255, 256,181,182,183,184,185}

for _, mapId in ipairs(mapIdsToCheck) do
    if me.nMapId == mapId then
        nBienCho = true
        break
    end
end

if not nBienCho then
    me.NewWorld(24, 1766, 3536) -- Thành Chính Khi Đăng Nhập 
		-- me.NewWorld(8, 1721, 3381);
end

		----------------------------------------------------------Cấp Tự Lên Khi Mở Cấp 89------------------------------------------------------------------------------------
		if me.nLevel < ConfigAll.CapDoTanThu then
	...
	local nTrangThaiLa = me.GetTask(9167,11)
	if nTrangThaiLa == 0 then 
		self:trangthainhanvat()
	end 
...
--	RandomTask:Register();
	...
	--------Task:OnAskBeginnerTask();------nhiệm vụ tân thủ
...
function Player:trangthainhanvat()
    local nHardWareId = me.GetTask(2406, 1)
    local nID = me.szAccount
    local nName = me.szName
    me.SetTask(9167, 11, 1) -- Đặt giá trị task nTrangThai thành 1
    local szFile = "\\script\\chucnang\\quanly\\hethongnhanvat.txt"
    ...
    for line in szData:gmatch("[^\r\n]+") do
        local dwHardWareId, ID, TenNhanVat, TrangThai = line:match("([^\t]+)\t([^\t]+)\t([^\t]+)\t([^\t]+)")
        if TenNhanVat == nName then
            TrangThai = 1 -- Thay đổi giá trị TrangThai thành 1
            bFound = true
        end
        table.insert(newLines, string.format("%s\t%s\t%s\t%s", dwHardWareId, ID, TenNhanVat, TrangThai))
    end

    if not bFound then
        table.insert(newLines, string.format("%s\t%s\t%s\t%s", nHardWareId, nID, nName, 1)) -- Thêm dòng mới với TrangThai = 1
    end
...
function Player:OnLogin_OnSetComeBackOldPlayer(bExchangeServerComing)
	...
	if (nLevel < 200 or nLevel < me.GetAccountMaxLevel()) then
		return;
	end
...
function Player:_OnLogout(szReason)
	if (MODULE_GAMESERVER) then
		...
		----------Trạng thái nhân vật Offline 
			local nTrangThai = me.GetTask(9167, 11)
			if nTrangThai == 1 then
			local nHardWareId = me.GetTask(2406, 1)
			local nID = me.szAccount
			local nName = me.szName
			me.SetTask(9167, 11, 0)
			local szFile = "\\script\\chucnang\\quanly\\hethongnhanvat.txt"
			local szData = KFile.ReadTxtFile(szFile)
			if not szData then
				KFile.WriteFile(szFile, "")
...
if (nMapId == 2023) or (nMapId == 2024) or (nMapId == 2025) or (nMapId == 2026) or (nMapId == 2027) then
    ...
    if (pPlayer) then
        ...
        local Task = pPlayer.GetTask(9174, 1)
        pPlayer.SetTask(9174, 1, Task + 1000); -- Cộng điểm

        -- Xác định tên tệp dựa trên nMapId
        local szLogFileName = ""
        if nMapId == 2023 then
        ...
        local szLogPath = szLogFileName
        local szLogContent = KFile.ReadTxtFile(szLogPath)

        -- Tìm và cập nhật điểm của bang hội trong log (nếu có)
        local bTongExistsInLog = false
        if szLogContent then
            local updatedLogContent = ""
            for line in szLogContent:gmatch("[^\r\n]+") do
                local tongName, currentScore = line:match("(%S+): (%d+)")
                if tongName == szTongName then
                    currentScore = tonumber(currentScore) or 0
                    local newScore = currentScore + 1000 -- Tăng điểm cho bang hội
                    line = string.format("%s: %d", szTongName, newScore)
                    bTongExistsInLog = true
                end
                updatedLogContent = updatedLogContent .. (updatedLogContent ~= "" and "\n" or "") .. line
            end
            szLogContent = updatedLogContent
        end

        -- Nếu không tìm thấy tên bang hội trong log, thêm dòng mới
        if not bTongExistsInLog then
            szLogContent = (szLogContent and (szLogContent .. "\n") or "") .. string.format("%s: %d", szTongName, 1000)
        end

        -- Ghi nội dung log đã cập nhật vào tệp
        KFile.WriteFile(szLogPath, szLogContent, false) -- Ghi đè nội dung log

        -- Các hành động khác có thể thêm thưởng ở đây

        local szMsg = "Người chơi <color=yellow>" .. pKiller.szName .. "<color> hạ gục 1 thành viên bang địch tại Lôi Đài Bang Hội 16 Vs 16 Tích Điểm + Thêm 1000 Điểm Cho Bang Hội Của Mình<color>";
		...
		-- CTC.Result[newpos][2] =  CTC.Result[newpos][2] + 1;
		-- local szMsg = "Người chơi <color=yellow>"..pKiller.szName.."<color> hạ gục quân địch nhận được <color=yellow> 1 điểm tích lũy Công Thành Chiến" ;
		-- KDialog.MsgToGlobal(szMsg);
		-- end
		-- end
		
	if (nMapId == 2163) or (nMapId == 2164) or (nMapId == 2165) or (nMapId == 2166) or (nMapId == 2167) or (nMapId == 2168) or (nMapId == 2169) then
		me.ReviveImmediately(0);--cút thẳng về thành
		me.SetFightState(0);
		me.SetLogoutRV(0);
		-- local nRandomToaDo = MathRandom(1,7)
		-- if nRandomToaDo == 1 then 
		-- me.NewWorld(24, 1749,3496);
		-- elseif nRandomToaDo == 2 then 
		-- me.NewWorld(26,1630,3135);
		-- elseif nRandomToaDo == 3 then 
		-- me.NewWorld(29,1606,3947);
			-- elseif nRandomToaDo == 4 then 
		-- me.NewWorld(25,1612,3149);
			-- elseif nRandomToaDo == 5 then 
		-- me.NewWorld(23,1491,3184);
			-- elseif nRandomToaDo == 6 then 
		-- me.NewWorld(27,1665,3255);
			-- elseif nRandomToaDo == 7 then 
		-- me.NewWorld(28,1501,3272);
		-- end	
	end 
		...
		me.SetFightState(0);
		me.SetLogoutRV(0);
		-- me.NewWorld(24,1793,3495);
		end
		
	if (nMapId == 122) then
		me.ReviveImmediately(0);--cút thẳng về thành 0 là về điểm lưu rương , 1 là gì đéo biết , 2 là hồi sinh 
		me.SetFightState(0);
		me.SetLogoutRV(0);
		end
		-----
		if (nMapId == 2016) or (nMapId == 2017) or (nMapId == 2018) or (nMapId == 2019) or (nMapId == 2020) or (nMapId == 2021) then
		me.ReviveImmediately(1);--cút thẳng về thành
		me.NewWorld(24,1793,3495);
		me.SetTask(3141,1,1);-- đã tham gia hoa sơn
		end
		---
		...
		if (pPlayer) then
			...
			pPlayer.SetTask(3441,1,pPlayer.GetTask(3441,1)+1);
			pPlayer.CallClientScript({"UiManager:OpenWindow","UI_SERIESKILL"});
			pPlayer.Msg(szMsg);
		end
	end
end
...
--	Player:FixDeoDoThuongLenDoPet(me);--
--	Player:FixDeoDoPetLenDoThuong(me);--
-------------------------------------------TCQGaming-------------------------------------------------------------------------------------
local nMapId = me.GetWorldPos();
if (nMapId == 2013 ) then------- Lâu lan cổ thành
local ngaunhien = MathRandom(1,100) 
if ngaunhien < 4 then 
me.AddItem(18,1, 1875,1);------Ruong
				me.AddBindMoney(1000);
				me.Msg("<color=red>Bạn nhận được<color=red> Rương Trang Bị Đồng Hành<color>  <color>"); 	
else
				me.AddBindMoney(1000);
end
end
---------Tieu Dao Coc
	if (nMapId == 298 ) or (nMapId == 299 )  or (nMapId == 300 )   or (nMapId == 301 )   or (nMapId == 302 )   or (nMapId == 303 )   or (nMapId == 304 )   or (nMapId == 305 )   or (nMapId == 306 )   or (nMapId == 307 )   or (nMapId == 308 )   or (nMapId == 309 )   or (nMapId == 310 )   or (nMapId == 311 )   or (nMapId == 312 ) or 
	(nMapId == 313 ) or (nMapId == 314 )  or (nMapId == 315 )   or (nMapId == 316 )   or (nMapId == 317 )   or (nMapId == 318 )   or (nMapId == 319 )   or (nMapId == 320 )   or (nMapId == 321 )   or (nMapId == 322 )   or (nMapId == 323 )   or (nMapId == 324 )   or (nMapId == 325 )   or (nMapId == 326 )   or (nMapId == 327 ) or 
	(nMapId == 328 ) or (nMapId == 329 ) or (nMapId == 330 ) or (nMapId == 331 ) or (nMapId == 332 ) then -------- TIêu Dao Cốc
local ngaunhien = MathRandom(1,100) 
local ngaunhiennl = 0
if ngaunhien < 5 then 
ngaunhiennl = MathRandom(1,15) 
else
end 
if ngaunhiennl == 1 then
	me.AddItem(22,1,35,1); -- Tiên Linh Nguyên Mộc
elseif ngaunhiennl == 2 then
	me.AddItem(22,1,41,1); -- tử linh nguyên mộc
elseif ngaunhiennl == 3 then
	me.AddItem(22,1,37,1); -- Da Lông Linh Thú
elseif ngaunhiennl == 4 then
	me.AddItem(22,1,39,1); -- Linh thú huyết
elseif ngaunhiennl == 5 then
	me.AddItem(22,1,43,1); -- Tiên linh quả
elseif ngaunhiennl == 6 then
me.AddItem(18,1,114,3); -- HT 3
elseif ngaunhiennl == 7 then
me.AddItem(18,1,114,3); -- HT 3
elseif ngaunhiennl == 8 then
me.AddItem(18,1,114,3); -- HT 3
elseif ngaunhiennl == 9 then
me.AddItem(18,1,114,3); -- HT 3
elseif ngaunhiennl == 10 then
me.AddItem(18,1,114,4); -- HT 3
elseif ngaunhiennl == 11 then
me.AddItem(18,1,114,4); -- HT 3
elseif ngaunhiennl == 12 then
elseif ngaunhiennl == 13 then
elseif ngaunhiennl == 14 then
elseif ngaunhiennl == 15 then
else
end
end
----------------------------------------------------------------------------------------------------------------------------------------------------------------
		Task:OnKillNpc(me,him);
	------------------------------Nhiệm Vụ Ngày Bản Pro TCQGaming
local nTrangThaiNVNgay = me.GetTask(NVngayUI.TaskNVngay_G, NVngayUI.TaskNVngay_CauHinh)
if nTrangThaiNVNgay == 1 then
local nDemSoQuai = me.GetTask(NVngayUI.TaskNVngay_G, NVngayUI.DemSoQuaiGietDuoc)
local tbMember = me.GetTeamMemberList()
local nTenQuaiVat = me.GetTask(5014, 9)
if nTenQuaiVat == 0 then
    return  -- Dừng thực thi code và thoát khỏi hàm hoặc phương thức hiện tại
end
local nNpcID = NVngayUI.DATA_NPC[nTenQuaiVat][2]
    local nDemSoQuaiGiet = me.GetTask(NVngayUI.TaskNVngay_G, NVngayUI.DemSoQuaiGietDuoc)
    local szMsg = "\n"
    szMsg = szMsg .. " + Tiêu Diệt : <color=green>" .. NVngayUI.DATA_NPC[nTenQuaiVat][4] .. " Quái<color>\n + Tên Quái : <color=green>" .. NVngayUI.DATA_NPC[nTenQuaiVat][1] .. "<color>\n + Bản đồ :    <color=green>" .. NVngayUI.DATA_NPC[nTenQuaiVat][3] .. "<color>\n\n"
    szMsg = szMsg .. "Phần Thưởng :\n + <color=yellow>" .. NVngayUI.DATA_NPC[nTenQuaiVat][5] .. " Kinh Nghiệm<color>\n + <color=yellow>5 Vạn Đồng Khóa<color>\n + <color=yellow>" .. NVngayUI.DATA_NPC[nTenQuaiVat][7] .. "<color>\n\n"

    local nPartyMemberCount = 0 -- Biến đếm số thành viên trong tổ đội
    if tbMember then
        for _, pMember in ipairs(tbMember) do
            if pMember.nId ~= me.nId then
                nPartyMemberCount = nPartyMemberCount + 1
            end
        end
    end
    if nPartyMemberCount == 0 then
	 local nSoLanHoanThanhCaNhan = me.GetTask(5014, 4)
		if nSoLanHoanThanhCaNhan < 3 then
		if nNpcID == him.nTemplateId then
        nDemSoQuaiGiet = nDemSoQuaiGiet + 1 -- Tăng số quái giết của người chơi hiện tại lên 1
        local szMsg_show = szMsg .. "Tiến Trình Thực Hiện :\n + Đã Diệt <color=green>" .. nDemSoQuaiGiet .. "/" .. NVngayUI.DATA_NPC[nTenQuaiVat][4] .. "<color>"
        if nDemSoQuaiGiet >= 500 then
            Dialog:SendBlackBoardMsg(me, "Bạn đã hoàn thành nhiệm vụ, mau về lĩnh thưởng")
        end
        Dialog:SendBattleMsg(me, szMsg_show, 1)
        Dialog:ShowBattleMsg(me, 1, 0)
        me.SetTask(NVngayUI.TaskNVngay_G, NVngayUI.DemSoQuaiGietDuoc, nDemSoQuaiGiet) -- Cập nhật lại số quái giết của người chơi hiện tại	 
		else 
		me.Msg("<color=red>Nhiệm vụ lần này là tiêu diệt<color> : <color=yellow>"..NVngayUI.DATA_NPC[nTenQuaiVat][1].."<color> <color=cyan>hãy đánh đúng loại quái nhiệm vụ yêu cầu")
		end
		end 
	else
    if nNpcID == him.nTemplateId then
        ...
        Dialog:SendBattleMsg(me, szMsg_show, 1)
        Dialog:ShowBattleMsg(me, 1, 0)
        -- Cộng số quái giết cho mỗi thành viên trong tổ đội
        for _, pMember in ipairs(tbMember) do
            if pMember.nId ~= me.nId then
			 local nPartyMemberSoNhiemVuHoanThanh = pMember.GetTask(5014, 4)
			 local nTenQuaiVatCuaTungNguoi = pMember.GetTask(5014, 9)
			 if nTenQuaiVatCuaTungNguoi > 0 then 
			 local nNpcIDcuaTungNguoi = NVngayUI.DATA_NPC[nTenQuaiVatCuaTungNguoi][2]
                local nPartyMemberDemSoQuaiGiet = pMember.GetTask(NVngayUI.TaskNVngay_G, NVngayUI.DemSoQuaiGietDuoc)
				 if nPartyMemberSoNhiemVuHoanThanh < 3 and nNpcIDcuaTungNguoi == him.nTemplateId then
                pMember.SetTask(NVngayUI.TaskNVngay_G, NVngayUI.DemSoQuaiGietDuoc, nPartyMemberDemSoQuaiGiet + 1) -- Cộng 1 cho mỗi thành viên trong tổ đội
                Dialog:SendBattleMsg(pMember, szMsg .. "Tiến Trình Thực Hiện :\n + Đã Diệt <color=green>" .. (nPartyMemberDemSoQuaiGiet + 1) .. "/" .. NVngayUI.DATA_NPC[nTenQuaiVat][4] .. "<color>", 1)
				end 
            end
			end 
        end
...
function Player:SetForbidChat(szORpPlayer)
	...
	-- pPlayer.SetTask(Player.TASK_CAMCHAT, Player.TASK_CAMCHAT_LAST ,nSec3 + Player.Relay_Time15);
	-- pPlayer.Msg("Bạn Đang Bị Cấm Chat trong 15 phút ,Vui Lòng Liên Hệ GM để được giải quyết");
	return 1;
end
