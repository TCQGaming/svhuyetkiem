...
tbOffline.LEVEL_LIMIT_EXP = 70; -- giới hạn cấp nhận exp
...
if (MODULE_GAMESERVER) then
	tbOffline.tbLevelInfo = {
		...
		{
			nLevel	= 109,
			nTimeTskId	= DBTASD_SERVER_SETMAXLEVEL109,
		},
				{
			nLevel	= 119,
			nTimeTskId	= DBTASD_SERVER_SETMAXLEVEL119,
		},
		{
			nLevel	= 129,
			nTimeTskId	= DBTASD_SERVER_SETMAXLEVEL129,
		},
		{
			nLevel	= 150,
...
function tbOffline:AddExp(pPlayer, nAddExp1, nAddExp2)
	...
	for i = 1, nAddExp2 do
	if pPlayer.nLevel < self.LEVEL_LIMIT_EXP then
...
function tbOffline:CoZoneAddOfflineTime()
	...
	--	print(nGbCoZoneTime);
		if nSelfCoZoneTime ~= nGbCoZoneTime then
			...
			me.SetTask(self.TSKGID, self.TSKID_COZONE_GIVEOFFLINETIME_FLAG, nGbCoZoneTime);
...
function tbOffline:ProcessOfflineTime()
	...
	-- ******合服优惠*******
	nRestTime = nRestTime + self:CoZoneAddOfflineTime();
	...
	KStatLog.ModifyAdd("roleinfo", me.szName, "总托管时间", nOffLiveTime);
	
	szMsg	= string.format("           Ủy thác rời mạng \n" .. 
		"Ngày offline: %s\n" ..
		"Ngày online: %s\n" .. 
		"Thời gian offline: %s\n" ..
		"Thêm thời gian: %s\n" ..
		"Nhận kinh nghiệm: <color=Yellow>%.f<color> điểm\n" ..
		"Lúc đầu: %s\n" ..
		"Đạt được: %s\n" ..
		"Loại   Thời gian dùng   Còn   Nhận được\n",
		self:GetTimeDesc(nLastTime), self:GetTimeDesc(nNowTime), self:GetDTimeDesc(nOffTime), self:GetDTimeDesc(nOffLiveTime),
...
function tbOffline:GetLevelDesc(nLevel, nExp)
	return string.format("<color=Green>Cấp %d %.1f%% kinh nghiệm<color>", nLevel, nExp * 100 / self.tbLevelData[nLevel].nUpExp);
end
