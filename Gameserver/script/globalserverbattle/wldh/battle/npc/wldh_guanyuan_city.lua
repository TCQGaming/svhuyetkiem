...
if nDong == 1 then 
	...
	local szMsg = "Thật đáng tiếc! Bạn chưa được phép tham gia cuộc thi võ thuật và không thể đến địa điểm";
	local tbOpt = {
		--{"<color=green>Mua nhẫn thi đấu Liên Đấu<color>", self.OpenShop, self},
		{"Hỏi về ràng buộc bạc đặc biệt dành cho thi đấu Liên Đấu", self.AskForCurrencyMoney, self},
		};
	...
	if (Wldh:CheckIsOpen() == 1) then
		...
		if Wldh.Qualification:CheckMember(me) > 0 then
			...
			szMsg = "Chúc mừng bạn đã đủ điều kiện tham gia cuộc thi võ thuật!";
			table.insert(tbOpt, 1, {"Đến địa điểm thi đấu võ thuật", self.TransToServer, self});
			if nCurDate >= Wldh.STATE3_DATE[1] and nCurDate <= Wldh.STATE3_DATE[2] then
				table.insert(tbOpt, 2, {"Nhận phần thưởng cho trận chung kết cuộc thi nhỏ",self.GetMiniFinalAward, self});
			end
			if nCurDate >= Wldh.STATE5_DATE[1] and nCurDate <= Wldh.STATE5_DATE[2] then
				table.insert(tbOpt, 2, {"Nhận phần thưởng cuối cùng của cuộc thi đồng đội",self.GetTeamFinalAward, self});
			end
			if nCurDate >= Wldh.STATE4_DATE[1] and nCurDate <= Wldh.STATE4_DATE[2] then
				table.insert(tbOpt, 2, {"Thay thế phần thưởng thi đấu đồng đội",self.GetTeamExtraAward, self});	
			end
		end
		
		if nCurDate >= Wldh.STATE4_DATE[1] and nCurDate <= Wldh.STATE4_DATE[2] then
			table.insert(tbOpt, 2, {"Nhận phần thưởng máy chủ khu vực nhóm", self.GetServerAward, self});
		end
	end
...
function tbNpc:GetMiniFinalAward()
	local szMsg = "Tôi có thể thu thập phần thưởng khi giành chiến thắng trong trận chung kết cuộc thi nhỏ ở đây。";
	local tbOpt = {
		{"Trận đấu một người chơi", self.OnGetMiniFinalAward, self, 1},
		{"Song Đấu", self.OnGetMiniFinalAward, self, 2},
		{"Tam Đấu", self.OnGetMiniFinalAward, self, 3},
		{"Ngũ Đấu", self.OnGetMiniFinalAward, self, 4},
		{"Ta đã hiểu rồi"},
	};
	Dialog:Say(szMsg, tbOpt);
end
...
function tbNpc:OnGetMiniFinalAward(nType, nSure)
	...
	if nGetCount > 0 then
		Dialog:Say("Bạn đã nhận được phần thưởng cuối cùng thuộc loại này");
		return 0;
	end
	local tbAward = Wldh:GetFinalAwardTable(nType, nWinCount, nAttendTotle);
	if not tbAward then
		Dialog:Say("Không có phần thưởng cho loại cuộc thi này");
		return 0;
	end
	if not nSure then
	...
	if tbAward.tbAward and tbAward.tbAward.nNeedBag and me.CountFreeBagCell() < tbAward.tbAward.nNeedBag then
		Dialog:Say(string.format("Hành Trang không đủ chỗ trống，需要%s格背包空间。", tbAward.tbAward.nNeedBag));
		return 0;
	end	
	...
	local szFriend = string.format("您的好友[<color=green>%s<color>]在武林大会%s中获得了<color=red>第%s名<color>的好成绩。", me.szName, Wldh.LADDER_ID[nType][1], nWinCount);
...
function tbNpc:GetTeamFinalAward(nSure)
	...
	if tbAward.nNeedBag and me.CountFreeBagCell() < tbAward.nNeedBag then
		Dialog:Say(string.format("Hành Trang không đủ chỗ trống，需要%s格背包空间。", tbAward.nNeedBag));
		return 0;
	end	
