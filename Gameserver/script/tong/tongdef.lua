if not Tong then --调试需要
	Tong = {}
	print(GetLocalDate("%Y\\%m\\%d  %H:%M:%S").." build ok ..")
end

local function BitCombine(...)
	local nRet = 0
	for i,v in ipairs(arg) do
		nRet = nRet + math.pow(2, v - 1)
	end
	return nRet
end
local preEnv = _G	--保存旧的环境
setfenv(1, Tong)	--设置当前环境为Tong

CREATE_TONG_MONEY = 10000000	--1000 Vạn

TEST_PASS_BUILD_FUND = 10000000 --Kinh phí xây dựng cần thiết để vượt qua thời gian thử việc

CHANGE_CAMP = 200000 -- Cần kinh phí xây dựng để sửa đổi trại
CHANGE_CAMP_TIME = 2 * 3600 -- khoảng thời gian để sửa đổi trại
INHERIT_MASTER = 50 -- uy tín cá nhân cần có để giao nộp chủ bang hội
BUILD_TO_REPUTE = 10000 -- Tỷ lệ quy đổi giữa kinh phí xây dựng và uy tín Giang Hồ tạm thời quy định là 10000:1 (nếu không đủ thì làm tròn số)
DISPENSE_FUND_APPLY = 1000000 -- Ngưỡng ứng dụng để phân phối tiền là 100W
DISPENSE_FUND_RECORD = 10000 -- Số tiền ban đầu được phân phối trên đầu người
TAKEFUND_APPLY = 1000000 -- Ngưỡng áp dụng để rút tiền là 100W
STORAGE_FUND_TO_KIN_APPLY = 1000000 -- Giới hạn cho ứng dụng gia đình chuyển nhượng là 100w
DISPENSE_OFFER_APPLY = 1000 -- 1000 điểm cần thiết để đăng ký phân bổ đóng góp
DISPENSE_TIME = 6 * 3600 -- Khoảng thời gian giữa hai lần phân phối tiền cần 6 giờ
DISPENSE_AGREE_COUNT = 1 -- Số lượng sự đồng ý cần thiết để đăng ký phát hành là 1
DISPENSE_APPLY_LAST = 10 * 60 * 18 -- Ứng dụng đã phát hành bị trễ 10 phút
TAKEFUND_TIME = 6 * 3600 -- Khoảng thời gian giữa hai lần rút tiền cần 6 giờ
STORAGEFUND_TO_KIN_TIME = 6 * 3600 -- Thời gian chuyển quỹ bang hội cho gia đình hai lần
TAKEFUND_APPLY_LAST = 10 * 60 * 18 -- Chậm 10 phút khi đăng ký rút tiền
TAKEFUND_AGREE_COUNT = 1 -- Số lần đồng ý cần thiết để đăng ký rút tiền là 1
FIREKIN_APPLY_LAST = 10 * 60 * 18 -- Chậm 10 phút để đăng ký trục xuất một gia đình
KICK_KIN_AGREE_COUNT = 1 -- Số lượng đồng ý cần thiết để đá một bang hội
TONG_TEST_TIME = 14 * 24 * 3600 -- thời gian thử nghiệm bang hội
DEFAULT_STOCKPRICE = 100 -- giá cổ phiếu mặc định
DIRECTORATE_MEMBERS = 10 -- số thành viên hội đồng quản trị
MIN_BUILDFUND = 1000000 -- giá trị tối thiểu của quỹ xây dựng bang hội (dưới 100W, không thể chia cổ tức)
MIN_CAN_COST = 10000 -- số tiền tối thiểu có thể chi tiêu
QUIT_REDUCE_STOCK = 1 -- Rời khỏi bang hội với mất tiền rút tiền
JOIN_TONG_STOCK = 0,8 -- tỷ lệ tiền trên cổ phiếu khi tham gia bang hội
CHANGE_TAKESTOCK_ENERGY = 100 -- Đặt sức mạnh hành động được tiêu thụ bởi tỷ lệ cổ tức
MAX_ENERGY = 2400 -- Năng lượng tối đa cho năng lượng
SPILT_STOCK_MIN_PRICE = 1000 -- Giá cổ phiếu tối thiểu để tách cổ phiếu là 1000

ANNOUNCE_MAX_LEN = 1000 -- độ dài byte tối đa của thông báo

TONG_TASK_GROUP = 2081 -- Nhóm biến nhiệm vụ cho bang hội
TONG_TAKE_STOCK_WEEKS = 1 -- số tuần để nhận tiền thưởng

PRESDIENT_CONFIRM_WDATA = 1 -- Số tuần để lựa chọn người lãnh đạo và duy trì xếp hạng
TONG_LEVE_UNION_LAST = 86400 -- Khoảng thời gian giữa rời khỏi và tham gia

MAX_KIN_NUM = 5 --tổng gia tộc 

MAX_STORED_OFFER = 2000000000 -- giới hạn đóng góp dự trữ là 20E
MAX_BUILD_FUND = 4000000000 -- quỹ xây dựng được giới hạn ở mức 40E
MAX_TONG_FUND = 4000000000 -- Quỹ Bang hội 40E

SMCT_UI_TONG_REQUEST_LIST  = 27			-- 见gamedatadef.h,SYS_MESSAGE_CONFIRM_TYPE
CAPTAIN_NORMAL = 0	--普通长老，不能分配任何权限
CAPTAIN_MASTER = 1	--帮主
CAPTAIN_VICEMASTER = 2	--副帮主
CAPTAIN_ASSISTANT2 = 3	--战争长老1
CAPTAIN_ASSISTANT3 = 4	--战争长老2
CAPTAIN_ASSISTANT4 = 5	--内政长老
CAPTAIN_ASSISTANT5 = 6	--外交长老
CAPTAIN_ASSISTANT6 = 7	--执事长老
CAPTAIN_CUSTOM_BEGIN = 7 --自定义长老启始

POW_TITLE = 1 			--变更头衔
POW_RECRUIT = 2 		--招收家族
POW_ANNOUNCE = 3 		--公告编辑
POW_CAMP = 4				--阵营
POW_WORKSHOP = 5 		--作坊
POW_ENVOY = 6				--任免掌令使
POW_WAGE = 7				--调整分红
POW_FUN = 8 				--操作帮会资金
POW_GATHER = 9			--发起聚集
POW_STOREDOFFER = 10 	--储备贡献度
POW_UNION	= 11			--帮会联盟
POW_SPLIT	= 12			--拆分帮会
POW_WAR	= 13				--宣战
POW_COMMAND	= 14		--战指挥
POW_BANTALK	= 15		--禁言
POW_ACTIVITY = 16 	--发起帮会活动

POW_MASTER = 32	-- 定义最高位为帮主权限（这个权限不能给予，部分操作只有帮主有权限）
POWCB_ALL = 0xffffffff	--所有权限(帮主)

POWCB_VICEMASTER = 0x0fffffff	--副帮主默认权限.
POWCB_WAR = BitCombine(POW_ANNOUNCE,POW_CAMP,POW_ENVOY,POW_FUN,POW_STOREDOFFER,POW_COMMAND,POW_BANTALK)
POWCB_INTERIOR = BitCombine(POW_ANNOUNCE,POW_TITLE,POW_RECRUIT,POW_WORKSHOP,POW_ENVOY,POW_WAGE,POW_SPLIT,POW_BANTALK)
POWCB_DIPLOMAT = BitCombine(POW_ANNOUNCE,POW_TITLE,POW_CAMP,POW_ENVOY,POW_UNION,POW_WAR,POW_BANTALK)
POWCB_SECRETARY = BitCombine(POW_ANNOUNCE,POW_TITLE,POW_ENVOY,POW_GATHER,POW_FUN,POW_STOREDOFFER,POW_BANTALK,POW_ACTIVITY)
POWCB_CUSTOM = BitCombine(POW_ANNOUNCE,POW_TITLE,POW_ENVOY,POW_BANTALK)

CAMP_JUSTICE 	= 1		-- 宋阵营
CAMP_EVIL		= 2		-- 金阵营
CAMP_NEUTRALITY	= 3		-- 中立阵营

NONE_STOCK_RIGHT	= 0	-- 普通股民
PRESIDENT 			= 1 -- 首领
DIRECTORATE			= 2 -- 股东会成员
PRESIDENT_CANDIDATE	= 3 -- 首领侯选人

SORT_POSITION	= 0		--按职位排序
SORT_REPUTE		= 1		--按威望排序
SORT_VOTED		= 2		--按票数排序
SORT_STOCK		= 3		--按股权排序
SORT_GREAT_MEMBER_VOTED = 4; --按优秀票数排序

NO_PAGE_TONG_NONE		= -1;
NO_PAGE_TONG_INFO		= 0;
NO_PAGE_TONG_NOTE		= 1;
NO_PAGE_TONG_POSITION	= 2;

TONG_NOTE_PAGE_NONE		= -1;
TONG_NOTE_PAGE_AFFICHE	= 0;
TONG_NOTE_PAGE_HISTORY	= 2;
TONG_NOTE_PAGE_EVENT	= 1;


TONG_FIGURE_MASTER 			= 1;		-- 帮主
TONG_FIGURE_CAPTAIN_END 	= 20;		-- 长老
TONG_FIGURE_ENVOY_END 		= 34;		-- 掌令使
TONG_FIGURE_EXCELLENCT		= 500;		-- 精英
TONG_FIGURE_NORMAL 			= 3000;		-- 普通帮众
TONG_FIGURE_SIGNED 			= 4000;		-- 记名帮众
TONG_FIGURE_RETIRE 			= 5000;		-- 荣誉帮众

REQUEST_DISPENSE_FUND		= 1;		-- 申请发放人均100万以上的资金
REQUEST_TAKE_FUND			= 2;		-- 申请取出100万以上的帮会资金
REQUEST_DISPENSE_OFFER		= 3;		-- 申请发放人均1000点以上的储备贡献度
REQUEST_KICK_KIN			= 4;		-- 发起开除家族
REQUEST_JOIN				= 5;		-- 入帮申请
REQUEST_STORAGE_FUND_TO_KIN = 6;		-- 帮会资金转存家族(与申请帮会资金是互斥的)

tbCrowdTitle = {"Trưởng lão", "Chưởng lệnh", "Tinh anh", "Thành viên chính thức", "Thành viên ký danh"};
TITLE_MENU 		= {"  Danh hiệu nam", "  Danh hiệu nữ", " Danh hiệu ẩn sĩ", " Danh hiệu tinh anh"};

CAMP =
{
	[CAMP_JUSTICE]		= "Tống",
	[CAMP_EVIL]			= "Kim",
	[CAMP_NEUTRALITY]	= "Trung lập",
};


DISPENSE_FUND				= 1			-- 发放资金
DISPENSE_OFFER				= 2			-- 发放贡献度

WS_LIANGONG = 1
WS_TIANGONG = 2
WS_BINGJIA = 3
WS_MIANJU = 4
WS_SHILIAN = 5
WS_MIJI = 6
WS_RENWU = 7
WS_LIWU = 8
WS_CIFU = 9

if not WAGE_LOCKSTATE then
	WAGE_LOCKSTATE = {} --状态锁
end
WAGE_LEVEL = 90;
WAGE_HIGHFIGURE = 2;	--长老，精英等级
WAGE_LOWFIGURE 	= 1;	--正式成员
WAGE_NOFIGURE 	= 0;	--无资格

TASK_LEVEL_LOW = 50;
TASK_LEVEL_MID = 80;
TASK_LEVEL_HIGH = 90;

OFFICIAL_LEVEL_CONDITION = {1, 3, 6, 10, 18, 42};

MAX_TONG_OFFICIAL_NUM = 10;

OFFICIAL_RANK_NAME = {" Thủ lĩnh", "Chức quan cấp 1", "Chức quan cấp 2", "Chức quan cấp 3", "Chức quan cấp 4", "Chức quan cấp 5", "Chức quan cấp 6", "Chức quan cấp 7",  "Chức quan cấp 8",  "Chức quan cấp 9"  };

TONG_OFFICIAL_LEVEL_CHARGE	= {100000, 100000, 100000, 100000, 100000, 100000};

OFFICIAL_TITLE_GENRE = 10;

OFFICIAL_TITLE_DETAIL = 1;

OFFICIAL_TABLE = 
{
	{3, 1, 1, 1, 0, 0, 0, 0, 0, 0},
	{4, 2, 2, 1, 1, 1, 0, 0, 0, 0},
	{5, 3, 3, 2, 2, 1, 1, 1, 1, 0},
	{6, 4, 3, 3, 2, 2, 2, 2, 2, 0},
	{7, 5, 4, 4, 3, 3, 2, 2, 2, 0},
	{8, 6, 5, 5, 4, 4, 4, 3, 3, 3},
};

GREAT_MEMBER_COUNT = 5;
GREAT_MEMBER_VOTE_START_DAY = 5;
GREAT_MEMBER_VOTE_END_DAY = 0;

TONGANNOUNCE_MAX_TIMES = 20;
TONGANNOUNCE_MIN_TIMES = 1;
TONGANNOUNCE_MAX_DISTANCE = 300;
TONGANNOUNCE_MIN_DISTANCE = 10;

preEnv.setfenv(1, preEnv)	--恢复全局环境
