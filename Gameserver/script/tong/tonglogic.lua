...
function Tong:CreateTong(anKinId, szTongName, nCamp, nCreateTime)
	...
	--将列表的Kin加入帮会中
	for i, nKinId in ipairs(anKinId) do
		nTotalRepute = nTotalRepute +  self:_AddKin2Tong(nTongId, cTong, nKinId, nCreateTime, i)
	end
	...
	for i = 1,8 do
		cTong.SetCaptainTitle(self.CAPTAIN_CUSTOM_BEGIN + i, "Trưởng lão thường vụ")
...
function Tong:_AddKin2Tong(nTongId, cTong, nKinId, nCreateTime, i)
	local cKin = KKin.GetKin(nKinId)
	if not cKin then
		return 0;
	end
	...
	while pMember do
		...
		local nNewStock = math.floor(nPersonalStock * self.JOIN_TONG_STOCK / nStockPrice);		-- 成员新股份数
		nToAddFund = nToAddFund + nPersonalStock;						-- 总资金叠加
		nTotalStock = nTotalStock + nNewStock;							-- 股份数叠加
		pMember.SetPersonalStock(nNewStock);
		pMember = pMemberItor.NextMember();
	end
	cTong.AddBuildFund(nToAddFund);		-- 增加资金
	cTong.SetTotalStock(nTotalStock);	-- 增加股份
	return nAddRepute, nToAddFund;
end
...
-- 为帮会的下属家族分配周任务
function Tong:SetMemberKinTask(cTong, nLowTask, nMidTask, nHighTask)
	if (not cTong) then
		return 0;
	end
	local pMemberIt = cTong.GetKinItor();
	local nKinId = pMemberIt.GetCurKinId();
	if (0 == nKinId) then
		return 0;
	end
	local pMember = KKin.GetKin(nKinId);
	while pMember do
		if (MODULE_GC_SERVER) then
			Kin:StatisticsWeeklyTaskLevel(pMember);
		end
		
		local nTempTask = pMember.GetWeeklyTask();	-- 记录上周的周任务目标
		pMember.SetLastWeeklyTask(nTempTask);
		local nOffer = pMember.GetWeeklyKinOffer();	-- 记录上周的贡献度，把本周的贡献度清零
		pMember.SetLastWeekKinOffer(nOffer);
		pMember.SetWeeklyKinOffer(0);
		pMember.SetIsCaptainGetAward(0);
		
		local nTaskLevel = pMember.GetTaskLevel();	-- 记录上周的周任务目标等级
		if (0 == nTaskLevel) then
			nTaskLevel = self.TASK_LEVEL_LOW;
			pMember.SetTaskLevel(nTaskLevel);	-- 默认等级是初级，50
		end
		pMember.SetLastTaskLevel(nTaskLevel);
		if (pMember.GetNewTaskLevel() ~= 0) then	-- 如果有新的等级修改，把新的等级修改生效
			local nNewTaskLevel = pMember.GetNewTaskLevel();
			pMember.SetTaskLevel(nNewTaskLevel);
			pMember.SetNewTaskLevel(0);
		end
		nTaskLevel = pMember.GetTaskLevel(); -- 获取当前周有效的目标等级，并为该家族分配相应等级的目标任务
		if (self.TASK_LEVEL_LOW == nTaskLevel) then
			pMember.SetWeeklyTask(nLowTask);
		elseif (self.TASK_LEVEL_MID == nTaskLevel) then
			pMember.SetWeeklyTask(nMidTask);
		elseif (self.TASK_LEVEL_HIGH == nTaskLevel) then
			pMember.SetWeeklyTask(nHighTask);
		else
			local szErrLog = string.format("Gia tộc: %s, Bang hội: %s, Cấp mục tiêu tuần: %s, Do cấp mục tiêu nên chưa thể thiết lập.",
				pMember.GetName(), cTong.GetName(), nTaskLevel);
			if (MODULE_GC_SERVER) then
				szErrLog = szErrLog .. "MODULE_GC_SERVER";
			else
				szErrLog = szErrLog .. "MODULE_GAMESERVER";
			end
			Dbg:WriteLog("Mục tiêu tuần gia tộc", szErrLog);
			return 0;
		end
		Kin:PerKinMemberWeeklyTask(pMember);		
		nKinId = pMemberIt.NextKinId();
		pMember = KKin.GetKin(nKinId);
	end	
end

function Tong:PerKinMemberWeeklyTask(cKin)
	if (not cKin) then
		return 0;
	end
		...
		if pKin then
			...
			if i == 1 then 	-- 第一个是首领
				if bSetPosition ~= 0 then
					...
					-- Add TongLog 成为首领
					local szLogMsg = string.format("[%s] trở thành thủ lĩnh",  KGCPlayer.GetPlayerName(pMember.GetPlayerId()));
