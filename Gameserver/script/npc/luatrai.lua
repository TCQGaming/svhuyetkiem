...
local tbGouhuoNpc = Npc:GetClass("luatrai");

tbGouhuoNpc.nNpcId				= 20023;		-- 篝火npc的Id
...
function tbGouhuoNpc:InitGouHuo(nNpcId, nType,	nRestTime, nPeriodTime, nAddExpDis, nBaseMultip, nCanUseJIu, nCanUseXiuLianZhu)
...
function tbGouhuoNpc:AddAroundExp_Team(nNpcId)
	...
	if tbTmp.nCanUseJIu ~= 0 then
		...
		if tbTmp.nAnnouce == 1 and szMsg then
			KTeam.Msg2Team(tbTmp.nTeamId, szMsg);
			if szMsg2 then
				KTeam.Msg2Team(tbTmp.nTeamId, szMsg2);
			end
		end
	end
						...
						for _, nId in pairs(tbPlayerId2) do
							if nId ~= nPlayerId then
								...
								if pMemPlayer and pPlayer.IsFriendRelation(pMemPlayer.szName) == 1 then
									...
									pPlayer.Msg(string.format("Bạn và <color=yellow>%s<color> Tăng độ thân mật hảo hữu lên  % điểm.", pMemPlayer.szName, 2));
									pMemPlayer.Msg(string.format("Bạn và <color=yellow>%s<color> Tăng độ thân mật hảo hữu lên  % điểm.", pPlayer.szName, 2));
								end
							end
						end
...
function tbGouhuoNpc:AddExp2Player(nPlayerId, nNpcId, nAnnouce)
	...
	if tbTmp.nCanUseXiuLianZhu == 1 then
		...
		pPlayer.AddBindMoney(500); -- bạc khóa
	else
		pPlayer.AddExp(nExp);
		pPlayer.AddBindCoin(500);-- đồng khóa
		pPlayer.AddBindMoney(500);-- bạc khóa
...
function tbGouhuoNpc:CreateAnnouce(nNpcId, tbPlayerName, szEventJiuName)
	...
	if nAnnouce == 1 then
		if nJiuMax == 0 then
			...
			szMsg = string.format("%s tăng kinh nghiệm lửa trại <color=yellow>%s<color=blue>.", szMsg, nQuotiety)
		else
			if szEventJiuName then
				szMsg = string.format("Đồng đội đang chia sẻ %s %s，kinh nghiệm tăng %s.", szJiuName, szEventJiuName, nQuotiety)
			else	
				szMsg = string.format("Đồng đội đang chia sẻ %s, kinh nghiệm tăng %s.", szJiuName, nQuotiety)
			end
