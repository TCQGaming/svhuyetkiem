...
Npc.HONOR_CLASS_MONEY		= 8;	
Npc.tbHonorLevelInfo	= {
	["money"]	= {
		nHonorId	= Npc.HONOR_CLASS_MONEY,
	},
};
function Npc:OnDialog(szClassName, szParam)
	Player:FixCamSuDungDoPet(me);
	Player:FixDeoDoThuongLenDoPet(me);--
	Player:FixDeoDoPetLenDoThuong(me);--
--------------------------------------------------------------------------------
	local nNgay = tonumber(GetLocalDate("%w"));
	local nThoiGian = tonumber(os.date("%H%M", GetTime()));
	local nCheck = 0;
	local tbAroundNpc    = KNpc.GetAroundNpcList(me, 30);-- khoảng cách từ nhân vật đến NPC
	for _, pNpc in ipairs(tbAroundNpc) do
			if (pNpc.nTemplateId == 2965) and nNgay == 0 and (nThoiGian >= 2300 and nThoiGian <= 2359) then
				nCheck = 1;
				Dialog:Say("Chủ nhật từ 23h00 - 23h59 Hiện không thể nhận nhiệm vụ, hãy quay lại sau 00h nhé ! ");
				break;
			end
			if (pNpc.nTemplateId == 2965) then
				-----giới hạn tài phú thương hội
		...
		if nHonorValue < 0 then
						nCheck = 1;
			Dialog:Say("Bạn không đạt đủ <color=yellow>0 <color>điểm Tài Phú để tham gia\nTài Phú Hiện Nay<color=yellow> "..nHonorValue.."<color>");
			return 0;
			end
		end
		end 
