...
Faction.SWITCH_FACTION_CD			= 1800;   -- 切换门派CD时间，单位：秒
Faction.SWITCH_FACTION_BUF_ID		= 897;
Faction.MAX_MODIFY_TIME				= 3;	-- 最大换辅修次数
...
function Faction:__RecordCurrFaction(pPlayer)
	...
	-- 记录路线
	pPlayer.SetTask(nFactionGroupId, self.TSKID_FACTION_ROUTE, pPlayer.nRouteId);
...
function Faction:SwitchFaction(pPlayer, nFactionId)
	...
	if(self:IsForbidSwitchFaction(pPlayer) == 1)then
		return 0, "Không được phép chuyển đổi môn phái.";
	end
	...
	if(pPlayer.nTeamId > 0)then
		return 0, "Không thể chuyển đổi môn phái khi đang ở trong tổ đội.";
	end
	
	local nCurrTime = GetTime();
	
	if(pPlayer.nFightState == 1)then
		return 0, "Khi chuyển đổi môn phải phải ở trạng thái Phi Chiến Đấu.";
	end
	
	if( pPlayer.nMapId ~= 255 and  -- 洗髓岛无cd限制
		pPlayer.GetSkillState(self.SWITCH_FACTION_BUF_ID) > 0
	)then
		return 0, "Đổi phái cần có thời gian. Thời gian hiện tại vẫn chưa đến, bạn không thể chuyển đổi.";
	end
	...
	if (pPlayer.nMapId ~= 255) then
		szMsg = "Chuyển thành công sang %s, Thời gian chuyển đổi giữa các môn phái là 30 phút. Hãy chú ý điều này";
	else
		szMsg = "Chuyển thành công sang %s."
	end
...
function Faction:CheckGenreFaction(pPlayer, nFactionGenre, nFactionId)
	...
	if(nCurrFactionId == 0)then
		return 0, "Hãy gia nhập 1 môn phái";
	end
	
	if(self:__GetFactionUseIndex(pPlayer, nFactionId) > 0 or 
		nCurrFactionId == nFactionId)
	then
		return 0, "Bạn đã ở môn phái này, không cần chuyển đổi nữa.";
	end
	...
	if(nChangeFactionIndex >  self.MAX_USED_FACTION)then
		return 0, "Chuyển đổi giới tính chưa chắc đã tốt hơn (!cy)";
	end
