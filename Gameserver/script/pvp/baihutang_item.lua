...
function tbItem:OnUse()
	...
	if nLimitLevel > 0 and me.nLevel <= nLimitLevel then
		me.Msg(string.format("Bạn chưa đạt cấp %s!", nLimitLevel))
		return 0;
	end
	...
	if nTimes > 0 then
		...
		me.Msg(string.format("Số lần tham gia Bạch Hổ Đường tăng %s lần", nTimes));
	end
	return 1;
end
...
function tbItem:GetTip()	
	...
	if nTaskGroupId and nTaskGroupId ~= 0 then		
		if nTaskData and nTaskTimes and nTaskTimes_Max and nTaskData ~= 0 and nTaskTimes ~= 0 and nTaskTimes_Max ~= 0 then		
			...
			szMsg = szMsg..string.format("<color=%s>Số lần sử dụng còn: %s/%s<color>", szColor, nTimes, nTaskTimes_Max);
		end
	end	
	if nLimitLevel and nLimitLevel ~= 0 then		
		szMsg = szMsg..string.format("<color=red>\nNgười chơi trên cấp %s mới có thể dùng<color>", nLimitLevel);
	end	
	return szMsg;
end
