...
--	 状态 					定时时间			时间到回调函数(函数返回0表示不在继续定时，结束活动)
	{1, 	Env.GAME_FPS * 60 * 9, 		    "ShowGongGao"		},		--  显示公
	{2,		Env.GAME_FPS * 60 * 9,			"ShowGongGao"		},		--  显示公告
	{3,		Env.GAME_FPS * 60 * 5,			"ShowGongGao"		}, 		--  显示公告
--	{4,		Env.GAME_FPS * 60 * 9,			"CallBoss"			}, 		-- Call 第一层BOSS
--	{5,		Env.GAME_FPS * 60 * 9,			"CallBoss"			}, 		-- Call 第二层BOSS
--	{6,		Env.GAME_FPS * 60 * 5,			"CallBoss"			}, 		-- Call 第三层BOSS
	{BaiHuTang.END}
};
...
BaiHuTang.tbHotroInfo = {bForceBind=1,};
function BaiHuTang:OnPlayerTrap(nMapId)
	...
	--Thưởng Qua Trap
		ConfigAll.ThuongQuaAiBachHoDuong()
		me.SetTask(Cf.MuaLaiHD_nGROUP[1], Cf.MuaLaiHD_nTASK_ID,me.GetTask(Cf.MuaLaiHD_nGROUP[1], Cf.MuaLaiHD_nTASK_ID)+1);
		 self:NhiemVuNgay(pPlayer);
	
	local tbNhiemVu = SpecialEvent.NhiemVuHangNgay;
	local nCountNV  = me.GetTask(tbNhiemVu.TaskGroup, tbNhiemVu.TaskBHD);
	me.SetTask(tbNhiemVu.TaskGroup, tbNhiemVu.TaskBHD,nCountNV + 1);
...
function BaiHuTang:NhiemVuNgay(pPlayer)	
-- local tbMualaihoatdong = me.GetTask(3092,1) --DAY
	local NhiemVuTuanBHD = me.GetTask(9192,1) --DAY
		me.SetTask(9192,1,NhiemVuTuanBHD +1);
		-- me.SetTask(3092,1,tbMualaihoatdong +1);
