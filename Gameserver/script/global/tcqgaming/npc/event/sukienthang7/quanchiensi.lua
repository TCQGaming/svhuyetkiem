-----Dev : TCQGaming
local tbTapVo = Item:GetClass("quanchiensi_thang7");

function tbTapVo:OnUse()	
local ngaunhien = MathRandom(1,200)
-- local tbItemId	= {18,1,2028,24,0,0};  -- Gio hoa
local SoLuongSuDung = me.GetTask(9186,85)
if SoLuongSuDung >= 500 then 
me.Msg("Bạn đã sử dụng tối đa<color=yellow> 500 Quần Chiến Sĩ <color>rồi<color>");
return 0
end
    local nNeedBag = 30;
...
KDialog.MsgToGlobal("<color=yellow>Người chơi <color=cyan>[ "..me.szName.." ]<color> đã sử dụng tối đa 500 Quần Chiến Sĩ xin chúc mừng<color>");
end
if ngaunhien == 1 then
...
me.SetTask(9186,85, DemSoLuong)
me.Msg("Bạn đã sử dụng <color=gold>"..DemSoLuong.." Quần Chiến Sĩ<color>");
