local tb_LatThiep ={};
SpecialEvent.Lat_TheBai= tb_LatThiep;

local tbItemInfoArray = {}; -- Mảng để lưu thông tin của các vật phẩm
tb_LatThiep.HONOR_CLASS_MONEY		= 8;	
tb_LatThiep.tbHonorLevelInfo	= {
	["money"]	= {
		nHonorId	= tb_LatThiep.HONOR_CLASS_MONEY,
	},
};

function c2s:LatThiep_Sever()
tb_LatThiep:OnDialog()
end 
function tb_LatThiep:OnDialog()
local nDong = 1
if nDong == 0 then 
...
local nSoLan = me.GetTask(9178,80)
    ...
    local tbOpt = {
        {"<color=yellow>Lật Thiệp May Mắn<color>", self.latthiep, self},
