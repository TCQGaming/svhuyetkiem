...
-- by:TCQGaming
-------------------------------------------------------
local tbSuKien_NapTien_T8 ={};
SpecialEvent.SuKienT8= tbSuKien_NapTien_T8;


function tbSuKien_NapTien_T8:OnDialog()
	local szMsg = "<color=yellow>Sự Kiện Tháng 8<color>";
	local nLan = Config_Event_Bank.Thang8
	    local tbOpt =     {} 
		if nLan == 1 then 
					table.insert(tbOpt, {"<color=yellow>Tích Lũy Thẻ Nạp Sự Kiện Cách Mạng Tháng Tám 16/08-23/08<color>" , self.tichluy_thang8_lan1, self});
		elseif nLan == 2 then 
					table.insert(tbOpt, {"<color=yellow>Tích Lũy Thẻ Nạp Sự Kiện Cách Mạng Tháng Tám Lần 2 - 23/08-30/08<color>" , self.tichluy_thang8_lan2, self});
		elseif nLan == 3 then 
					table.insert(tbOpt, {"<color=yellow>Tích Lũy Thẻ Nạp Sự Kiện Cách Mạng Tháng TámLần 3 - 26/07-01/08<color>" , self.tichluy_thang7_lan3, self});
		elseif nLan == 4 then 
					table.insert(tbOpt, {"<color=yellow>Tích Lũy Thẻ Nạp Sự Kiện Cách Mạng Tháng TámLần 4 - 02/08-09/08<color>" , self.tichluy_thang7_lan4, self});
		elseif nLan == 5 then 
					table.insert(tbOpt, {"<color=yellow>Tích Lũy Thẻ Nạp Sự Kiện Cách Mạng Tháng TámLần 5 - 09/08-16/08<color>" , self.tichluy_thang7_lan5, self});
		end 
...
function tbSuKien_NapTien_T8:tichluy_thang8_lan1() 
	local nDongRut = me.GetTask(9186,145)
	local szMsg = "Tổng tích lũy nạp sự kiện từ ngày<color=gold>\n16/08-23/08<color> của bạn hiện tại là: <color=yellow>"..nDongRut.." vạn đồng<color>";
...
function tbSuKien_NapTien_T8:tichluy_thang8_lan2() 
...
function tbSuKien_NapTien_T8:CachMangThang8_100k_lan1()
	local nNeedBag = 5;
...
function tbSuKien_NapTien_T8:CachMangThang8_200k_lan1()
	...
	local nCount = me.GetTask(9186,147)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
...
function tbSuKien_NapTien_T8:CachMangThang8_300k_lan1()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
	...
	local nCount = me.GetTask(9186,148)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
...
function tbSuKien_NapTien_T8:CachMangThang8_400k_lan1()
	...
	local nCount = me.GetTask(9186,149)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
	...
	me.SetTask(9186,149, nCount + 1);
me.AddStackItem(18,1,114,7,{bForceBind=1},3)
me.AddBindMoney(50000);---5v Bạc Khóa
me.AddBindCoin(15*10000);	-- 30 đồng khóa
me.AddStackItem(18,1,553,1,{bForceBind=1},200)--200-Tiền Du Long
end
function tbSuKien_NapTien_T8:CachMangThang8_500k_lan1()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
	...
	local nCount = me.GetTask(9186,150)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
...
function tbSuKien_NapTien_T8:CachMangThang8_600k_lan1()
	...
	local nCount = me.GetTask(9186,151)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
...
function tbSuKien_NapTien_T8:CachMangThang8_700k_lan1()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
	...
	local nCount = me.GetTask(9186,152)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
...
function tbSuKien_NapTien_T8:CachMangThang8_1000k_lan1()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
	...
	local nCount = me.GetTask(9186,153)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
...
KDialog.MsgToGlobal("<color=green>Người chơi <color=yellow>[ "..me.szName.." ]<color> nhận mốc tích lũy nạp thẻ 1000k Sự Kiện Cách Mạng Tháng Tám Lần 1\nNhận được<color=gold>  :\n50 MG Hòa Thị Ngọc thô\n10 Lệnh bài đồ hải tặc cấp 1\n5 Vạn Bạc Khóa\n30 Vạn Đồng Khóa\n500 Tiền Du Long\n1 Tín Vật Minh Chủ\n1 Cặp VLMT TTK Trung");
end 

function tbSuKien_NapTien_T8:CachMangThang8_1500k_lan1()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
	...
	local nCount = me.GetTask(9186,154)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
...
KDialog.MsgToGlobal("<color=green>Người chơi <color=yellow>[ "..me.szName.." ]<color> nhận mốc tích lũy nạp thẻ 1500k Sự Kiện Cách Mạng Tháng Tám Lần 1\nNhận được<color=gold>  :\n100 MG Hòa Thị Ngọc thô\n25 thất sắc hồn\n10 Rương Ngọc Cấp 1\n1 Huyền Tinh 9\n5 Vạn Bạc Khóa\n30 Vạn Đồng Khóa\n200 Rương trang bị đồng hành\n500 Tiền Du Long");
end 

function tbSuKien_NapTien_T8:CachMangThang8_2000k_lan1()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
	...
	local nCount = me.GetTask(9186,155)
	if nCount >= 1 then
		Dialog:Say(string.format("Đã nhận quà, sao lại đến nữa"));
		return 0;
	end
...
KDialog.MsgToGlobal("<color=green>Người chơi <color=yellow>[ "..me.szName.." ]<color> nhận mốc tích lũy nạp thẻ 2000k Sự Kiện Cách Mạng Tháng Tám Lần 1\nNhận được<color=gold>  :\n500 Chân Nguyên Tu Luyện Đơn\n5 Lệnh bài triệu hồi Boss Tiêu Dao Cốc\n25 thất sắc hồn\n150 MG Hòa Thị Ngọc (thô)\n10 Rương Ngọc Cấp 1\n10 Lệnh bài vũ khí hải tặc\n1 lượt chuyền cường hóa\n1 Huyền Tinh 9\n5 Vạn Bạc Khóa\n30 Vạn Đồng Khóa\n200 Rương trang bị đồng hành\n500 Tiền Du Long");
end 


function tbSuKien_NapTien_T8:CachMangThang8_100k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
function tbSuKien_NapTien_T8:CachMangThang8_200k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
function tbSuKien_NapTien_T8:CachMangThang8_300k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
function tbSuKien_NapTien_T8:CachMangThang8_400k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
function tbSuKien_NapTien_T8:CachMangThang8_500k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
function tbSuKien_NapTien_T8:CachMangThang8_600k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
function tbSuKien_NapTien_T8:CachMangThang8_700k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
function tbSuKien_NapTien_T8:CachMangThang8_1000k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
me.AddStackItem(18,1,2030,8,{bForceBind=1},1)--Tín vật minh chủ
me.AddStackItem(18,1,1327,13,nil,50)-- mảnh ghép hòa thị ngọc thô
KDialog.MsgToGlobal("<color=green>Người chơi <color=yellow>[ "..me.szName.." ]<color> nhận mốc tích lũy nạp thẻ 1000k Sự Kiện Cách Mạng Tháng Tám Lần 2\nNhận được<color=gold>  :\n50 MG Hòa Thị Ngọc thô\n10 Lệnh bài đồ hải tặc cấp 1\n5 Vạn Bạc Khóa\n30 Vạn Đồng Khóa\n500 Tiền Du Long\n1 Tín Vật Minh Chủ\n1 Cặp VLMT TTK Trung");
end 

function tbSuKien_NapTien_T8:CachMangThang8_1500k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
me.AddStackItem(18,1,2026,1,nil,25)---Thất Sắc Hồn
me.AddStackItem(18,1,1327,13,nil,100)-- mảnh ghép hòa thị ngọc thô
KDialog.MsgToGlobal("<color=green>Người chơi <color=yellow>[ "..me.szName.." ]<color> nhận mốc tích lũy nạp thẻ 1500k Sự Kiện Cách Mạng Tháng Tám Lần 2\nNhận được<color=gold>  :\n100 MG Hòa Thị Ngọc thô\n25 thất sắc hồn\n10 Rương Ngọc Cấp 1\n1 Huyền Tinh 9\n5 Vạn Bạc Khóa\n30 Vạn Đồng Khóa\n200 Rương trang bị đồng hành\n500 Tiền Du Long");
end 

function tbSuKien_NapTien_T8:CachMangThang8_2000k_lan2()
	local nNeedBag = 5;
	if me.CountFreeBagCell() < nNeedBag then
    	local szAnnouce = "Sắp xếp hành trang còn trống "..nNeedBag.." ô";
    	Dialog:Say(szAnnouce);
...
me.AddStackItem(18,1,2026,1,nil,25)---Thất Sắc Hồn
 ...
 me.AddStackItem(18,1,1327,13,nil,150)-- mảnh ghép hòa thị ngọc thô
 	me.AddStackItem(18,1,402,1,nil,500)---Chân Nguyên Tu Luyện Đơn Exp Đồng Hành
KDialog.MsgToGlobal("<color=green>Người chơi <color=yellow>[ "..me.szName.." ]<color> nhận mốc tích lũy nạp thẻ 2000k Sự Kiện Cách Mạng Tháng Tám Lần 2\nNhận được<color=gold>  :\n500 Chân Nguyên Tu Luyện Đơn\n5 Lệnh bài triệu hồi Boss Tiêu Dao Cốc\n25 thất sắc hồn\n150 MG Hòa Thị Ngọc (thô)\n10 Rương Ngọc Cấp 1\n10 Lệnh bài vũ khí hải tặc\n1 lượt chuyền cường hóa\n1 Huyền Tinh 9\n5 Vạn Bạc Khóa\n30 Vạn Đồng Khóa\n200 Rương trang bị đồng hành\n500 Tiền Du Long");
end 
