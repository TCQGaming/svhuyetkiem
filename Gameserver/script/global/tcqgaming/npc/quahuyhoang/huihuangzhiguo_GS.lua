...
Require("\\script\\global\\tcqgaming\\npc\\quahuyhoang\\huihuangzhiguo_head.lua")
...
local tbConfig =
{
	[132] = {szMapName = "Tàn Tích Cung A Phòng",szPosFile = "\\setting\\event\\quahuyhoang\\tantich\\seedposfile.txt"},
	-- [122] = {szMapName = "Mạn Bắc Thảo Nguyên",szPosFile = "\\setting\\event\\quahuyhoang\\manbac\\seedposfile.txt"},
};
...
function HuiHuangZhiGuo:GreatSeedExecute(nMapId, nSeedLevel, nCount, nGrowPhase, nWetherMakeGoldFruit)
	...
	if (nWorldIdx < 0) then	--如果不在该组服务器上
		...
		KDialog.MsgToGlobal("<color=yellow>"..tbConfig[nMapId].szMapName.."<color> Xuất Hiện Hạt Giống Quả Huy Hoàng");	
		KDialog.MsgToGlobal("<color=yellow>"..tbConfig[nMapId].szMapName.."<color> Xuất Hiện Hạt Giống Hoàng Kim");	
		elseif nGrowPhase == 1 then 
		KDialog.MsgToGlobal("<color=yellow>"..tbConfig[nMapId].szMapName.."<color> Quả huy hoàng đã chín rồi");	
		KDialog.MsgToGlobal("<color=yellow>"..tbConfig[nMapId].szMapName.."<color> Quả Hoàng Kim đã chín rồi");	
		end 
...
function HuiHuangZhiGuo:AddMapSeeds(nWorldIdx, nMapId, nSeedLevel, nCount,  nWetherMakeGoldFruit, nGrowPhase)
	...
	if (nGrowPhase == 1) then --删除之前的npc
		ClearMapNpcWithName(nMapId, "Hạt Huy Hoàng");
		ClearMapNpcWithName(nMapId, "Hạt Hoàng Kim");
	else
	...
	for i = 1, nCount do
		...
		if (i ~= self.nGoldenSeedRow or nWetherMakeGoldFruit ~= 1) then --普通果子
			...
			self:AddSeed(nNpcGoldenTmpl,nWorldIdx,57088,113824,nSeedLevel);-----Thả hạt hoàng kim zô
		else--如果是黄金之种或黄金之果
			--需要公告下
			if (nGrowPhase == 0) then
...
function HuiHuangZhiGuo:ShowEventInfo(szItemName)
	...
	if (szItemName == "Quả Huy Hoàng") then
		szContent = "    <color=red>Mỗi Đêm Vào Lúc 22h00 Đến 22h30<color> trong thời gian này các map thông trấn xuất hiện Quả Huy Hoàng<color=red>5 ra quả<color>，và<color=red>3 phút kết trái<color>.<enter><enter><color=yellow> Xuất hiện tại：<enter>    Chiến Trường Cổ,Quán Trọ Long Môn,Quân Mã Trường,Mai Hoa Lĩnh,Trường Giang Hà Cốc,Thị trấn Bạch Tộc,Nhạn Đãng Long Thu,Bờ Hồ Động Đình<color><enter><enter><color=yellow> Hái Quả：<enter>    Trong thời gian hái quả không được cùng lúc hái quả khác nếu không quả đang hái sẽ bị giành mất.<color>";
