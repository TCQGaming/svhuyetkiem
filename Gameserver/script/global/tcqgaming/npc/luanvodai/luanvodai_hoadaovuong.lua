...
local tbQinshihuangBoss	= Npc:GetClass("hoadaovuong");
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "hoadaovuong");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "hoadaovuong");
			end
		end
	end
	...
	local szThongBaoTheGioi1 = string.format("Người chơi <color=green>%s<color> Kết Liễu Boss Luận Võ Đài<color><color=yellow> "..him.szName.."", pPlayer.szName);
