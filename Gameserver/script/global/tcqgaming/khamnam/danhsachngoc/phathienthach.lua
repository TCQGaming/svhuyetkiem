...
local tbPhaThienThach = Item:GetClass("phathienthach");

function tbPhaThienThach:OnUse()	
DoScript("\\script\\global\\tcqgaming\\khamnam\\danhsachngoc\\phathienthach.lua");
...
local nVuKhi_C = me.GetTask(9183,3)
	...
	local nTen = ""
		if (nLevel == 1) then
		nSkillCount = 1
				nTen = "Phá Thiên Thạch (vỡ) (cấp 1)"
		elseif (nLevel == 2) then
		nSkillCount = 2
				nTen = "Phá Thiên Thạch (mòn) (cấp 2)"
		elseif (nLevel == 3) then
		nSkillCount = 3
				nTen = "Phá Thiên Thạch (hoàn chỉnh) (cấp 3)"
		elseif (nLevel == 4) then
		nSkillCount = 4
				nTen = "Phá Thiên Thạch (tinh xảo) (cấp 4)"
		elseif (nLevel == 5) then
		nSkillCount = 5
				nTen = "Phá Thiên Thạch (lấp lánh) (cấp 5)"
		elseif (nLevel == 6) then
		nSkillCount = 6
				nTen = "Phá Thiên Thạch (óng ánh) (Cấp 6)"
		elseif (nLevel == 7) then
		nSkillCount = 7
				nTen = "Phá Thiên Thạch (lung linh) (Cấp 7)"
		elseif (nLevel == 8) then
		nSkillCount = 8
				nTen = "Phá Thiên Thạch (hoàn mỹ) (Cấp 8)"
		elseif (nLevel == 9) then
		nSkillCount = 9
				nTen = "Phá Thiên Thạch (chí tôn) (Cấp 9)"
		elseif (nLevel == 10) then
		nSkillCount = 10
				nTen = "Phá Thiên Thạch (vô song) (Cấp 10)"
		end 
	...
	local nCapBacKeTiep = nLevel + 1
		if (nCapBacKeTiep == 2) then
				nTenPC_BacTiep = "Phá Thiên Thạch (mòn) (cấp 2)"
		elseif (nCapBacKeTiep == 3) then
				nTenPC_BacTiep = "Phá Thiên Thạch (hoàn chỉnh) (cấp 3)"
		elseif (nCapBacKeTiep == 4) then
				nTenPC_BacTiep = "Phá Thiên Thạch (tinh xảo) (cấp 4)"
		elseif (nCapBacKeTiep == 5) then
				nTenPC_BacTiep = "Phá Thiên Thạch (lấp lánh) (cấp 5)"
		elseif (nCapBacKeTiep == 6) then
				nTenPC_BacTiep = "Phá Thiên Thạch (óng ánh) (Cấp 6)"
		elseif (nCapBacKeTiep == 7) then
				nTenPC_BacTiep = "Phá Thiên Thạch (lung linh) (Cấp 7)"
		elseif (nCapBacKeTiep == 8) then
				nTenPC_BacTiep = "Phá Thiên Thạch (hoàn mỹ) (Cấp 8)"
		elseif (nCapBacKeTiep == 9) then
				nTenPC_BacTiep = "Phá Thiên Thạch (chí tôn) (Cấp 9)"
		elseif (nCapBacKeTiep == 10) then
				nTenPC_BacTiep = "Phá Thiên Thạch (vô song) (Cấp 10)"
		end 
						local nSoLuongDaKham = me.GetItemCountInBags(18,1,1304,nLevel)
...
function tbPhaThienThach:nangcapall(nTen,nSoLuongDaKham,nLevel)
if nLevel == 10 then 
me.Msg("<color=yellow>"..nTen.."<color> đã đạt cấp tối đa không thể nâng cấp")
return 
end 
local nNguyenLieuDaKham = me.GetItemCountInBags(18,1,1304,nLevel) -- Hoa hồng đỏ
...
return 
	...
	me.AddStackItem(18,1,1304,nCapBacKeTiep,{bForceBind=1},nSoLuongDaThanhPham)
	 local tbItemId	= {18,1,1304,nLevel,0,0};  -- Gio hoa
...
function tbPhaThienThach:nangcap(nTen,nLevel)
if nLevel == 10 then 
me.Msg("<color=yellow>"..nTen.."<color> đã đạt cấp tối đa không thể nâng cấp")
return 
end 
...
return 
	...
	me.AddBindCoin(-nDongTon)
	me.AddStackItem(18,1,1304,nCapBacKeTiep,{bForceBind=1},1)
	 local tbItemId	= {18,1,1304,nLevel,0,0};  -- Gio hoa
...
function tbPhaThienThach:khamnam(nTen,nSkillCount,nLevel)
	local nVK_ViTri_A = me.GetTask(9184,1)
	if nVK_ViTri_A == 0 then 
	Dialog:Say("Vũ Khí dòng 1 chưa đục lỗ không thể khảm đá này")
	return 
	end 
	me.SetTask(9183,2,1)
	me.AddFightSkill(1882,nSkillCount);
	 local tbItemId	= {18,1,1304,nLevel,0,0};  -- Gio hoa
