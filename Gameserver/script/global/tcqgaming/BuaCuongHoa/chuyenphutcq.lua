local tbChuyenPhu ={};
SpecialEvent.tbChuyenPhu= tbChuyenPhu;
tbChuyenPhu.TASK_RutCuongHoa = 3130;

function tbChuyenPhu:OnDialog() 
	...
	if not pItem then
		Dialog:Say("Ngươi chưa trang bị <color=yellow>Hộ Phù<color> lên người, không thể chuyển");
		return 0;
	end  
	 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_AMULET, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Phù<color>bên Trang Bị Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenPhu:ChuyenCuongHoa(tbItemObj)
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_AMULET, 0);
	local nCount = 0; 
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
		...
		if szLoai ~= "6" then
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Hộ Phù qua Hộ Phù", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
...
if (nCapCuongHoaChuyen == 16) then 
	...
	Dialog:Say("Đẳng cấp Phù được nhận cường hóa không phù hợp\nLưu Ý : Phù +16 chỉ có thể chuyển cho Phù Cấp 10 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then 
		if (nCapDoTrenNguoi < 10) then
	Dialog:Say("Đẳng cấp Phù được nhận cường hóa không phù hợp\nLưu ý : Phù Cộng 15 chỉ có thể chuyển cho Phù Cấp 10 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end	
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Phù được nhận cường hóa không phù hợp\nLưu ý : Phù Cộng 14 chỉ có thể chuyển cho Phù Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
elseif (nCapCuongHoaChuyen == 13) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Phù được nhận cường hóa không phù hợp\nLưu ý : Phù Cộng 13 chỉ có thể chuyển cho Phù Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
end 
	---------------------------------------------------------------------------------------------------------------------
	...
	local nRet = pItem.Regenerate(
	    ...
	    -- Dialog:Say("Chuyển Cường Hóa Thành Công");
