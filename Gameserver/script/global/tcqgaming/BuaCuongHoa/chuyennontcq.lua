local tbChuyenNon ={};
SpecialEvent.tbChuyenNon= tbChuyenNon;
tbChuyenNon.TASK_RutCuongHoa = 3130;

function tbChuyenNon:OnDialog()
	...
	if not pItem then
		Dialog:Say("Ngươi chưa trang bị <color=yellow>Nón<color> lên người, không thể chuyển");
		return 0;
	end  
	 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_HEAD, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Nón<color>bên Trang Bị Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenNon:ChuyenCuongHoa(tbItemObj)
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
		...
		if szLoai ~= "9" then
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Nón qua Nón", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
...
if (nCapCuongHoaChuyen == 16) then 
	...
	Dialog:Say("Đẳng cấp Nón được nhận cường hóa không phù hợp\nLưu Ý : Nón +16 chỉ có thể chuyển cho Nón Cấp 10 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then 
		if (nCapDoTrenNguoi < 10) then
	Dialog:Say("Đẳng cấp Nón được nhận cường hóa không phù hợp\nLưu ý : Nón Cộng 15 chỉ có thể chuyển cho Nón Cấp 10 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end	
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Nón được nhận cường hóa không phù hợp\nLưu ý : Nón Cộng 14 chỉ có thể chuyển cho Nón Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
elseif (nCapCuongHoaChuyen == 13) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Nón được nhận cường hóa không phù hợp\nLưu ý : Nón Cộng 13 chỉ có thể chuyển cho Nón Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
end 
	---------------------------------------------------------------------------------------------------------------------
