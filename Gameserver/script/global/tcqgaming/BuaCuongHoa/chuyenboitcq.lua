local tbChuyenBoi ={};
SpecialEvent.tbChuyenBoi= tbChuyenBoi;
tbChuyenBoi.TASK_RutCuongHoa = 3130;

function tbChuyenBoi:OnDialog() 
	...
	if not pItem then
		Dialog:Say("Ngươi chưa trang bị <color=yellow>Bội<color> lên người, không thể chuyển");
		return 0;
	end  
	 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_PENDANT, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Bội<color>bên Trang Bị Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenBoi:ChuyenCuongHoa(tbItemObj)
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_PENDANT, 0);
	local nCount = 0; 
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
		...
		if szLoai ~= "11" then
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Bội qua Bội", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
...
if (nCapCuongHoaChuyen == 16) then 
	...
	Dialog:Say("Đẳng cấp Bội được nhận cường hóa không phù hợp\nLưu Ý : Bội +16 chỉ có thể chuyển cho Bội Cấp 10 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then 
		if (nCapDoTrenNguoi < 10) then
	Dialog:Say("Đẳng cấp Bội được nhận cường hóa không phù hợp\nLưu ý : Bội Cộng 15 chỉ có thể chuyển cho Bội Cấp 10 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end	
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Bội được nhận cường hóa không phù hợp\nLưu ý : Bội Cộng 14 chỉ có thể chuyển cho Bội Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
elseif (nCapCuongHoaChuyen == 13) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Bội được nhận cường hóa không phù hợp\nLưu ý : Bội Cộng 13 chỉ có thể chuyển cho Bội Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
end 
	---------------------------------------------------------------------------------------------------------------------
