local tbChuyenTay ={};
SpecialEvent.tbChuyenTay= tbChuyenTay;
tbChuyenTay.TASK_RutCuongHoa = 3130;

function tbChuyenTay:OnDialog()
	...
	-- local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_HEAD, 0);-- Nón
	...
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_CUFF, 0);--Tay

	-- local PItems = pItem1 or pItem2 or pItem3 or pItem4 or pItem5;
	if not pItem then
		Dialog:Say("Ngươi chưa trang bị <color=yellow>Hộ Uyển<color> lên người, không thể chuyển");
		return 0;
	end  
	 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_CUFF, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Hộ Uyển<color> bên Trang Bị Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenTay:ChuyenCuongHoa(tbItemObj)
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_CUFF, 0);
	local nCount = 0; 
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
		...
		if szLoai ~= "10" then
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Hộ Uyển qua Hộ Uyển", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
...
if (nCapCuongHoaChuyen == 16) then 
	...
	Dialog:Say("Đẳng cấp Hộ Uyển được nhận cường hóa không phù hợp\nLưu Ý : Hộ Uyển +16 chỉ có thể chuyển cho Hộ Uyển Cấp 10 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then 
		if (nCapDoTrenNguoi < 10) then
	Dialog:Say("Đẳng cấp Hộ Uyển được nhận cường hóa không phù hợp\nLưu ý : Hộ Uyển Cộng 15 chỉ có thể chuyển cho Hộ Uyển Cấp 10 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end	
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Hộ Uyển được nhận cường hóa không phù hợp\nLưu ý : Hộ Uyển Cộng 14 chỉ có thể chuyển cho Hộ Uyển Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
elseif (nCapCuongHoaChuyen == 13) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Hộ Uyển được nhận cường hóa không phù hợp\nLưu ý : Hộ Uyển Cộng 13 chỉ có thể chuyển cho Hộ Uyển Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
end 
	---------------------------------------------------------------------------------------------------------------------
