local tbChuyenLien ={};
SpecialEvent.tbChuyenLien= tbChuyenLien;
tbChuyenLien.TASK_RutCuongHoa = 3130;

function tbChuyenLien:OnDialog() 
	...
	-- local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_AMULET, 0);-- Phù
	-- local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_RING, 0);--Nhẫn
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_NECKLACE, 0);---Liên
	-- local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_PENDANT, 0);---Bội
	...
	if not pItem then
		Dialog:Say("Ngươi chưa trang bị <color=yellow>Hạng Liên<color> lên người, không thể chuyển");
		return 0;
	end  
	 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_NECKLACE, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Liên<color>bên Trang Bị Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenLien:ChuyenCuongHoa(tbItemObj)
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_NECKLACE, 0);
	local nCount = 0; 
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
		--     	if szLoai ~= "4" and szLoai ~= "5" and szLoai ~= "6" and szLoai ~= "11" then
		if szLoai ~= "5" then
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Hạng Liên qua Hạng Liên", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
...
if (nCapCuongHoaChuyen == 16) then 
	...
	Dialog:Say("Đẳng cấp Hạng Liên được nhận cường hóa không phù hợp\nLưu Ý : Hạng Liên +16 chỉ có thể chuyển cho Hạng Liên Cấp 10 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then 
		if (nCapDoTrenNguoi < 10) then
	Dialog:Say("Đẳng cấp Hạng Liên được nhận cường hóa không phù hợp\nLưu ý : Hạng Liên Cộng 15 chỉ có thể chuyển cho Hạng Liên Cấp 10 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end	
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Hạng Liên được nhận cường hóa không phù hợp\nLưu ý : Hạng Liên Cộng 14 chỉ có thể chuyển cho Hạng Liên Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
elseif (nCapCuongHoaChuyen == 13) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Hạng Liên được nhận cường hóa không phù hợp\nLưu ý : Hạng Liên Cộng 13 chỉ có thể chuyển cho Hạng Liên Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
end 
	---------------------------------------------------------------------------------------------------------------------
