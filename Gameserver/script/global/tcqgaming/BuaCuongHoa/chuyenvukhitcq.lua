local tbChuyenVuKhi ={};
SpecialEvent.tbChuyenVuKhi= tbChuyenVuKhi;
tbChuyenVuKhi.TASK_RutCuongHoa = 3130;

function tbChuyenVuKhi:OnDialog()
	...
	if not pItem then
 	...
 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_WEAPON, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Vũ Khí<color> Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenVuKhi:ChuyenCuongHoa(tbItemObj)
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_WEAPON, 0);
	local nCount = 0; 
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
        ...
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Vũ Khí qua Vũ Khí", {"Kết thúc đối thoại !"}); 
	    return 0;
        end
	...
	local nCheckCHMax = pItem.nParticular;
	local nCapDoTrenNguoi = pItem.nLevel;
	local nCapCuongHoaChuyen = CheckItem.nEnhTimes;
if (nCapCuongHoaChuyen == 16)  then 
		-- if (nCapDoTrenNguoi < 9) and (nCheckCHMax >= 10) then ------ cho phép + 16 vũ khí hải tặc cấp 8x
		if (nCapDoTrenNguoi < 9) then 
		Dialog:Say("Đẳng cấp vũ khí được nhận cường hóa không phù hợp\nLưu Ý : Vũ Khí +16 chỉ có thể chuyển cho Vũ Khí cấp 9 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then
    -- Kiểm tra nếu cấp cường hóa chuyển là +15
    if (nCapDoTrenNguoi < 9) then 
        Dialog:Say("Đẳng cấp vũ khí được nhận cường hóa không phù hợp\nLưu Ý : Vũ Khí +15 chỉ có thể chuyển cho Vũ Khí cấp 9 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
        return 0;
    end
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 8) then
	Dialog:Say("Đẳng cấp vũ khí được nhận cường hóa không phù hợp\nLưu ý : Vũ Khí Cộng 14 chỉ có thể chuyển cho Vũ Khí Cấp 8 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
		else
				-- if (nCapDoTrenNguoi < 8) then 
