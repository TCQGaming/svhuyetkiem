local tbChuyenDai ={};
SpecialEvent.tbChuyenDai= tbChuyenDai;
tbChuyenDai.TASK_RutCuongHoa = 3130;

function tbChuyenDai:OnDialog()
	...
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_BELT, 0);---Đai
	...
	if not pItem then
		Dialog:Say("Ngươi chưa trang bị <color=yellow>Đai Lưng<color> lên người, không thể chuyển");
		return 0;
	end  
	 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_BELT, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Đai<color>bên Trang Bị Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenDai:ChuyenCuongHoa(tbItemObj)
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
		...
		if szLoai ~= "8" then
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Đai Lưng qua Đai Lưng", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
...
if (nCapCuongHoaChuyen == 16) then 
	...
	Dialog:Say("Đẳng cấp Đai Lưng được nhận cường hóa không phù hợp\nLưu Ý : Đai Lưng +16 chỉ có thể chuyển cho Đai Lưng Cấp 10 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then 
		if (nCapDoTrenNguoi < 10) then
	Dialog:Say("Đẳng cấp Đai Lưng được nhận cường hóa không phù hợp\nLưu ý : Đai Lưng Cộng 15 chỉ có thể chuyển cho Đai Lưng Cấp 10 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end	
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Đai Lưng được nhận cường hóa không phù hợp\nLưu ý : Đai Lưng Cộng 14 chỉ có thể chuyển cho Đai Lưng Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
elseif (nCapCuongHoaChuyen == 13) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Đai Lưng được nhận cường hóa không phù hợp\nLưu ý : Đai Lưng Cộng 13 chỉ có thể chuyển cho Đai Lưng Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
end 
	---------------------------------------------------------------------------------------------------------------------
