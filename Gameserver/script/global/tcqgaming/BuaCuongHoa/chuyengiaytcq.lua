local tbChuyenGiay ={};
SpecialEvent.tbChuyenGiay= tbChuyenGiay;
tbChuyenGiay.TASK_RutCuongHoa = 3130;

function tbChuyenGiay:OnDialog()
	...
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_FOOT, 0);---Giày
	...
	if not pItem then
		Dialog:Say("Ngươi chưa trang bị <color=yellow>Giày<color> lên người, không thể chuyển");
		return 0;
	end  
	 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_FOOT, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Giày<color>bên Trang Bị Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenGiay:ChuyenCuongHoa(tbItemObj)
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
		...
		if szLoai ~= "7" then
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Giày qua Giày", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
...
if (nCapCuongHoaChuyen == 16) then 
	...
	Dialog:Say("Đẳng cấp Giày được nhận cường hóa không phù hợp\nLưu Ý : Giày +16 chỉ có thể chuyển cho Giày Cấp 10 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then 
		if (nCapDoTrenNguoi < 10) then
	Dialog:Say("Đẳng cấp Giày được nhận cường hóa không phù hợp\nLưu ý : Giày Cộng 15 chỉ có thể chuyển cho Giày Cấp 10 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end	
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Giày được nhận cường hóa không phù hợp\nLưu ý : Giày Cộng 14 chỉ có thể chuyển cho Giày Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
elseif (nCapCuongHoaChuyen == 13) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Giày được nhận cường hóa không phù hợp\nLưu ý : Giày Cộng 13 chỉ có thể chuyển cho Giày Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
end 
	---------------------------------------------------------------------------------------------------------------------
