local tbChuyenNhan ={};
SpecialEvent.tbChuyenNhan= tbChuyenNhan;
tbChuyenNhan.TASK_RutCuongHoa = 3130;

function tbChuyenNhan:OnDialog() 
	...
	if not pItem then
		Dialog:Say("Ngươi chưa trang bị <color=yellow>Giới Chỉ<color> lên người, không thể chuyển");
		return 0;
	end  
	 	local pItemPhu = me.GetItem(Item.ROOM_EQUIPEX,Item.EQUIPEXPOS_RING, 0);
	...
	Dialog:Say("Ngươi cần tháo <color=yellow>Nhẫn<color>bên Trang Bị Phụ xuống hành trang mới có thể chuyển");
	return 0;
end 
end

function tbChuyenNhan:ChuyenCuongHoa(tbItemObj)
	local pItem = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_RING, 0);
	local nCount = 0; 
    ...
    local szLever = string.format("%s",CheckItem.nEnhTimes);
     local szLoai = string.format("%s",CheckItem.nDetail);
		...
		if szLoai ~= "4" then
        Dialog:Say("Sai rồi , bạn chỉ có thể chuyển Cường Hóa từ Giới Chỉ qua Giới Chỉ", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
...
if (nCapCuongHoaChuyen == 16) then 
	...
	Dialog:Say("Đẳng cấp Nhẫn được nhận cường hóa không phù hợp\nLưu Ý : Nhẫn +16 chỉ có thể chuyển cho Nhẫn Cấp 10 hoặc Ngược Lại ", {"Kết thúc đối thoại !"}); 
		return 0;
		end 
elseif (nCapCuongHoaChuyen == 15) then 
		if (nCapDoTrenNguoi < 10) then
	Dialog:Say("Đẳng cấp Nhẫn được nhận cường hóa không phù hợp\nLưu ý : Nhẫn Cộng 15 chỉ có thể chuyển cho Nhẫn Cấp 10 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end	
elseif (nCapCuongHoaChuyen == 14) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Nhẫn được nhận cường hóa không phù hợp\nLưu ý : Nhẫn Cộng 14 chỉ có thể chuyển cho Nhẫn Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
elseif (nCapCuongHoaChuyen == 13) then 
		if (nCapDoTrenNguoi < 9) then
	Dialog:Say("Đẳng cấp Nhẫn được nhận cường hóa không phù hợp\nLưu ý : Nhẫn Cộng 13 chỉ có thể chuyển cho Nhẫn Cấp 9 hoặc Ngược Lại", {"Kết thúc đối thoại !"}); 
		return 0; 
		end
end 
	---------------------------------------------------------------------------------------------------------------------
