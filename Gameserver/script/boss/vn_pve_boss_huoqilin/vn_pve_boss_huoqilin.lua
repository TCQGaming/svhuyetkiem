...
-- Șʼ܊boss
local tbQinshihuangBoss	= Npc:GetClass("vn_pve_boss_huoqilin");
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5,"vn_pve_boss_huoqilin");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5,"vn_pve_boss_huoqilin");
			end
		end
	end
