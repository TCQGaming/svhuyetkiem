-------------------------------------------------------
-- τݾĻaúvn_pve_boss_huoqilin_songjin_hero.lua
...
-- Șʼ܊boss
local tbQinshihuangBoss	= Npc:GetClass("vn_pve_boss_huoqilin_songjin_hero");
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5,"vn_pve_boss_huoqilin_songjin_hero");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5,"vn_pve_boss_huoqilin_songjin_hero");
			end
		end
	end
