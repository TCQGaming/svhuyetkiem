-------------------------------------------------------
-- τݾĻaúvn_pve_boss_huoqilin_wanted.lua
...
-- Șʼ܊boss
local tbQinshihuangBoss	= Npc:GetClass("vn_pve_boss_huoqilin_wanted");
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5,"vn_pve_boss_huoqilin_wanted");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5,"vn_pve_boss_huoqilin_wanted");
			end
		end
	end
