...
--	ClearMapNpcWithName(self.tbBase.nMapId,CrossTimeRoom.nBossWangS1TemplateId);
--	ClearMapNpcWithName(self.tbBase.nMapId,CrossTimeRoom.nBossWangS2TemplateId);
--	ClearMapNpcWithName(self.tbBase.nMapId,CrossTimeRoom.nStopNpc_BigTemplateId);
--	ClearMapNpcWithName(self.tbBase.nMapId,CrossTimeRoom.nStopNpc_NormalTemplateId);
--	ClearMapNpcWithName(self.tbBase.nMapId,CrossTimeRoom.nBossWangBeginerTemplateId);
end
...
function tb1stRoom:StartRoom()
	...
	self.tbBase:AllBlackBoard("Phía trước có vị bạch y hiệp sĩ, tới hỏi anh ta?");
end

function tb1stRoom:FailedRoom()
	self.nIsFailed = 1;
	self:ClearRoom();
	self.tbBase:AllBlackBoard("Quá khó khăn ! Hình như ngươi đã thất bại");
end
...
function tb1stRoom:TalkS1End()
	if self.pBossS1  then
		if self.nTalkState == 1 then
			self.tbBase:NpcTalk(self.pBossS1.dwId,"Nhất túy giang hồ ba mươi xuân, yên đắc thư kiếm giải hồng trần.. ");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 2 then
			self.tbBase:NpcTalk(self.pBossS1.dwId,"Ngươi yếu chống đỡ ta thấy văn Tiểu Nguyệt, ta tựu không khách.. ");
			self.nTalkState = self.nTalkState + 1;
			self.pBossS1.SetCurCamp(5);	--变为战斗状态
			self.pBossS1.SetActiveForever(1);
			self.nTalkTimer = 0;
			self.tbBase:UpdateUiState("<color=yellow>Ngăn cản Dương Vi Phong được biết Tiểu Nguyệt đã chết<color>");
			return 0
		else
			self.nTalkTimer = 0;
			return 0;
...
--		--Npc:RegPNpcLifePercentReduce(self.pBossS2,nPercent,self.CastRenxin,self);
	...
	self.tbBase:AllBlackBoard("Vương Di Phong dường ngư có một điềm báo Tiểu Nguyệt tao ngộ bất trắc");
	self.nTalkState = 1;
...
function tb1stRoom:TalkS2End()
	if self.pBossS2  then
		if self.nTalkState == 1 then
			self.tbBase:NpcTalk(self.pBossS2.dwId,"Các ngươi không cho ta thấy Tiểu Nguyệt, chẳng lẽ là Tiểu Nguyệt xảy ra sự tình?");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 2 then
			self.tbBase:NpcTalk(self.pBossS2.dwId,"Ta muốn đem các ngươi giết sạch báo thù cho Tiểu Nguyệt!");
			self.nTalkState = self.nTalkState + 1;
...
function tb1stRoom:CastXinMo()
	...
	self.tbBase:NpcTalk(self.pBossS2.dwId,"Chết không có chỗ chôn ");
end
...
--			Achievement:FinishAchievement(pPlayer,403);	--Thành tựu
		end
	end
	self:EndRoom();
end
...
function tb2ndRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Võ Mị Nương");
	ClearMapNpcWithName(self.tbBase.nMapId,"Hồ Ly");
	ClearMapNpcWithName(self.tbBase.nMapId,"Lý Thuần Phong");
	ClearMapNpcWithName(self.tbBase.nMapId,"Viên Thiên Canh");
	ClearMapNpcWithName(self.tbBase.nMapId,"Phương Sĩ");
end
...
function tb2ndRoom:FailedRoom()
	self.nIsFailed = 1;
	self:ClearRoom();
	self.tbBase:AllBlackBoard("Người phụ nữ quyết rũ giống như hồ ly , ngươi có muốn thay đổi lịch sử?");	
end
...
function tb2ndRoom:TalkEnd()
	if self.pMeiniang  then
		if self.nTalkState == 1 then
			self.tbBase:NpcTalk(self.pMeiniang.dwId,"Ta là Hoàng hậu, ngươi vì sao xuất hiện tại nhà của ta trong Đại điện?");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 2 then
...
--			Npc:RegDeathLoseItem(self.pBossYuan,self.tbBase.OnBossDrop,self.tbBase);	--掉落回调
			...
			self.tbBase:NpcTalk(self.pMeiniang.dwId,"Các ngươi là ai, a! ! ! ! !");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 3 then
			self.tbBase:NpcTalk(self.pBossLi.dwId,"Yêu nữ...... ");
			self.pBossYuan.SetCurCamp(5);
			self.pBossLi.SetCurCamp(5);
			self:AddFangshi();
			self:SetBossLiAi();
			self.nTalkTimer = 0;
			self.tbBase:UpdateUiState("<color=yellow>Đạo sĩ chòm sao Bắc Đẩu đã giải cứu được Hoàng hậu<color>");
			return 0;
		else
			self.nTalkTimer = 0;
			return 0;
...
function tb2ndRoom:OnBossYuanDeath()
	...
	for _,pPlayer in pairs(tbPlayer) do
		if pPlayer then
			...
			--Achievement:FinishAchievement(pPlayer,404);	--Thành tựu
		end
	end
	self:EndRoom();
end
...
function tb3rdRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Đứa bé");
	ClearMapNpcWithName(self.tbBase.nMapId,"Vương Gia");
	ClearMapNpcWithName(self.tbBase.nMapId,"Tiểu Thái Giám");
	ClearMapNpcWithName(self.tbBase.nMapId,"Người Giúp Đỡ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Định Cốt Băng Phách");
	ClearMapNpcWithName(self.tbBase.nMapId,"Ngự tiền quân sĩ");
end
...
function tb3rdRoom:FailedRoom()
	self.nIsFailed = 1;
	self:ClearRoom();
	self.tbBase:AllBlackBoard("Tài cán không có bao nhiêu mà bon chen sự đời");
end
...
function tb3rdRoom:TalkEnd()
	if self.pMan then
		if self.nTalkState == 1 then
			self.tbBase:NpcTalk(self.pMan.dwId,"Tiểu tử, tuy rằng ngươi nghe không hiểu...");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 2 then
			self.tbBase:NpcTalk(self.pMan.dwId,"Ta phụ hoàng vừa nói cho ta biết, bởi vì một ... quan hệ ...");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 3 then
			self.tbBase:NpcTalk(self.pMan.dwId,"...Để tránh đã thương thần dân thiên hạ , chúng ta phải trục xuất"); 
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
...
--				Npc:RegDeathLoseItem(self.pBoss,self.tbBase.OnBossDrop,self.tbBase);	--掉落回调
		...
		elseif self.nTalkState == 5 then
			if self.pBoss then
				self.tbBase:NpcTalk(self.pBoss.dwId,"Mau mau rời khỏi không thì chết ngay tức thì");
			end
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 6 then
			if self.pBoy then
				self.tbBase:NpcTalk(self.pBoy.dwId,"Cha...");
			end
			if self.pBoss then
			...
			self.tbBase:UpdateUiState("<color=yellow>Giết Tiểu Thái Giám , bảo vệ Hoài Bích Vương Gia<color>");
			self.nTalkTimer = 0;
			return 0;
		else
			self.nTalkTimer = 0;
...
function tb3rdRoom:OnBorn()
	...
	self.tbBase:NpcTalk(self.pBoss.dwId,"Muốn chạy? Người a!");
...
function tb3rdRoom:OnBossDeath()
	...
	for _,pPlayer in pairs(tbPlayer) do
		...
		--	Achievement:FinishAchievement(pPlayer,405);	--Thành tựu
		end
	end
	self:EndRoom();
end
...
function tb4thRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Hiệu ứng BOSS triệu hồi gia tộc");
	ClearMapNpcWithName(self.tbBase.nMapId,"Dương Ảnh Phong");
	ClearMapNpcWithName(self.tbBase.nMapId,"Trác Phi Phàm");
	ClearMapNpcWithName(self.tbBase.nMapId,"Tử Hiên");
	ClearMapNpcWithName(self.tbBase.nMapId,"Nạp Lan Chân");
	ClearMapNpcWithName(self.tbBase.nMapId,"Hoa Nguyệt Ảnh");
	ClearMapNpcWithName(self.tbBase.nMapId,"Thương tình thảo của Tử Hiên");
end
...
function tb4thRoom:FailedRoom()
	self.nIsFailed = 1;
	self:ClearRoom();
	self.tbBase:AllBlackBoard("Tốt xấu độc đích tâm ma trận, liên ngươi cũng mê thất trong đó");
end
...
function tb4thRoom:TalkEnd()
	if self.pYangyingfeng then
		if self.nTalkState == 1 then
			self.tbBase:NpcTalk(self.pYangyingfeng.dwId,"Nạp Lan chân thương mến....");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 2 then
			self.tbBase:NpcTalk(self.pYangyingfeng.dwId,"Sở dĩ không thể vượt qua phụ thân để bố trí Đích Tâm Ma Trận. . ....");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 3 then 
			self.tbBase:NpcTalk(self.pYangyingfeng.dwId,"One more time ! Thử lần nữa xem...");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 4 then
			...
			self.tbBase:UpdateUiState("<color=yellow>帮助为情所困的杨影枫，勇闯心魔阵<color>");
			self.nTalkTimer = 0;
			return 0;
		else
			self.nTalkTimer = 0;
...
function tb4thRoom:CastTianxianzi()
	...
	self.tbBase:AllBlackBoard("Thiên tiên hạ phàm, mê hoặc chúng sinh.....");
...
function tb4thRoom:OnBossNalanDeath()
	...
	for _,pPlayer in pairs(tbPlayer) do
		...
		---	Achievement:FinishAchievement(pPlayer,406);	--Thành tựu
		end
	end
	--黑条显示
	self:EndRoom();
...
function tb5thRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Đại Tư Mệnh");
	ClearMapNpcWithName(self.tbBase.nMapId,"Bù Nhìn");
	-- ClearMapNpcWithName(self.tbBase.nMapId,CrossTimeRoom.nChildFightTemplateId);
	ClearMapNpcWithName(self.tbBase.nMapId,"U Minh Khuyển");
	ClearMapNpcWithName(self.tbBase.nMapId,"Tâm Ma Trận Nhãn");
	ClearMapNpcWithName(self.tbBase.nMapId,"Thủ Hộ Thâm Uyên");
	ClearMapNpcWithName(self.tbBase.nMapId,"Thi Quỷ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Xác Sống");
	ClearMapNpcWithName(self.tbBase.nMapId,"Hắc Thủy");
end
...
function tb5thRoom:FailedRoom()
	self.nIsFailed = 1;
	self:ClearRoom();
	self.tbBase:AllBlackBoard("Ngươi công lực chưa đủ");	
end
...
function tb5thRoom:TalkEnd()
	if self.pBoss  then
		if self.nTalkState == 1 then
			self.tbBase:NpcTalk(self.pBoss.dwId,"Các ngươi phát hiện lối vào Thời Quang Điện , còn đây là địa điểm tuyệt mật");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 2 then
			self.tbBase:NpcTalk(self.pBoss.dwId,"Thế nhân cho rằng từ lâu Nho giáo đã nắm quyền kiểm soát");
			self.nTalkState = self.nTalkState + 1;
			return 4 * Env.GAME_FPS;
		elseif self.nTalkState == 3 then
			self.tbBase:NpcTalk(self.pBoss.dwId,"Không nghĩ tới bị các ngươi phát hiện, đã biết nhiều lắm đích nhân thị bất năng sống sót");
			self.pBoss.SetCurCamp(5);	--变为战斗状态
			self:ChangeChildFight();	--改变傀儡战斗状态
			self.tbBase:UpdateUiState("<color=yellow>Đánh bại Đại Ti Mệnh, xông ra khỏi Thời Quang Điện<color>");
			self.nTalkTimer = 0;
			return 0;
		else
			self.nTalkTimer = 0;
...
function tb5thRoom:ChangeChildFight()
	ClearMapNpcWithName(self.tbBase.nMapId,"Bù Nhìn");	--先清除非战斗傀儡
...
function tb5thRoom:BornChild(nPercent)
	...
	elseif nPercent == 80 then
		...
		if self.nKuileiCount > 0 and self.pBoss then
			self.tbBase:AllBlackBoard("Mê thất tại quá khứ đích thi nhân chính ý đồ thoát đi");
			self.pBoss.SetCurCamp(6);
		end	
	end
end
...
function tb5thRoom:ProcessStep(nPercent)
	if nPercent == 70 then	--第二阶段
		self:ProcessStep2();
		self.tbBase:AllBlackBoard("Âm u khuyển tựa hồ cũng bị dục vọng xanh phá");  --test
	elseif nPercent == 50 then	--第三阶段
		self:ProcessStep3();
		self.tbBase:AllBlackBoard("Tâm ma trận mở ra, đi xem ngươi nội tâm đích tuyển trạch");  --test
	elseif nPercent == 20 then	--第四阶段
		self:ProcessStep4();
		self.tbBase:AllBlackBoard("Thời không vực sâu đích người thủ hộ xuất hiện liễu");	--test
	end 
end
...
function tb5thRoom:ScanDeath()
	...
	for nId,nFlag in pairs(self.tbCrazyNpc) do
		if nFlag == 1 then
			...
			if pNpc then
				--增加一个30 phút的狂乱状态
				pNpc.AddSkillState(CrossTimeRoom.nCrazySkillIdIR,11,2,30 * 60 * Env.GAME_FPS,1,0,1);
				pNpc.AddSkillState(CrossTimeRoom.nCrazySkillIdAD,11,2,30 * 60 * Env.GAME_FPS,1,0,1);
			end
		end
	end
...
function tb5thRoom:AddShouhuzhe()
	...
	self.tbBase:NpcTalk(self.pBoss.dwId,"Vô biên đích trong vực sâu có người đang chờ ngươi...");
end
...
function tb5thRoom:OnBossDeath()
	...
	for _,pPlayer in pairs(tbPlayer) do
		...
		--	Achievement:FinishAchievement(pPlayer,407);	--Thành tựu
		--	Achievement:FinishAchievement(pPlayer,408);	--Thành tựu
...
function tb5thRoom:RoomFinish()
	self.tbBase:RoomFinish();
	self.tbBase:AllBlackBoard("雨过天晴，前尘湮灭，可以离开此地了");
	self:AddTransferNpc();	--刷传送npc
end
