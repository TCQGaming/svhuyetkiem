...
--	ClearMapNpcWithName(self.tbBase.nMapId,"NPC Cơ quan");
--	ClearMapNpcWithName(self.tbBase.nMapId,"Vòng Sáng Trắng");
	for _,tbInfo in pairs(self.tbMachineEnemeyNpc) do
--		ClearMapNpcWithName(self.tbBase.nMapId,"Trinh sát");
	end
	for nTempId,_ in pairs(self.tbMachineManager) do
--		ClearMapNpcWithName(self.tbBase.nMapId,"Vòng Sáng Vàng");
--		ClearMapNpcWithName(self.tbBase.nMapId,"Vòng Sáng Đỏ");
--		ClearMapNpcWithName(self.tbBase.nMapId,"Vòng Sáng Xanh");
	end
end
...
function tb1stRoom:StartRoom()
	...
	self:InitTrapEnemy();
	self:StartSwitch();
	self.tbBase:UpdateUiState("Đêm gặp mê trấn trinh sát ẩn\n\n<color=red>Giải quyết tất cả trinh sát<color>");
end
...
function tb1stRoom:FailedRoom()
	...
	self.tbBase:AllBlackBoard("Bày trận người quá mức lợi hại! Xem ra còn cần từ đầu thương nghị. ");
end
...
function tb1stRoom:EndRoom()
	...
	self.tbBase:AllBlackBoard("Trận pháp trong nháy mắt biến mất, mau mau bắt được phía trước trinh sát!");
	self:ClearRoom();
	self:RoomFinish();
end
...
tb2ndRoom.nBossCastSkillId = 1843;	--杀死平民释放的技能
...
{
	"Phong Tu Cư Sĩ?",
	"Hắn liền ở tại bên ngoài trấn hàn quang châu......",
	"Thường xuyên đến trong trấn mua thêm chút ăn uống......",	
};

tb2ndRoom.tbBossTalkText = 
{	
	"Nguyên, nguyên lai là hiểu lầm. Các vị muốn tìm Phong Tụ Cư Sĩ? Bao tại ta trên người!",
};

--logic
function tb2ndRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Cô Nương Đi Đường");
	ClearMapNpcWithName(self.tbBase.nMapId,"Cô Nương Đi Đường");
	ClearMapNpcWithName(self.tbBase.nMapId,"Tinh Hà Dị Thú");
	ClearMapNpcWithName(self.tbBase.nMapId,"Ám Vệ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Địch Phách Tư");
	ClearMapNpcWithName(self.tbBase.nMapId,"Trí Liêu Tát Mãn");
	ClearMapNpcWithName(self.tbBase.nMapId,"Phó Đề");
	ClearMapNpcWithName(self.tbBase.nMapId,"Dân thường");
	ClearMapNpcWithName(self.tbBase.nMapId,"Mộc Đạc Lạc");
end
...
function tb2ndRoom:StartRoom()
	...
	self.tbBase:UpdateUiState("Phố xá sầm uất ngẫu nhiên gặp ngông cuồng vừa thôi\n\n<color=red>Đánh thắng mộc đạc rơi<color>");
end
...
function tb2ndRoom:OnBossPercent(nNpcId)
	...
	self.tbBase:AllBlackBoard("Các huynh đệ kiềm chế một chút，Đừng làm bị thương bên cạnh người!");
end
...
function tb2ndRoom:OnBossDeath()
	...
	ClearMapNpcWithName(self.tbBase.nMapId,"Dân thường"); --清除平民
...
function tb2ndRoom:OnBossTalk(nNpcId)
	...
	if szContent then
		...
		self.tbBase:UpdateUiState("<color=red>Đi theo mộc đạc hướng về tiến lên đi<color>");
		return 0;
	end
end
...
function tb2ndRoom:FailedRoom()
	...
	self:ClearRoom();
	self.tbBase:StartCurrentRoom();
	self.tbBase:AllBlackBoard("Mộc đạc rơi:“Dám bắt chuyện Ma La?!Thấy các ngươi một lần đánh một lần!”");
end
...
function tb2ndRoom:EndRoom()
	self.nIsFinished = 1;
	self.tbBase:AllBlackBoard("Ân，Thuận con đường này，Cuối cùng thạch ốc chính là. Sau này còn gặp lại!");
	self:ClearRoom();
	self:RoomFinish();
	-- 发经验
...
tb3rdRoom.tbNotifyNpcTalkContent = { --提示说的话语
	"Hì hì，Lam mà sống，Hoàng vì chết. ",
	"Bên này bên này~Bên này là màu vàng. ",
	"Bên này bên này~Bên này là màu lam. ",
	"=3=~Bên này mới không phải màu lam. ",
	"=3=~Bên này mới không phải màu vàng. ",
	"Ta sẽ nói cho ngươi biết bên này là màu lam tinh tinh sao?",
	"Ta sẽ nói cho ngươi biết bên này là màu vàng tinh tinh sao?",
	"Bạn Đoán bên nào là màu lam tinh tinh?~",
	"Bạn Đoán bên nào là màu vàng tinh tinh?~",
	"Ha ha!Đồ đần mới tin ta nói là màu vàng~",
	"Ha ha!Đồ đần mới tin ta nói là màu lam~",
};	
...
function tb3rdRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Tinh Hà Dị Thú");
	ClearMapNpcWithName(self.tbBase.nMapId,"Tinh Trận Tiểu Đồng");
end
...
function tb3rdRoom:OnBossDeath()
	ClearMapNpcWithName(self.tbBase.nMapId,"Tinh Trận Tiểu Đồng");	--如果有提示npc，先清除
--	self.tbBase:NpcDropItem(him);
	self:EndRoom();
end
...
function tb3rdRoom:FailedRoom()
	...
	self:ClearRoom();
	self.tbBase:StartCurrentRoom();
	self.tbBase:AllBlackBoard("Trước mắt tối sầm, ngân hà dị thú của lam quang ly các ngươi càng ngày càng xa");
end
...
function tb3rdRoom:EndRoom()
	self.nIsFinished = 1;
	self.tbBase:AllBlackBoard("Ngân hà dị thú đi phía trước phòng bằng đá hình vuông trốn chạy, mau đuổi theo!");
	self:ClearRoom();
	self:RoomFinish();
end
...
{
	"Đường đột các vị, phong tụ xấu hổ. Chư vị sở tầm thương đội, ta từ lâu tống chi rời khỏi.",
	"Ta nãi tinh tượng gia tộc của người chết tối hậu một người, cừu gia vi độc khuy thiên vận",
	"Không chỉ có diệt ta bộ tộc, hoàn bức ta trốn chết đến tận đây",	
	"Bất đắc dĩ tài tại trấn khẩu dữ nơi này bày trận pháp",
	"Ta hiện tại sẽ đưa các vị rời khỏi, xin theo ta lai",
};

---logic
function tb4thRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Kỳ Đăng Vệ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Đèn sáng");
	ClearMapNpcWithName(self.tbBase.nMapId,"Phong Tụ Cư Sĩ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Phong Tụ Cư Sĩ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Phong Tụ Cư Sĩ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Đèn Chưa Đốt");
	ClearMapNpcWithName(self.tbBase.nMapId,"Đèn Đã Đốt");
	ClearMapNpcWithName(self.tbBase.nMapId,"Tinh Hỏa Đăng");
	ClearMapNpcWithName(self.tbBase.nMapId,"Kỳ Đăng Vệ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Kỳ Đăng Vệ");
end
...
function tb4thRoom:StartRoom()
	...
	self:ClearRoom();
	self:AddStarManager();
	self.tbBase:UpdateUiState("Nguyệt mộng tinh kỳ ai thương\n\n<color=red>Giải khai đèn cờ trận<color>");
end
...
function tb4thRoom:StartFireLight()
	...
	self.nIsLightBegin = 1;	-- đã bắt đầu点灯了
	self.tbBase:UpdateUiState(string.format("Nguyệt mộng tinh kỳ ai thương\n\n<color=green>Đèn được thắp sáng:%s/%s<color>\n\n<color=red>Đèn đốt sai:%s/%s<color>",
...
function tb4thRoom:ProcessFireLight(nIsCorrect,nNpcId,nX,nY)
	if nIsCorrect == 1 then
		...
		if nIdx then
			...
			self.tbBase:AllBlackBoard("Đui đèn phát sinh một tiếng giòn hưởng, nhiễu loạn bế trận phương pháp. ");
			...
			if self.nFireErrorLightCount > self.nCanFireErrorCount then	--点错超过数量
				self.tbBase:AllBlackBoard("Trật khớp giữ đèn ba lần, đèn mảng cờ khởi động lại -");
...
function tb4thRoom:ProcessStep()
	...
	if self.nCurrentStep > #self.tbNeedFireLightCount then
		...
		self.tbBase:UpdateUiState("Nguyệt mộng tinh kỳ ai thương\n\n<color=red>Tiêu diệt Phong Tụ Cư Sĩ<color>");
		self:ClearTimer();
		self:ClearLight();
		self:ClearNpc();
		self:AddStarBoss();
...
function tb4thRoom:OnCastMovieSkill(nNpcId)
	...
	self.tbBase:AllBlackBoard("Mảng cờ Light có các giải pháp, bạn Mo hoảng loạn———————");
	return 0;
end
...
function tb4thRoom:FailedRoom()
	...
	self:ClearRoom();
	self.tbBase:StartCurrentRoom();
	self.tbBase:AllBlackBoard("Chẳng lẽ thế gian tái vô hiệp sĩ khả giải đăng kỳ trận chi mê sao??");
end
...
function tb4thRoom:EndRoom()
	self.nIsFinished = 1;
	self.tbBase:AllBlackBoard("Hắn dù chưa nói rõ, chắc chắn tất cả gian khổ, tiên theo hắn rời khỏi nơi đây ba");
	self:ClearRoom();
	self:RoomFinish();
end
...
{
	"Ta tại trấn khẩu an trí hạ tam căn định hồn thạch",
	"Có thể dùng truy binh vô pháp đi ra ngoài cầu viện",
	"Hiện nay chúng ta tổn hại định hồn thạch liền khả phá trận rời khỏi",
};
...
tb5thRoom.nNotifySkillId = 1842; --提示aoe

tb5thRoom.nPassDebuffId = 1839;	--传递debuff

tb5thRoom.nPassChildDebuffId = 1840;
...
function tb5thRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Tùy Tùng");
	ClearMapNpcWithName(self.tbBase.nMapId,"Phong Tụ Cư Sĩ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Tinh Hà Dị Thú");
	ClearMapNpcWithName(self.tbBase.nMapId,"Ngựa Thần");
	ClearMapNpcWithName(self.tbBase.nMapId,"Ám Vệ");
	ClearMapNpcWithName(self.tbBase.nMapId,"Tùy Tùng");
end
...
function tb5thRoom:StartRoom()
	...
	self:AddStarManager();
	self:AddBoss();
	self.tbBase:UpdateUiState("Đánh một trận rầm ri tại đây\n\n<color=red>Đánh bại Thủ Lĩnh Truy Binh<color>");
end
...
function tb5thRoom:AddWalkNpc()
	...
	if pNpc then
		...
		self.tbBase:AllBlackBoard("Phong Tu Cư Sĩ: Đừng vội ham chiến! Khoái theo ta đi!!");
	end
end
...
function tb5thRoom:OnBossPercent(nBossId,nPercent)
	...
	self.tbBase:AllBlackBoard("Trong đám người một người bộ dạng khả nghi, tiên bả người nọ giải quyết thử xem!");
...
function tb5thRoom:CastPassDebuff(nBossId)
	...
	self.tbBase:AllBlackBoard("Phong Tu Cư Sĩ:“Tiêu tản mác! Hai người truyền lại tài bảo không chết!!”");
...
function tb5thRoom:AddPassDebuff(nBossId)
	...
	if pPlayer then
		pPlayer.AddSkillState(self.nPassDebuffId,2,0,5 * 60 * Env.GAME_FPS,0,0);
	end
end
...
function tb5thRoom:FailedRoom()
	...
	self:ClearRoom();
	self.tbBase:StartCurrentRoom();
	self.tbBase:AllBlackBoard("追兵头儿:“Ha ha!九世星盘我带走了!”");
end
...
tb6thRoom.nBossSkillId = 1841;

tb6thRoom.nBossExp = 3600000;	

tb6thRoom.nPassDebuffId = 1839;	--传递debuff

tb6thRoom.nPassChildDebuffId = 1840;
...
{
	"Huyết anh thiên phóng vi hư, đóa vu sóng gợn trong vòng không việc gì.",
	"Phược thần anh sát nhu dĩ thân lan kiếm khí, háo huyết khí tăng công tu.",
	"Dạ anh mạn thiên la sở triền người hưu lộn xộn, qua đi tốc lai ngô chỗ.",
};

--logic
function tb6thRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Tinh Hà Dị Thú");		--清boss
	ClearMapNpcWithName(self.tbBase.nMapId,"Phong Tụ Cư Sĩ");	
end
...
function tb6thRoom:OnEnd(nNpcId)
	...
	self.nIsMovieStart = 1;	--剧情 bắt đầu
	self:EndRoom();
	return 0;	
end
...
function tb6thRoom:FailedRoom()
	...
	self:ClearRoom();
	self.tbBase:StartCurrentRoom();
	self.tbBase:AllBlackBoard("Thác Bạt Phù Anh:“Các ngươi mang không đi hắn. ”");
end
...
function tb6thRoom:EndRoom()
	...
	self.tbBase:AllBlackBoard("Phong Tu Cư Sĩ:“Người này giao cho ta，Chờ dị thú vừa đến，Các ngươi nhanh thừa dị thú rời đi!”");
end
...
tb7thRoom.tbTalkContent = {
	"Bạn Có thể quên thù truyền kiếp theo ta đi?",
	"Ta không dám vong, ta nhất vong. . . Trên đời liền không nữa di động anh.",
	"Ha ha... Trên đời vốn là không có gì di động anh, chỉ có ta.",
	". . . Ngươi xác thực điều không phải tha, ngươi là thác bạt thị của anh hùng.",
	"Ngươi biết rõ... Ta không có khả năng ruồng bỏ ta của bộ tộc.",
	"Ha ha, ta hay thái biết. . . Ngươi duẫn ta một chuyện, ta liền đi theo ngươi",
	"... Chuyện gì?",
	"Theo ta tại đây ngốc một hồi ba. Nhượng di động anh hảo hảo nhìn ta của hình dạng.",
	"... Ngươi. . .",
	"Ta yếu tha nhớ kỹ, trên đời này từng có một người, danh làn gió tụ."
};

--logic
function tb7thRoom:ClearNpc()
	ClearMapNpcWithName(self.tbBase.nMapId,"Định Hồn Thạch");
	ClearMapNpcWithName(self.tbBase.nMapId,"Thác Bạt Phù Anh");
	ClearMapNpcWithName(self.tbBase.nMapId,"Phong Tụ Cư Sĩ");
end
...
function tb7thRoom:StartRoom()
	...
	self:AddStarManager();
	self:AddUnFightBoss();
	self.tbBase:UpdateUiState("Biển lửa ngàn trọng quân chớ lưu\n\n<color=red>Xông qua biển lửa, quan bế cơ quan<color>");
end
...
function tb7thRoom:FailedRoom()
	...
	self:ClearRoom();
	self.tbBase:StartCurrentRoom();
	self.tbBase:AllBlackBoard("Các ngươi mê thất tại thần trùng của đại hỏa trung, tìm không được lối ra.");
end
...
function tb7thRoom:EndRoom()
	...
	self.tbBase:AllBlackBoard("Phong tụ hòa di động anh lưu tại hỏa hải lý, chỗ ngồi này tên là thần trùng của trấn nhỏ nếu không tằng xuất hiện quá.")
	self.tbBase:DropBox();	--加宝箱
	self:ClearRoom();
	self:RoomFinish();
end
