...
function tbBase:RoomFinish()
	...
	self:AddWeiwang();	--加uy danh giang hồ
--	self:FinishAchievement();	--完成Thành tựu
	if self.nCurrentRoomId == ChenChongZhen.MAX_ROOM_COUNT then
		...
		self:PlayerMsg("Phong tụ hòa di động anh lưu tại hỏa hải lý, chỗ ngồi này tên là Thần Trùng Trấn nhỏ nếu không tằng xuất hiện quá, sau đó gần rời khỏi!");
	end
	if self.nCurrentRoomId < ChenChongZhen.MAX_ROOM_COUNT then
...
--检测是否所有玩家死亡，如果死亡，当前关卡 thất bại, 所有玩家复活到重生点
...
function tbBase:OnDeath()
	...
	if self.nCurrentRoomId ~= 7 then
		self:BlackBoard(me,"Bạn đã bị trọng thương");
	else
		self:BlackBoard(me,"Ngươi mất đi dị thú che chở, đã bị hỏa hải đốt trọng thương");
	end
	local nIsFailed = self:CheckAllPlayerDeath();
	if nIsFailed == 1 then
		self.tbRoom[self.nCurrentRoomId]:FailedRoom();
...
function tbBase:ProcessTrap(szTrapName)
	if szTrapName == "trap_join" then
		if self:CheckAllPlayerLock() == 0 and self.nTransferRoomMaxId > 1 and me.GetTask(2191, 5) == 1 then
			Dialog:SendBlackBoardMsg(me, "Xin đợi trước mắt là trạm kiểm soát sau khi kết thúc, tái chiến đấu.");
			me.SetFightState(0);
...
function tbBase:GetRoom4ErrorLightCount()	--获取Đèn đốt sai的数量
	if self.nCurrentRoomId ~= 4 then
		return 0;
	end
...
function tbBase:UpdateEndUi()
	...
	local szMsg = "<color=green>Thời Gian Đóng PD Còn<color><color=white>%s<color>"
	for _,pPlayer in pairs(tbPlayer) do
		if pPlayer then
...
function tbBase:AddActive()
	...
	for _,pPlayer in pairs(tbPlayer) do
		if pPlayer then
--			SpecialEvent.ActiveGift:AddCounts(pPlayer,48);	--完成Thần Trùng Trấn活跃度
		end	
	end
end
...
function tbBase:FinishAchievement()
	...
	if nId then
		...
		for _,pPlayer in pairs(tbPlayer) do
			if pPlayer then
--				Achievement:FinishAchievement(pPlayer,nId);	--Thành tựu
			end
		end
	end	
end
