...
local tbQinshihuangBoss	= Npc:GetClass("boss_new_95_tcqgaming");
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "boss_new_95_tcqgaming");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "boss_new_95_tcqgaming");
			end
		end
	end
	...
	KDialog.MsgToGlobal(szThongBaoTheGioi1);	
		...
		local ngaunhiensoluong = MathRandom(1,5)
			    -- pPlayer.AddStackItem(18,1,2024,1,nil,1);
				-- local szThongBaoTheGioi = string.format("Người chơi <color=green>"..pPlayer.szName.."<color>Kết Liễu Boss Võ Lâm Cao Thủ<color=yellow> "..him.szName.." <color>nhận được <color=green> 1 Voucher Thần Long");
