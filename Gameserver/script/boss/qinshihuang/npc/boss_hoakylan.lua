...
local tbQinshihuangBoss	= Npc:GetClass("boss_hoakylan");
-- 对话事件
...
function tbQinshihuangBoss:DeathLoseItem(tbLoseItem)
	...
	local szMsg = "<color=green>Hỏa Kỳ Lân vong mạng rơi ra vật phẩm:<color>\n";
	local tbList = {};
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "boss_hoakylan");
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "boss_hoakylan");
			end
		end
	end
	...
	KDialog.MsgToGlobal("<color=red>[Anh Hùng]<color> tại <color=green>"..GetMapNameFormId(nMapId).." <color=yellow><color=Turquoise>"..pPlayer.szName.."<color> :Kết liễu  Hỏa Kỳ Lân Nhận Được:\n<color=green> 5 Tần Lăng Hòa Thị Ngọc\n50 Sách Kinh Nghiệm Chân Nguyên<color>");	

	local nNpcMapId, nNpcPosX, nNpcPosY = him.GetWorldPos();
		local nMoKhamNam = ConfigAll.OpenKham
	local nSoLuongRuongDa = ConfigAll.nSoRuong_KetLieu_HKL	
		if nMoKhamNam == 1 then 
		pPlayer.AddStackItem(18,1,1303,1,nil,nSoLuongRuongDa)
	KDialog.MsgToGlobal("<color=red>[Anh Hùng]<color> tại <color=green>"..GetMapNameFormId(nMapId).." <color=yellow><color=Turquoise>"..pPlayer.szName.."<color> :Kết liễu  Hỏa Kỳ Lân Nhận Được:\n<color=green>"..nSoLuongRuongDa.." Rương Ngọc Vỡ Cấp 1<color>");	
		end 
	pPlayer.AddStackItem(22,1,81,1,nil,5);--Tần lăng hòa thị ngọc
	pPlayer.AddStackItem(18,1,402,1,nil,50)---Chân Nguyên Tu Luyện Đơn Exp Đồng Hành
