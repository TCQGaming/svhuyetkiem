...
-- 秦始皇boss
local tbQinshihuangBoss	= Npc:GetClass("boss_sieubossxxxxx");
...
function tbQinshihuangBoss:DeathLoseItem(tbLoseItem)
	...
	local szMsg = "<color=green>Yến Thập Tam rơi vật phẩm:<color>\n";
	local tbList = {};
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	pPlayer.AddItem(18,1,1058,1);
	pPlayer.AddItem(18,1,1058,1);
	pPlayer.AddItem(18,1,1057,1); -- vi du HT 10
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "boss_sieubossxxxxx");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "boss_sieubossxxxxx");
			end
		end
	end
	...
	local szMsg = string.format("Tổ đội của <color=green>%s<color> đánh thắng Yến Thập Tam!!!", pPlayer.szName);
	KDialog.MsgToGlobal(szMsg);
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 80 then
		...
		if Boss.Qinshihuang:GetBossStep() == 0 then
			...
			local szMsg = "Tẩu quả nhập ma";
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Yến Thập Tam nói: "..szMsg);
	...
	elseif nLifePercent == 20 then
		...
		if Boss.Qinshihuang:GetBossStep() == 2 then
			...
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Yến Thập Tam: "..szMsg);
