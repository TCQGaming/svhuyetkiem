...
-- 秦始皇boss
local tbQinshihuangBoss	= Npc:GetClass("boss_bhd_dacbiet");
...
function tbQinshihuangBoss:DeathLoseItem(tbLoseItem)
	...
	local szMsg = "<color=green>Boss Bạch Hổ Đường Đặc Biệt rơi vật phẩm:<color>\n";
	local tbList = {};
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	pPlayer.AddStackItem(18,1,1,7,nil,10);------Huyền Tinh 7
	pPlayer.AddStackItem(18,1,1338,1,nil,2);------KNB Đại
	
	local nMoKhamNam = ConfigAll.OpenKham
	local nSoLuongRuongDa = ConfigAll.nSoRuong_Boss_BHD_DacBiet	
		if nMoKhamNam == 1 then 
			pPlayer.AddStackItem(18,1,1303,1,nil,nSoLuongRuongDa)
			local szThongBao = string.format("Tổ đội của <color=green>%s<color> Kết Liễu Thủ Lĩnh Sấm Đường Tặc Đặc Biệt nhận được "..nSoLuongRuongDa.." Rương Ngọc Vỡ Cấp 1", pPlayer.szName);
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "boss_bhd_dacbiet");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "boss_bhd_dacbiet");
			end
		end
	end
	...
	local szMsg = string.format("Tổ đội của <color=green>%s<color> kết liễu Thủ Lĩnh Sấm Đường Tặc Đặc Biệt!!!", pPlayer.szName);
	local szMsg1 = string.format("Tổ đội của <color=green>%s<color> 200 Kim Nguyên Bảo Và 10 Huyền Tinh Cấp 7", pPlayer.szName);
	KDialog.MsgToGlobal(szMsg);
	KDialog.MsgToGlobal(szMsg1);
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 80 then
		...
		if Boss.Qinshihuang:GetBossStep() == 0 then
			...
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Thủ Lĩnh nói: "..szMsg);
