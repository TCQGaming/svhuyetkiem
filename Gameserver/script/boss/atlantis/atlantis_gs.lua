...
Atlantis.HONOR_CLASS_MONEY		= 8;	
Atlantis.tbHonorLevelInfo	= {
	["money"]	= {
		nHonorId	= Atlantis.HONOR_CLASS_MONEY,
	},
};
...
function Atlantis:OpenRightUI(pPlayer, nRemainFrame)
	...
	local szMsg = string.format("\n<color=green>Đánh bại : <color=yellow>%s\n<color=green>Thành viên trong nhóm: <color=yellow>%s\n<color=green>Số rương bạn đã nhặt: <color=yellow>%s<color>\n", 0, 0, 0); 
...
function Atlantis:UpdateRightUI(pPlayer)
	...
	local szMsg = string.format("\n<color=green>Đánh bại: <color=yellow>%s\n<color=green>Thành viên trong nhóm: <color=yellow>%s<color>\n<color=green>Số rương bạn đã nhặt: <color=yellow>%s\n", nKillCount, nMemberCount , nChipCount);
	Dialog:SendBattleMsg(pPlayer, szMsg, 1);
end
...
function Atlantis:PlayerLostChip(pPlayer, pReceiver, szType)
	...
	for i, tbInfo in pairs(self.ITEM_CHIP_ID) do
		...
		if nLost > 0 then
			...
			if nRet ~= 0 then
				Dbg:WriteLog("Lâu Lan Cổ Thành ", "Atlantis", pPlayer.szAccount, pPlayer.szName, string.format("Trừ %s của %s", nLost, tbInfo.szName));
			end
			self:SendMessage(pPlayer, self.MSG_CHANNEL, string.format("Bạn bị mất <color=yellow>%s %s<color>", nLost, tbInfo.szName));
			...
			Dbg:WriteLog("Atlantis", "Lâu Lan Cổ Thành ", pPlayer.szAccount, pPlayer.szName, string.format("掉落了%s个%s", nLost, tbInfo.szName));
...
function Atlantis:PlayerLostEquip(pPlayer, szType)
	...
	if nFlag > 0 then
		if szType == "drop" then
			...
			if pNpc then
				local szMsg = "<color=yellow>Có một vật phát sáng, mau mau đi tìm nó!<color>";
...
function Atlantis:CheckGetEquip(pPlayer)
	...
	if tbTeam.nSuperTeam == 1 then
		self:SendMessage(pPlayer, self.MSG_MIDDLE, "Trong đội đã có người sở hữu Thần Khí!");
		return 0;
	end
	
	local nNeed = 2;
	if pPlayer.CountFreeBagCell() < nNeed then
		self:SendMessage(pPlayer, self.MSG_MIDDLE, string.format("Xin vui lòng chưa hành trang %s ô!", nNeed));
		return 0;
	end
...
function Atlantis:CheckCanEnter()
	...
	if self:CheckIsOpen() ~= 1 or self._Open ~= 1 then
		Dialog:Say(string.format("    Xa phu ta thuê hôm qua bây giờ vẫn chưa thấy tới, ngươi hãy quay lại sau khi mở server <color=yellow>%s ngày<color>, vào lúc <color=yellow>16:00 - 19:00<color>!", Atlantis.OPEN_DAY));
		return 0;
	end
	]]
	...
	if me.nLevel < self.MIN_LEVEL then
		Dialog:Say(string.format("Lâu lan cổ thành là nơi vô cùng nguy hiểm - đẳng cấp %s hiện tại không thể vào được", self.MIN_LEVEL));
		return 0;
	end
	
	-- 门派限制
	if me.nFaction <= 0 then
		Dialog:Say("Lâu lan cổ thành là nơi vô cùng nguy hiểm - Gia nhập môn phái rèn luyện võ công rồi hãy quay lại");
		return 0;
	end
	
	-- 混天披风
	if self:GetMantleLevel(me) < self.MIN_MANTLE then
		Dialog:Say("Lâu lan cổ thành là nơi vô cùng nguy hiểm - Hãy mang Phi Phong và cùng đồng đội khám phá");
		return 0;
	end
	...
	if nUseTime >= self.MAX_TIME then
		Dialog:Say("Thời gian Lâu Lan đã hết! Ra ngoài luyện tập mai hãy quay lại");
		return 0;
	end
...
function Atlantis:PlayerEnter()
	...
	if (nCurTime >= 1100 and nCurTime < 1154) or (nCurTime >= 1700 and nCurTime < 1754) or (nCurTime >= 2100 and nCurTime < 2200) then
		Dialog:Say("Không thể vào lâu lan trong thời gian này")
		return 0;
	end
	...
	if nHonorValue < ConfigAll.GioiHanTaiPhu_LauLan then
			Dialog:Say("Bạn không đạt đủ <color=yellow>"..ConfigAll.GioiHanTaiPhu_LauLan.." <color>điểm Tài Phú để tham gia lâu lan cổ thành \nTài Phú Hiện Nay<color=yellow> "..nHonorValue.."<color>");
			return 0;
	end
end
...
--	local bGreenServer = KGblTask.SCGetDbTaskInt(DBTASK_TIMEFRAME_OPEN);
	...
	if me.nLevel < 90 then
		Dialog:Say("Cấp 90 rồi quay lại gặp ta");
		return 0;
	end
	-- 是否队长
	...
	if not tbMemberId then
		Dialog:Say("Thành viên không ở gần đây. ");
		return 0;
	end
	...
	for _, nMemberId in pairs(tbMemberId) do
		...
		if pMember then
			...
			if self:CheckCanEnter() ~= 1 then
				KTeam.Msg2Team(pMember.nTeamId, string.format("[%s]Không thể vào Lâu La. Sau khi rời nhóm ", pMember.szName));
				nFlag = 1;
			end
	...
	if nMemberCount + nMapCount > self.MAX_PLAYER then
		Dialog:Say("    Số lượng đội vào Lâu La đã quá nhiều ! Không thể tiếp nhận thêm");
		return 0;
	end
...
--	pPlayer.UpdateKinTongTitle();
	...
	pPlayer.AddSpeTitle("Lâu Lan Cổ Thành", GetTime() + 2 * 60 * 60 * 24, "orange");
	
--	Partner:SetForbitOut(pPlayer, 1);				-- 隐藏同伴
	
	local szMsg = "Vùng đất an toàn - Lâu Lan Cổ Thành!";
	...
	-- Dbg:WriteLog("Atlantis", "Lâu Lan Cổ Thành ", pPlayer.szAccount, pPlayer.szName, "đi vào Lâu Lan Cổ Thành ", string.format("Phi Phong %s", pPlayer.GetHonorLevel()));
...
--	pPlayer.UpdateKinTongTitle();
	...
	pPlayer.RemoveSpeTitle("Lâu Lan Cổ Thành");
...
--	end
	...
	local nUseTime = pPlayer.GetTask(self.TASK_GID, self.TASK_USE_TIME);
	-- Dbg:WriteLog("Atlantis", "Lâu Lan Cổ Thành ", pPlayer.szAccount, pPlayer.szName, "đi chỗ khác Lâu Lan Cổ Thành ", string.format("Cấp Phi Phong: %s", pPlayer.GetHonorLevel()), string.format("Thời gian tích lũy trong ngày: %s", nUseTime));
			...
			elseif pPlayer.GetCamp() ~= 6 then
				...
				-- 时间不足提示
					...
					if self.MAX_TIME - nTime < 300 then
						self:SendMessage(pPlayer, self.MSG_BOTTOM, "Thời gian còn lại quá ít - mau chóng rời khỏi. ");
					end
	...
	for i, tbPos in ipairs(self.MAP_MOVER_POS) do
		if MathRandom(1, 10000) <= self.MOVER_RATE and self.nMoverCount < self.MAX_MOVER then
			...
			if pNpc then
				...
				--local szMsg = string.format("%s trốn nhanh không bọn hắn tìm được", pNpc.szName);
				--self:BroadCast(self.MSG_BOTTOM, szMsg);
				--self:BroadCast(self.MSG_CHANNEL, szMsg);
			end
		end
...
function Atlantis:TimerBoss()	
	if not self.tbBoss.nBossDwId and MathRandom(1, 10000) <= self.BOSS_RATE and self.tbBoss.nTotal < self.MAX_DAY_BOSS then
		...
		if pNpc then
			...
			local szMsg = string.format("%s đã xuất hiện, hãy đi tìm nó!", pNpc.szName);
			self:BroadCast(self.MSG_BOTTOM, szMsg);
			self:BroadCast(self.MSG_CHANNEL, szMsg);
		end
	end
...
function Atlantis:FreshBoss_GS()
	...
	GlobalExcute({"Dialog:GlobalNewsMsg_GS","<color=yellow><color=pink>BOSS LÂU LAN SẮP XUẤT HIỆN <color>"});
	KDialog.MsgToGlobal("<color=yellow><color=pink>BOSS LÂU LAN SẮP XUẤT HIỆN<<color><color>");
end
...
function Atlantis:TimerFreshBoss()
	...
	local pNpc = KNpc.Add2(self.NPC_BOSS_ID, self.NPC_LEVEL, -1, tbPos[1], tbPos[2], tbPos[3]);
	if pNpc then
		local szMsg = string.format("%s đã xuất hiện rồi!", pNpc.szName);
		self:BroadCast(self.MSG_BOTTOM, szMsg);
		self:BroadCast(self.MSG_CHANNEL, szMsg);
		pNpc.SetLoseItemCallBack(1);
	end
...
function Atlantis:DeathLoseItem(tbLoseItem)
	
	local tbItem = tbLoseItem.Item;
	local szMsg = "<color=green>Boss Lâu Lan Rơi Vật Phẩm:<color>\n";
	local tbList = {};
...
function Atlantis:OnMoverDeath(pMover)
	...
	if nRand <= self.EQUIP_RATE and self.nEquipCount < self.MAX_EQUIP then
		...
		local pNpc = KNpc.Add2(self.NPC_EQUIP_ID, self.NPC_LEVEL, -1, nMapId, nMapX, nMapY);
		if pNpc then
			local szMsg = "<color=yellow>Một vật đang phát sáng - mau mau đi tìm nó!<color>";
			self:BroadCast(self.MSG_BOTTOM, szMsg);
			self:BroadCast(self.MSG_CHANNEL, szMsg);
			self.nEquipCount = self.nEquipCount + 1;
...
function Atlantis:ServerDailyEvent_GS()
	...
	Dbg:WriteLog("Atlantis", "Lâu Lan Cổ Thành ", string.format("Thời gian: %s, tổng số lần rơi: %s", GetLocalDate("%Y%m%d"), self._nTotalDropTimes or 0));
end
...
function Atlantis:OpenSystem_GS()
	...
	KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, "Lâu Lau cổ thành xuất hiện dị biến, xung quanh không ngừng tỏa ra ánh sáng huyền bí . Mau mau về Phượng tường tim Yến Tiểu Lâu. ");
...
function Atlantis:CloseSystem_GS()
	...
	KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, "Lâu Lan Cổ Thành đã hết thời gian báo danh. ");
...
function Atlantis:OnPlayerDeath(pKillerNpc)
	...
	if pKillerPlayer then
		...
		self:SendMessage(pKillerPlayer, self.MSG_MIDDLE, string.format("Người chơi <color=yellow>%s<color> đã trọng thương", me.szName));
		self:SendMessage(pKillerPlayer, self.MSG_CHANNEL, string.format(" Sau khi <color=yellow>%s<color> lột bỏ được mặt nạ người thần bí đã biết sự thật", me.szName));
		KTeam.Msg2Team(pKillerPlayer.nTeamId, string.format("Người Thần Bí đánh [%s] trọng thương!", pKillerPlayer.szName, me.szName));
		KTeam.Msg2Team(me.nTeamId, string.format("Ngươi chơi [%s] đã bị đánh trọng thương đưa về doanh trại", me.szName));
		self:SendMessage(me, self.MSG_BOTTOM, "Sau khi được những phụ tá của lang trung mông cổ chữa trị bạn đã khôi phục!");
		
		Dbg:WriteLog("Atlantis", "Lâu Lan Cổ Thành ", 
			string.format("Kẻ giết người: %s, cấp áo choàng: %s, võ công: %s, lộ trình: %s", pKillerPlayer.szName, self:GetMantleLevel(pKillerPlayer), pKillerPlayer.nFaction, pKillerPlayer.nRouteId),
			string.format("Bị giết: %s, cấp áo choàng: %s, võ công: %s, lộ trình: %s", me.szName, self:GetMantleLevel(me), me.nFaction, me.nRouteId)
		);
...
function Atlantis:_Test()
	me.Msg("Bản đồ số :"..self:GetMapPlayerCount());
	me.Msg("Maps đội:"..Lib:CountTB(self.tbTeamList));
	me.Msg("Rớt Vật Phẩm:"..self:GetCurDropTimes());
	me.Msg("Trang Bị:"..self.nEquipCount);
	me.Msg("Số lần di chuyển:"..self.nMoverCount);
end

function Atlantis:DoUpdateQinBoss(nTemplateId, nMapId, nMapX, nMapY)
