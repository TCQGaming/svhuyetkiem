...
function tbTrap:OnPlayer()
	if self.nMapX and self.nMapY and self.nFightState then
		if self.nFightState == 1 then
			...
			Atlantis:SendMessage(me, Atlantis.MSG_BOTTOM, "Ngươi đã bước vào Vùng Đất Hoang Mạc - Lâu Lan Cổ Thành!");
			Atlantis:SendMessage(me, Atlantis.MSG_CHANNEL, "Ngươi đã bước vào Vùng Đất Hoang Mạc - Lâu Lan Cổ Thành!");
		else
			if Atlantis:CheckHaveSuper(me.szName) == 1 then
				Atlantis:SendMessage(me, Atlantis.MSG_MIDDLE, "Thần Khí đang mang trên người không thể vào trong!");
				return 0;
			end
			Atlantis:SendMessage(me, Atlantis.MSG_BOTTOM, "Đã trở lại khu An Toàn!");
