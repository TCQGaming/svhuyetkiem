...
-- 秦始皇boss
local tbQinshihuangBoss	= Npc:GetClass("boss_laulan");
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "boss_laulan");
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "boss_laulan");
			end
		end
	end
	...
	-- local szMsg = string.format("Tổ đội của <color=green>%s<color> tại <color=green>"..GetMapNameFormId(nMapId).."<color>:Kết liễu  Hỏa Kỳ Lân Nhận Thêm 5 Hòa Thị Bích<color>", pPlayer.szName);
	-- GlobalExcute({"Dialog:GlobalNewsMsg_GS","<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>:Kết liễu  Hỏa Kỳ Lân Thêm 5 Hòa Thị Bích<color>"});
	-- KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>:Kết liễu  Hỏa Kỳ Lân Thêm 5 Hòa Thị Bích<color>");
	-- KDialog.MsgToGlobal("<color=red>[Anh Hùng]<color> <color=yellow><color=Turquoise>"..pPlayer.szName.."<color>:Kết liễu  Hỏa Kỳ Lân <color>");	
	-- KDialog.NewsMsg(0, Env.NEWSMSG_COUNT, szMsg);
	-- self:BroadCast(szMsg);
	-- local nNpcMapId, nNpcPosX, nNpcPosY = him.GetWorldPos();
	 -- 20 Thanh Long Bí Bảo Bảo Hạp Hồn
	-- pPlayer.AddExp(100000000) -- 200tr EXP
	-- pPlayer.AddStackItem(18,1,377,1,nil,5);--Tần lăng hòa thị bíc
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 80 then
		...
		if Boss.Qinshihuang:GetBossStep() == 0 then
			...
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Hỏa Kỳ Lân nói: "..szMsg);
