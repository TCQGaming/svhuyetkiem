...
function tbNpc:OnDialog()
	...
	GeneralProcess:StartProcess("Đang nhặt...", 10 * Env.GAME_FPS, {self.OnGetEquip, self, him.dwId, me.nId}, {self.OnBreak, self, me.nId}, tbBreakEvent);

--	me.AddSkillState(Newland.THRONE_BUFFER, 1, 1, 10 * Env.GAME_FPS, 1, 1);
end
--script\boss\atlantis\npc\atlantis_npc_equip.lua
...
--	print("pPlayer.nFaction : "..pPlayer.nFaction.."");
	-- if tbItem then
		...
		-- if pItem then
			...
			-- Atlantis:SendMessage(Atlantis.MSG_BOTTOM, "<color=yellow>Thần Khí Lâu Lan Cổ Thành!<color>");
			-- Atlantis:BroadCast(Atlantis.MSG_CHANNEL, "<color=yellow>Nhà ngươi đã được chọn!<color>");
			-- KTeam.Msg2Team(pPlayer.nTeamId, string.format("[%s] nhận được Thần Khí Lâu Lan!", pPlayer.szName));
