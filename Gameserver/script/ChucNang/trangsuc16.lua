local tbItem	= Item:GetClass("chtrangsuc");
tbItem.nNumCuong = 16;
function tbItem:OnUse()
	...
	local szMsg = "<color=red>TRANG BỊ ĐÃ ĐƯỢC LUYỆN HÓA MỚI DÙNG ĐC CHỨC NĂNG NÀY<color>";
	Dialog:OpenGift(szMsg, nil, {self.CuongHoa, self});
end

function tbItem:CuongHoa(tbItemObj)
	local nCount = me.GetItemCountInBags(18,1,20402,self.nNumCuong)
	if nCount < 1 then
    ...
    for i = 1, #tbItemObj do 
        ...
        if szKey ~= nCheck and szKey ~= nCheckk and szKey ~= nCheckkk then 
        ...
        Dialog:Say("Loại vật phẩm này chỉ dùng để cường hóa Trang sức", {"Kết thúc đối thoại !"}); 
    	return 0;
        end 

    	local nLevel = CheckItem.nLevel;
	    if (nLevel < 10) or (CheckItem.nRefineLevel < 1) then-- 炼化1级的才能强16
		Dialog:Say("PHẢI LUYỆN HÓA TRANG BỊ hoặc Không thể cường hóa vượt cấp cho phép cường hóa của trang bị", {"Kết thúc đối thoại !"}); 
		return 0;
    	end
    end
	...
	me.ConsumeItemInBags(1, 18, 1, 20402, self.nNumCuong);
	pEquip.Bind(1);
	Spreader:OnItemBound(pEquip);
	--local szHistory = "\\script\\iDoctor\\BuaCuongHoa\\LichSuCuongHoa.txt";
    --KFile.AppendFile(szHistory, ""..me.szName.."\t"..pEquip.szName.."\t"..nEnhanceMax.."\t"..nTonch.."\n");
	end
end
