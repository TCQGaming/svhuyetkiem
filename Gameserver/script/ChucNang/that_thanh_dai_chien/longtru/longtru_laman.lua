...
tbTruLA.nGouhuoSkillId		= 377;	
-- Hàm xóa Long Trụ cũ
function tbTruLA:RemoveOldNpc()
	ClearMapNpc(2165);
end

function tbTruLA:OnDeath_Boss(nTongId,pNpc)
if not pNpc then 
CTC_7_Thanh:CreatNPC_LA()
return 1;
end 
if not nTongId then 
CTC_7_Thanh:CreatNPC_LA()
...
KGblTask.SCSetDbTaskInt(DBTASD_CongThanhTruLamAn, nTaskValue)
	    local nThoiGianDemNguoc_BatTu

	if  him.nTemplateId == 10397 then -- Long Trụ Lâm An
		logFilePath = "\\script\\ChucNang\\that_thanh_dai_chien\\longtruchiem\\longtrulaman.txt"
		CTC_7_Thanh.LongTru_LamAn = szTongName;
		CTC_7_Thanh.StartTime_LongTru_LA = startTime
		nThoiGianDemNguoc_BatTu = CTC_7_Thanh.TimeBatTu -- Đặt thời gian bất tử riêng cho Long Trụ 
	end		

	local pNpc2 = KNpc.Add2(him.nTemplateId,him.nLevel,-1,2165, 1631, 3947, 0, 0, 1);
	pNpc2.SetVirtualRelation(Npc.emNPCVRELATIONTYPE_TONE, pPlayer.dwTongId);
	Npc:RegPNpcOnDeath(pNpc2, tbTruLA.OnDeath_Boss, self,pPlayer.dwTongId);
    ...
    local startTime = GetTime()
    local dateInfo = os.date("*t", startTime)
	local nTaskSaved = KGblTask.SCGetDbTaskInt(DBTASD_CongThanhTruLamAn)
	...
	nTinhToan = CTC_7_Thanh.TimeBatTu
				...
				if nTinhToan == CTC_7_Thanh.TimeBatTu then 
				tbTruLA:SetImmortalStatus(szTongName)
				end 
...
function tbTruLA:SetImmortalStatus(szTongName)
    self:RemoveOldNpc()
    self:AddImmortalNpc(szTongName)
		KGblTask.SCSetDbTaskInt(DBTASD_CongThanhTruLamAn, 0)
end

function tbTruLA:AddImmortalNpc(szTongName)
	local pNpc2 = KNpc.Add2(10402, 200, -1, 2165, 1631, 3947);
	pNpc2.SetTitle ( "<color=green>Bang Hội<color><color=gold> "..szTongName.." chiếm lĩnh<color>");
	pNpc2.SetCurCamp(6)    
			local szMsg = "Long Trụ Lâm An Đã Bất Tử Bang Hội Chiếm Lĩnh <color=yellow>" .. szTongName .. "<color>"
		KDialog.MsgToGlobal(szMsg)
	   return pNpc2.dwId
end


function tbTruLA:Restore_Life(nNpcId,nTongId)
...
function tbTruLA:OnNpcTimer(nNpcId,nTongId)
	local pNpc = KNpc.GetById(nNpcId);
	if not pNpc then
		return 0
	end
...
function tbTruLA:AddAroundPlayersExp(nNpcId,nTongId)
	local pNpc = KNpc.GetById(nNpcId);
	if (not pNpc) then
		return 0;
	end
...
function tbTruLA:AddExp2Player(nPlayerId, nNpcId)
