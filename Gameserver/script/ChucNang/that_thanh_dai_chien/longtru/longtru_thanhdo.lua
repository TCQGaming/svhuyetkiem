...
tbTruThanhDo.nGouhuoSkillId		= 377;	
-- Hàm xóa Long Trụ cũ
function tbTruThanhDo:RemoveOldNpc()
	ClearMapNpc(2167);
end

function tbTruThanhDo:OnDeath_Boss(nTongId,pNpc)
if not pNpc then 
...
KGblTask.SCSetDbTaskInt(DBTASD_CongThanhTruThanhDo, nTaskValue)
	    local nThoiGianDemNguoc_BatTu

	if  him.nTemplateId == 10398 then -- Long Trụ Thành Đô
		logFilePath = "\\script\\ChucNang\\that_thanh_dai_chien\\longtruchiem\\longtruthanhdo.txt"
		CTC_7_Thanh.LongTru_ThanhDo = szTongName;		
		CTC_7_Thanh.StartTime_LongTru_ThanhDo = startTime
		nThoiGianDemNguoc_BatTu = CTC_7_Thanh.TimeBatTu -- Đặt thời gian bất tử riêng cho Long Trụ 
	end		

	local pNpc2 = KNpc.Add2(him.nTemplateId,him.nLevel,-1,2167, 1621,3221, 0, 0, 1);
	pNpc2.SetVirtualRelation(Npc.emNPCVRELATIONTYPE_TONE, pPlayer.dwTongId);
	Npc:RegPNpcOnDeath(pNpc2, tbTruThanhDo.OnDeath_Boss, self,pPlayer.dwTongId);
    ...
    local startTime = GetTime()
    local dateInfo = os.date("*t", startTime)
	local nTaskSaved = KGblTask.SCGetDbTaskInt(DBTASD_CongThanhTruThanhDo)
	...
	nTinhToan = CTC_7_Thanh.TimeBatTu
				...
				if nTinhToan == CTC_7_Thanh.TimeBatTu then 
				tbTruThanhDo:SetImmortalStatus(szTongName)
				end 
...
function tbTruThanhDo:SetImmortalStatus(szTongName)
    self:RemoveOldNpc()
    self:AddImmortalNpc(szTongName)
		KGblTask.SCSetDbTaskInt(DBTASD_CongThanhTruThanhDo, 0)
end

function tbTruThanhDo:AddImmortalNpc(szTongName)
	local pNpc2 = KNpc.Add2(10402, 200, -1, 2167, 1621,3221);
	pNpc2.SetTitle ( "<color=green>Bang Hội<color><color=gold> "..szTongName.." chiếm lĩnh<color>");
	pNpc2.SetCurCamp(6)    
			local szMsg = "Long Trụ Thành Đô Đã Bất Tử Bang Hội Chiếm Lĩnh <color=yellow>" .. szTongName .. "<color>"
		KDialog.MsgToGlobal(szMsg)
	   return pNpc2.dwId
end


function tbTruThanhDo:Restore_Life(nNpcId,nTongId)
...
function tbTruThanhDo:OnNpcTimer(nNpcId,nTongId)
	local pNpc = KNpc.GetById(nNpcId);
	if not pNpc then
		return 0
	end
...
function tbTruThanhDo:AddAroundPlayersExp(nNpcId,nTongId)
	local pNpc = KNpc.GetById(nNpcId);
	if (not pNpc) then
		return 0;
	end
...
function tbTruThanhDo:AddExp2Player(nPlayerId, nNpcId)
