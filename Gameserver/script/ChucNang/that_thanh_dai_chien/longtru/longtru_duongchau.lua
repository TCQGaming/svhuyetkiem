...
tbTruDC.nGouhuoSkillId		= 377;	
-- Hàm xóa Long Trụ cũ
function tbTruDC:RemoveOldNpc()
	ClearMapNpc(2164);
end

function tbTruDC:OnDeath_Boss(nTongId,pNpc)
if not pNpc then 
...
KGblTask.SCSetDbTaskInt(DBTASD_CongThanhTruDuongChau, nTaskValue)
	    local nThoiGianDemNguoc_BatTu

	if  him.nTemplateId == 10399 then -- Long Trụ Dương Châu
		logFilePath = "\\script\\ChucNang\\that_thanh_dai_chien\\longtruchiem\\longtruduongchau.txt"
		CTC_7_Thanh.LongTru_DuongChau = szTongName;	
		CTC_7_Thanh.StartTime_LongTru_DC = startTime		 
		nThoiGianDemNguoc_BatTu = CTC_7_Thanh.TimeBatTu -- Đặt thời gian bất tử riêng cho Long Trụ 
	end		

	local pNpc2 = KNpc.Add2(him.nTemplateId,him.nLevel,-1,2164, 1593,3188, 0, 0, 1);
	pNpc2.SetVirtualRelation(Npc.emNPCVRELATIONTYPE_TONE, pPlayer.dwTongId);
	Npc:RegPNpcOnDeath(pNpc2, tbTruDC.OnDeath_Boss, self,pPlayer.dwTongId);
    ...
    local startTime = GetTime()
    local dateInfo = os.date("*t", startTime)
	local nTaskSaved = KGblTask.SCGetDbTaskInt(DBTASD_CongThanhTruDuongChau)
	...
	nTinhToan = CTC_7_Thanh.TimeBatTu
				...
				if nTinhToan == CTC_7_Thanh.TimeBatTu then 
				tbTruDC:SetImmortalStatus(szTongName)
				end 
...
function tbTruDC:SetImmortalStatus(szTongName)
    self:RemoveOldNpc()
    self:AddImmortalNpc(szTongName)
		KGblTask.SCSetDbTaskInt(DBTASD_CongThanhTruDuongChau, 0)
end

function tbTruDC:AddImmortalNpc(szTongName)
	local pNpc2 = KNpc.Add2(10402, 200, -1, 2164, 1593,3188);
	pNpc2.SetTitle ( "<color=green>Bang Hội<color><color=gold> "..szTongName.." chiếm lĩnh<color>");
	pNpc2.SetCurCamp(6)    
			local szMsg = "Long Trụ Dương Châu Đã Bất Tử Bang Hội Chiếm Lĩnh <color=yellow>" .. szTongName .. "<color>"
		KDialog.MsgToGlobal(szMsg)
	   return pNpc2.dwId
end


function tbTruDC:Restore_Life(nNpcId,nTongId)
...
function tbTruDC:OnNpcTimer(nNpcId,nTongId)
	local pNpc = KNpc.GetById(nNpcId);
	if not pNpc then
		return 0
	end
...
function tbTruDC:AddAroundPlayersExp(nNpcId,nTongId)
	local pNpc = KNpc.GetById(nNpcId);
	if (not pNpc) then
		return 0;
	end
...
function tbTruDC:AddExp2Player(nPlayerId, nNpcId)
