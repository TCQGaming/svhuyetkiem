...
local KnbBiz = Item:GetClass("ruongchiem_la");

function KnbBiz:OnUse()	
-- DoScript("\\script\\ChucNang\\that_thanh_dai_chien\\item\\ruongchiemlaman.lua");
	local nNeedBag = 3;
	...
	local pItem = 		me.AddItem(1,25,113,1)
	local pItem1 = 		me.AddItem(1,26,113,1)
			...
			local pItem = 		me.AddItem(1,25,112,1)
			local pItem1 = 		me.AddItem(1,26,112,1)
			pItem.SetCustom(Item.CUSTOM_TYPE_MAKER, nName);		
			pItem.Sync();	
			pItem1.SetCustom(Item.CUSTOM_TYPE_MAKER, nName);		
			pItem1.Sync();	
