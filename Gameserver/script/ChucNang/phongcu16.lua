local tbItem	= Item:GetClass("chphongcu");
tbItem.nNumCuong = 16;
function tbItem:OnUse()

	if me.CountFreeBagCell() < 3 then
	Dialog:Say("Hãy chú ý sắp xếp hành trang có đủ 3 ô trống");
	return 0;
	end	

	local szMsg = "<color=red>TRANG BỊ ĐÃ ĐƯỢC LUYỆN HÓA MỚI DÙNG ĐC CHỨC NĂNG NÀY<color>";
	Dialog:OpenGift(szMsg, nil, {self.CuongHoa, self});
end

function tbItem:CuongHoa(tbItemObj)
	local nCount = me.GetItemCountInBags(18,1,20401,self.nNumCuong)
	if nCount < 1 then
    	Dialog:Say("Đừng gian lận trong trò chơi, hãy thể hiện bạn là người chơi nghiêm túc");
		return 0;
	end

	local tbItemInfo = {bForceBind=1,};
	local tbItemList	= {};
	local nCount = 0; 
    for i = 1, #tbItemObj do 
        nCount = nCount + tbItemObj[i][1].nCount; 
        local CheckItem = tbItemObj[i][1]; 
        local nCheck = "2";
        local nCheckk = "4";
        local nCheckkk = "1";
        local szKey = string.format("%s",CheckItem.nGenre);
        if szKey ~= nCheck and szKey ~= nCheckk and szKey ~= nCheckkk then 
        Dialog:Say("Vật phẩm bỏ vào không phải trang bị", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 

        local szLoai = string.format("%s",CheckItem.nDetail);
	    if szLoai ~= "3" and szLoai ~= "7" and szLoai ~= "8" and szLoai ~= "9" and szLoai ~= "10" then
        Dialog:Say("Loại vật phẩm này chỉ dùng để cường hóa Phòng cụ", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 

		local nLevel = CheckItem.nLevel;
		if (nLevel < 10) or (CheckItem.nRefineLevel < 1) then-- 炼化1级的才能强16
		Dialog:Say("PHẢI LUYỆN HÓA TRANG BỊ hoặc Không thể cường hóa vượt cấp cho phép cường hóa của trang bị", {"Kết thúc đối thoại !"}); 
		return 0;
		end
    end

    if nCount ~= 1 then 
        Dialog:Say("Chỉ được đặt vào 1 trang bị", {"Ta biết rồi !"}); 
        return 0; 
    end
    for i = 1, #tbItemObj do 
    local pEquip = tbItemObj[i][1]; 
    local nEnhanceMax = self.nNumCuong;
	local nRet = pEquip.Regenerate(
		pEquip.nGenre,
		pEquip.nDetail,
		pEquip.nParticular,
		pEquip.nLevel,
		pEquip.nSeries,
		nEnhanceMax,			-- 强化次数加一
		pEquip.nLucky,
		pEquip.GetGenInfo(),
		0,
		pEquip.dwRandSeed,
		0
	);
	me.ConsumeItemInBags(1, 18, 1, 20401, self.nNumCuong);
	pEquip.Bind(1);
	Spreader:OnItemBound(pEquip);
	--local szHistory = "\\script\\iDoctor\\BuaCuongHoa\\LichSuCuongHoa.txt";
    --KFile.AppendFile(szHistory, ""..me.szName.."\t"..pEquip.szName.."\t"..nEnhanceMax.."\t"..nTonch.."\n");
	end
end
