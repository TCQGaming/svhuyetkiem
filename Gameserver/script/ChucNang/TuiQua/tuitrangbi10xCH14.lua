local tbItem = Item:GetClass("tuitrangbi10xCH14"); 
tbItem.nType = 12;


function tbItem:OnDialog()
			if me.GetTask(8009,1) > 0 then
	...
	Dialog:Say("Trang bị hỗ trợ tân thủ.\n<color=red>Mỗi nhân vật chỉ được nhận một lần duy nhất nên hãy cẩn thận<color>", {
			{"Lựa chọn môn phái", self.GetEquipFaction, self, 1, 14},
			{"Kết thúc đối thoại"},
	});
...
function tbItem:GetEquip(nFactionId, nRouteId, nQiangHua)
	...
	me.SetTask(8009,1,1);
--	me.AddStackItem(18	,1	,244,	1,{bForceBind = 1},Config.LevelUp[tbItem.nType][3]);
--	me.AddStackItem(18,1,1350,1,{bForceBind = 1},Config.LevelUp[tbItem.nType][4]);
--	me.ChangeCurMakePoint(Config.LevelUp[tbItem.nType][5]);
--	me.ChangeCurGatherPoint(Config.LevelUp[tbItem.nType][5]);
--	me.Earn(Config.LevelUp[tbItem.nType][6],0);
--	me.AddBindMoney(Config.LevelUp[tbItem.nType][7]);
--	me.AddBindCoin(Config.LevelUp[tbItem.nType][8]);
