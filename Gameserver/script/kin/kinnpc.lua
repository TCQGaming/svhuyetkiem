...
local szMsg = "Vì những đóng góp xuất sắc mà bạn đã làm cho gia tộc, bạn sẽ được khen thưởng cho công lao của mình hàng tuần, vì vậy hãy hành động nhanh chóng.\nNhân tiện, bạn đến đây để làm gì?";
	Dialog:Say(szMsg,
		{
{"Sửa đổi mức mục tiêu của các hoạt động trong tuần gia tộc", self.DlgModifyTaskLevel, self},
{"Nhận phần thưởng của mục tiêu hoạt động tuần gia tộc", self.DlgGetWeeklyActionAward, self, 0},
		});
end

-- 修改家族周活动目标等级
function Kin:DlgModifyTaskLevel()
	local szMsg = "Mục tiêu hoạt động trong tuần dành cho gia tộc cơ sở phù hợp với lựa chọn của gia tộc với cấp độ trung bình từ 50-79; mục tiêu hoạt động trong tuần ở cấp độ trung cấp phù hợp với lựa chọn của gia đình với cấp độ trung bình trong khoảng 80-89; mục tiêu hoạt động trong tuần dành cho gia đình nâng cao phù hợp với gia đình các lựa chọn có mức trung bình cao hơn lựa chọn Gia tộc ở mức 90." ..
"Tất nhiên, khi cấp độ tăng lên, phần thưởng cũng sẽ tăng theo. Bạn có muốn sửa đổi nó bây giờ không? Nếu nó được sửa đổi, nó sẽ được thực hiện từ tuần sau.";
	Dialog:Say(szMsg,
		{

{"Sửa đổi làm mục tiêu hoạt động chính trong tuần dành cho Gia Tộc", self.ModifyLevel, self, Kin.TASK_LEVEL_LOW},
{"Sửa đổi thành Mục tiêu Hoạt động trong Tuần dành cho Gia Tộc Trung cấp", self.ModifyLevel, self, Kin.TASK_LEVEL_MID},
{"Sửa đổi thành mục tiêu hoạt động trong tuần dành cho Gia Tộc cấp cao", self.ModifyLevel, self, Kin.TASK_LEVEL_HIGH},
		});
end
...
Dialog:Say("Bạn không có quyền sửa đổi nội dung này hoặc quyền đã bị khóa, xin hãy để tộc trưởng hoặc phó tộc trưởng của bạn đến.");
		return 0;
	end 
...
szMsg = szMsg.. "Hiện tại máy chủ này chưa mở giới hạn cấp 89, không thể đổi cấp mục tiêu của sự kiện hàng tuần thành trung cấp hoặc cao cấp.";
	elseif (0 == TimeFrame:GetState("OpenLevel99") and nTaskLevel == Kin.TASK_LEVEL_HIGH) then
szMsg = szMsg.. "Hiện máy chủ này chưa mở giới hạn trên cấp 99, và mục tiêu cấp sự kiện hàng tuần không thể thay đổi thành cao cấp.";
	end
	if ("" ~= szMsg) then
		Dialog:Say(szMsg);
		return 0;
...
szLevel = szLevel .. "Trung cấp";
	...
	szMsg = "Mức mục tiêu sự kiện hàng tuần của gia đình bạn đã được thay đổi thành công thành <color=yellow>" .. szLevel .. "<color>， sẽ có hiệu lực vào tuần tới.";
	Dialog:Say(szMsg);
end
-- 家族更换阵营
function Kin:DlgChangeCamp(nCamp)
	...
	if nRet ~= 1 then
		Dialog:Say("Chỉ tộc trưởng mới có thể thay đổi trại gia tộc");
		return 0;
	end
	if cKin.GetBelongTong() ~= 0 then
Dialog:Say("Gia tộc đang ở trong bang hội và trại gia đình không thể thay đổi!");
		return 0;
	end
	local nDate = tonumber(Lib:GetLocalDay(GetTime()));
	if cKin.GetChangeCampDate() == nDate then
		Dialog:Say("Trại gia đình chỉ có thể được thay đổi một lần mỗi ngày và trại đã được thay đổi một lần hôm nay.")
		return 0;
	end
	if me.nCashMoney < self.CHANGE_CAMP then
		Dialog:Say("Sửa đổi trại yêu cầu thanh toán<color=red>"..(self.CHANGE_CAMP / 10000).."10.000 kiếm sĩ xu <color>, bạn không có đủ kiếm sĩ xu!！");
		return 0;
	end
	if not nCamp then
		Dialog:Say("Bạn cần trả <color=red> để sửa đổi trại"..(self.CHANGE_CAMP / 10000).."10.000 xu kiếm sĩ <color>, vui lòng chọn trại bạn muốn thay đổi:",
{{"Song Fang", self.DlgChangeCamp, self, 1},
{"Hình vuông vàng", self.DlgChangeCamp, self, 2},
{"Trung lập", self.DlgChangeCamp, self, 3},
{"Để tôi nghĩ lại"}
});
	else
		if cKin.GetCamp() == nCamp then
		Dialog:Say("Gia đình bạn đã thuộc phe này rồi, không cần thay đổi!");
			return 0;
		end
...
function Kin:DlgKinExp()
	...
	Dialog:Say("Để giúp bạn bắt đầu sự kiện cắm cờ gia đình dễ dàng hơn, khi bạn nhận được mã thông báo gia đình và sử dụng nó, bạn có thể đặt thời gian khi sự kiện cắm cờ gia đình sẽ tự động bắt đầu trong tương lai. Tất nhiên, nếu bạn không hài lòng với thời gian đã đặt, bạn có thể sửa đổi Nó, tất nhiên chúng tôi tính một khoản phí xử lý nhỏ cho các sửa đổi. Vậy bạn muốn làm gì bây giờ?", tbSay)
end
...
Dialog:Say("Gia đình bạn đã nhận được mã thông báo gia đình hôm nay, nếu bạn muốn sửa đổi nó, vui lòng quay lại vào ngày mai.");
		return 0;
	end
...
Dialog:Say("Gia đình bạn đã nhận được nó. Vui lòng đặt thời gian và địa điểm cho hoạt động cắm cờ gia đình càng sớm càng tốt");
		return 0;
	end
	
	-- 还没领
	if bConfirm ~= 1 and cKin.GetGainExpState() ~= nNowDay then 
		Dialog:Say("Trong khi bạn đang sử dụng thông báo gia tộc, Vui lòng đặt thời gian để tự động bắt đầu hoạt động cắm cờ gia đình và chúng tôi sẽ tự động ghi lại vị trí cho bạn để hoạt động này có thể được tự động bắt đầu cho gia đình bạn trong tương lai.",
		{{"Có, tôi muốn nhận", self.DlgGetKinExp, Kin, 1},
		{"Đến lấy sau"}
				})
		return 0;
	end
