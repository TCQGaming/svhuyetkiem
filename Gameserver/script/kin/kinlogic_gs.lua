...
-------------------------------------------------------------------
if not Kin then --cần thiết để gỡ lỗi
	Kin = {}
...
Kin.c2sFun = {}
-- Đăng ký một hàm mà client có thể gọi trực tiếp
...
function Kin:_CheckMemberCount(nRegular, nSigned, nRetire)
	...
	if nRegular + nSigned >= ConfigAll.GioiHanThanhVienToc then
		return nil
	end
	return 1
end
...
function Kin:MemberAdd_GS2(nDataVer, nKinId, nPlayerId, nMemberId, nJoinTime, nStoredRepute, nPersonalStock, bCanJoinKinImmediately)
	...
	if cKin.GetRecruitmentPublish() == 1 and nMemberCount >= ConfigAll.GioiHanThanhVienToc then
...
--		return 0
--	end
	if GetTime() - KGCPlayer.OptGetTask(nPlayerId, KGCPlayer.TSK_LEAVE_KIN_TIME) < 1 * 3600 then
		me.Msg("Các bạn rời khỏi gia tộc ít hơn 1h")
		return 0
	end
...
function Kin:InviteAddReply_GS1(nKinId, nInvitorId, bAccept)
	...
	aInviteEvent[nPlayerId] = nil
	local bCanJoinKinImmediately = me.CanJoinKinImmediately();	--Được sử dụng để đánh giá xem bạn có thể tham gia gia đình ngay lập tức và trở thành một người thường xuyên hay không
...
--Xóa thành viên, nMethod = 0 để tự thoát, nMethod = 1 để trục xuất
...
function Kin:MemberLeave_GS1()
	...
	if (nMapId == 1832) or (nMapId == 1721) or (nMapId == 1539) or (nMapId == 1540) then
	me.Msg("Bạn Không Thể Rời Gia Tộc Trong Bản Đồ Này")
	return 0;
	end
...
function Kin:LeaveApply_GS1(bLeave, bCanLeaveKinImmediately)
	...
	if bLeave == 1 then
		if (not bCanLeaveKinImmediately or bCanLeaveKinImmediately == 0) then
			me.Msg("Bạn đã đồng ý rút khỏi gia tộc, sau 1 ngày bạn có rời tộc, bạn có thể hủy rời tộc")
...
function Kin:MemberRetire_GS2(nKinId, nMemberId, nTime)
	...
	local nFigure = cMember.GetFigure()
	if nFigure == self.FIGURE_ASSISTANT then
		cKin.SetAssistant(0); 			-- Phó trưởng ban nghỉ hưu
	end
	cMember.SetFigure(self.FIGURE_RETIRE)
	cMember.SetEnvoyFigure(0);			-- Hưu tríXóa chức Khâm sứ
	cMember.SetBitExcellent(0);			-- 退隐删除精英
	cMember.SetRetireTime(nTime);		-- Ghi lại thời gian nghỉ hưu
...
function Kin:ChangeCaptain_GS1(nMemberId)
	...
	if KGCPlayer.GetPlayerPrestige(nPlayerId) < 10 then
		me.Msg("Uy danh đạt 10 điểm mới có thể chuyển tộc trưởng")
		return 0
	end
	if nExcutorId == nMemberId then
		return 0
...
function Kin:RecruitmentPublish_GS1(nPublish, nLevel, nHonour)
	...
	if nMemberCount >= ConfigAll.GioiHanThanhVienToc then
		me.Msg("Gia tộc đã đầy "..ConfigAll.GioiHanThanhVienToc.." người rồi không chiêu mộ được hơn");
		return 0;
	end
...
function Kin:RecruitmentAgree_GS1(szName)
	...
	-- if nMemberCount >= self.MEMBER_LIMITED then
	if nMemberCount >= ConfigAll.GioiHanThanhVienToc then
		me.Msg("Gia tộc của bạn đã đầy "..ConfigAll.GioiHanThanhVienToc.." thành viên hiện tại không thể gia nhập");
		return 0;
	end
...
function Kin:AddKinKillBaiHuTangCount_GS(nPlayerId, nNum)
	-- É±ËÀboss»á¸ø¼Ò×åÔö¼ÓÒ»´Î±¾ÈÕ»÷É±boss¸öÊý
