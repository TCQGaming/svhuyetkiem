...
function tbRandomItem:GetItem(pPlayer, tbitem, tbTask, nMustGet)
		...
		if tbItemInfo.bTimeOut ~= 1 then
			...
			for i= 1, nCount do
				...
				if pItem then
					...
					local szAnnouce = string.format("Chúc mừng bạn đã nhận được <color=yellow>%s<color>", pItem.szName);
					pPlayer.Msg(szAnnouce);
	...
	-- if tbitem.nAnnounce == 1 then
		-- local szMsg = string.format("<color=Green>%s<color> mở <color=cyan>%s<color> nhận được <color=yellow>%s<color>, thật may mắn!", pPlayer.szName, tbitem.szDesc, tbitem.szName);
		-- KDialog.NewsMsg(1, Env.NEWSMSG_NORMAL, szMsg);
	-- end
	
	-- if tbitem.nFriendMsg == 1 then
		-- pPlayer.SendMsgToFriend("Hảo hữu của bạn [<color=yellow>"..pPlayer.szName.."<color>] thật may mắn khi mở "..tbitem.szDesc..
