...
function tbGouhuoItem:IsPickable(nObjId)
	...
	local nMapCoLua = ConfigAll.MapCoLuaTrai	
	 local nMapId = me.GetWorldPos()
if nKhoiDong == 1 and nMapId ~= nMapCoLua then
    me.Msg("<color=gold>Hiện nay đang trong thời gian đua top không thể mở lửa trại ngoài map thường hãy di chuyển lên map <color=green>"..GetMapNameFormId(nMapCoLua).."<color>")
    return 0;
end


	if (0 == me.nTeamId) then
		me.Msg("Tổ đội mới có thể đốt lửa trại.");
...
function tbGouhuoItem:PickUp(nX, nY)
	...
	if (nTeamId > 0) then
		KTeam.Msg2Team(nTeamId, "Lửa trại đã được đốt, các thành viên trong nhóm có thể cùng nhau chia sẻ điểm kinh nghiệm.");
	end
	return	0;
end
...
function tbGouhuoItem:DelayTime(pPlayer, nObjId)
	GeneralProcess:StartProcess("Đang đốt...", self.nDelayTime * Env.GAME_FPS,
