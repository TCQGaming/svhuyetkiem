...
tbChuangsongfu.nTime = 5;			-- 延时的时间(秒)
...
tbChuangsongfu.TongKimTrung =
{
	["Tống Kim Phe Tống"]	= { 181, 1677, 3294 }, -- Thay x1 và y1 bằng tọa độ mong muốn
	["Tống Kim Phe Kim"]	= { 184, 1677, 3294 }, -- Thay x2 và y2 bằng tọa độ mong muốn
};
tbChuangsongfu.TongKimCao =
{
	["Tống Kim Phe Tống"]	= { 182, 1677, 3294 }, -- Thay x1 và y1 bằng tọa độ mong muốn
	["Tống Kim Phe Kim"]	= { 185, 1677, 3294 }, -- Thay x2 và y2 bằng tọa độ mong muốn
};
-- 去新手村的传送符
...
tbChuangsongfu.tbCityMap =
{
	...
	["<color=yellow>[Thành Chính] Phượng Tường Phủ"]	= { 24, 1767, 3540 },
...
function tbChuangsongfu:OnUse()
	...
	local nTongBacKhoa = me.GetBindMoney()
local nTongBacKhoa1 = math.floor(nTongBacKhoa/10000);
local nSoBacKhoaTru = 50000*10000 ----- 5 ức
if nTongBacKhoa1 >= 10000 then -- nếu nhiều hơn hoặc bằng 5 ức thì trừ đi 1 ức tránh lỗi
	 me.CostBindMoney(nSoBacKhoaTru,0);
end 
			-- local nKhoiDong = ConfigAll.OpenDuaTOP	
	-- if nKhoiDong == 1 then 		
	-- DuaTopCap:capnhattopcap(me)
	-- end 
	if self.tbNewTransItem[it.nParticular] then
		...
		local tbOpt = {
			...
			-- {"Tống Kim Báo Danh", self.OnTransItem, self, it, self.TongKimCao, self.tbNewTransItem[it.nParticular]},
			 {"Phục Ngưu Sơn Quân Doanh",  self.OnTransArmyCamp, self, it.dwId},
			{"<color=yellow>Cửa Tần Lăng 1<color>", self.DelayTime2, self, it, self.toitanlang, self.tbNewTransItem[it.nParticular]},
			-- {"<color=blue>Cửa Tần Lăng 4(Khu 1)<color>", self.DelayTime3, self, it, self.toitanlang, self.tbNewTransItem[it.nParticular]},
			-- {"<color=blue>Cửa Tần Lăng 4(Khu 2)<color>", self.DelayTime4, self, it, self.toitanlang, self.tbNewTransItem[it.nParticular]},
			-- {"<color=blue>Cửa Tần Lăng 4(Khu 3)<color>", self.DelayTime5, self, it, self.toitanlang, self.tbNewTransItem[it.nParticular]},
			-- {"<color=blue> Đi Đến Tầng Lăng",  self.dentanglang4, self, it.dwId},
		}
...
-- thành chính
function tbChuangsongfu:DelayTime1(pItem, tbPos, nIsLimit, szName)
	...
	GeneralProcess:StartProcess("Đang truyền tống...", self.nTime * Env.GAME_FPS, {self.TransPlayer1, self, pItem.dwId, tbPos, nIsLimit, szName}, nil, tbEvent);
end
function tbChuangsongfu:TransPlayer1(nItemId, tbPos, nIsLimit, szName)
	me.NewWorld(24,1796,3496);
end
function tbChuangsongfu:DelayTime2(pItem, tbPos, nIsLimit, szName)
	local nCanUse, szRetMsg = Item:CheckIsUseAtMap(me.nMapId, "chuansong");
	if (nCanUse == 0) then
		me.Msg(szRetMsg);
		return;
	...
	GeneralProcess:StartProcess("Đang truyền tống...", self.nTime * Env.GAME_FPS, {self.TransPlayer2, self, pItem.dwId, tbPos, nIsLimit, szName}, nil, tbEvent);
end
function tbChuangsongfu:DelayTime3(pItem, tbPos, nIsLimit, szName)
	local pTong = KTong.GetTong(me.dwTongId);
	if not pTong then
		Dialog:Say("Bạn Không Có Bang Hội Không Thể Đến Nơi Đây")
		-- me.NewWorld(24,1802,3486);
		return;		--Phượng Tường Phủ
	end
	local nCanUse, szRetMsg = Item:CheckIsUseAtMap(me.nMapId, "chuansong");
	if (nCanUse == 0) then
		me.Msg(szRetMsg);
	...
	GeneralProcess:StartProcess("Đang truyền tống...", self.nTime * Env.GAME_FPS, {self.TransPlayer3, self, pItem.dwId, tbPos, nIsLimit, szName}, nil, tbEvent);
end
function tbChuangsongfu:DelayTime4(pItem, tbPos, nIsLimit, szName)
local pTong = KTong.GetTong(me.dwTongId);
	if not pTong then
		Dialog:Say("Bạn Không Có Bang Hội Không Thể Đến Nơi Đây")
		-- me.NewWorld(24,1802,3486);
	...
	GeneralProcess:StartProcess("Đang truyền tống...", self.nTime * Env.GAME_FPS, {self.TransPlayer4, self, pItem.dwId, tbPos, nIsLimit, szName}, nil, tbEvent);
end
function tbChuangsongfu:DelayTime5(pItem, tbPos, nIsLimit, szName)
local pTong = KTong.GetTong(me.dwTongId);
	if not pTong then
		Dialog:Say("Bạn Không Có Bang Hội Không Thể Đến Nơi Đây")
		-- me.NewWorld(24,1802,3486);
	...
	GeneralProcess:StartProcess("Đang truyền tống...", self.nTime * Env.GAME_FPS, {self.TransPlayer5, self, pItem.dwId, tbPos, nIsLimit, szName}, nil, tbEvent);
end
function tbChuangsongfu:TransPlayer2(nItemId, tbPos, nIsLimit, szName)
	me.NewWorld(132,1945,3310);
end
function tbChuangsongfu:TransPlayer3(nItemId, tbPos, nIsLimit, szName)
	me.NewWorld(1539,1920,3582);
end
function tbChuangsongfu:TransPlayer4(nItemId, tbPos, nIsLimit, szName)
	me.NewWorld(1539,1671,3825);
end
function tbChuangsongfu:TransPlayer5(nItemId, tbPos, nIsLimit, szName)
	me.NewWorld(1539,1811,3712);
end
