...
function tbHuichengfu:OnUse()
	...
	if nTrangThaiVanTieu == 1 then 
		me.Msg("<color=yellow>Trong khi vận tiêu không sử dụng được truyền tống phù!!")
		return 
	end
...
function tbHuichengfu:TransPlayer(nItemId, nPlayerId)
	...
	local nCanUse, szRetMsg = Item:CheckIsUseAtMap(nMapId, "chuansong");
	if (nCanUse == 0) then
		pPlayer.Msg(szRetMsg);
		return;
	end
