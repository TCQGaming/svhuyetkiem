...
function tbXiang:OnUseGet(nItemId)
	...
	GeneralProcess:StartProcess("Đang mở...", 4 * Env.GAME_FPS, {self.OnUseAskCount, self, nMaxTakeOutCount}, nil, tbBreakEvent);
	return 0;
end
...
function tbXiang:GetTip(nState)
	local szTip = "";
	szTip = szTip.."Rương có <color=yellow>"..KItem.GetNameById(it.GetExtParam(PARAM_IDX_GENRE), it.GetExtParam(PARAM_IDX_DETAILTYPE), it.GetExtParam(PARAM_IDX_PARTICULAR), it.GetExtParam(PARAM_IDX_LEVEL)).."<color>, <color=yellow>nhấp chuột phải<color> để lấy vật phẩm trong rương.\n";
