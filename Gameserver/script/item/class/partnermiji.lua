...
function tbItem:OnSelectUse(dwId, nParam, bSure)
	...
	if bSure == 0 then
		...
		--检查使用同类物品的次数
		if self:CheckSkillBookCount(nCount) == 10 then
		...
		if self:CheckPartnerGrade(nParam, dwId) == 1 then
			Dialog:Say("Điều kiện sử dụng: <color=yellow>Đồng hành 6 sao trở xuống<corlor> có thể dùng Mật tịch đồng hành (sơ), <color=yellow>Đồng hành 6.5 - 8 sao<corlor> có thể dùng Mật tịch đồng hành (trung), <color=yellow>Đồng hành 8 sao trở lên<corlor> có thể dùng Mật tịch đồng hành (cao). Đồng hành của ngươi không đáp ứng điều kiện, không thể sử dụng.", {"Ta biết rồi"});
			return 0;
		end
...
function tbItem:AddPartnerSkillLevel(nParam, dwId)
	...
	me.Msg(string.format("Bạn đã sử dụng Mật tịch đồng hành, kỹ năng đồng hành <color=yellow>%s<color> tăng lên 1 cấp",KFightSkill.GetSkillName(tbStatisSkil[nAddSkillIndex][2])));
	-- pItem.Delete(me);
