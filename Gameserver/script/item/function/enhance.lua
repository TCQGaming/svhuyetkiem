...
Item.VALID_PEEL_TIME = 1;	-- thoi gian xin tach
...
function Item:ApplyPeelHighEquipSure()
	...
	-- SkillId : 1358
	me.AddSkillState(1358, 1, 1, self.MAX_PEEL_TIME * Env.GAME_FPS, 1, 0, 1);
...
function Item:Enhance(pEquip, tbEnhItem, nMoneyType, nParam)	-- 程序接口：服务端执行装备强化
	...
	if nProb < 100 and pEquip.nEnhTimes >= 10 then
		me.Msg("<color=gold>Từ + 10 Trở Đi 100% Mới Có Thể Tiến Hành Cường Hóa.");
		return -1;
...
function Item:CalcMaxEnhanceTimes(pEquip)	-- 计算一个可强化装备能强化的次数(最大强化等级)
	...
	local nParticular = pEquip.nParticular;
	...
	if nScriptVersion == 1 then
		...
		elseif (nLevel > 9) and (pEquip.nRefineLevel >= 1) then-- 炼化1级的才能强16
			return 16;		
		elseif (nLevel == 10) and (nDetail ~= 1) and (nDetail ~= 2) then		
			return 16;		
		...
		elseif (nLevel <= 6) then
			return 8;					-- 4~6级可强化8次
		elseif (nLevel <= 8) and (nDetail  ~= 1) then
			return 12;					-- 7~9级可强化12次
		elseif (nLevel == 9) and (nDetail ~= 1) and (nDetail ~= 2) then
			return 12;					-- 7~9级可强化12次
		elseif (nLevel == 9) then
			return 14;					-- 7~9级可强化12次	
		elseif(nLevel  == 9) and (nParticular > 10) and (nPos == Item.EQUIPPOS_WEAPON) then--------Vũ Khí Chế Cấp 9
			return 14;
		elseif(nLevel  == 9) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_WEAPON) then---------Vũ Khí Hải Tặc Cấp 9 
			return 14;
		elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_BODY) then---------Áo hải tặc cấp 10
			return 14;
		elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_HEAD) then---------Nón hải tặc cấp 10
			return 14;
		elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_BELT) then---------Đai hải tặc cấp 10
			return 14;
		-- elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_CUFF) then---------Hộ uyển hải tặc cấp 10
			-- return 14;
			elseif (nLevel == 10) and (nParticular >= 2 and nParticular <= 10) and (nPos == Item.EQUIPPOS_CUFF) then  -- Hộ uyển hải tặc cấp 10
		return 14;
		elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_FOOT) then---------Giày hải tặc cấp 10
			return 14;
		elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_NECKLACE) then---------Liên hải tặc cấp 10
			return 14;
		elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_RING) then---------Nhẫn  hải tặc cấp 10
			return 14;
		elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_PENDANT) then---------Bội  hải tặc cấp 10
			return 14;
		elseif(nLevel  == 10) and (nParticular <= 10) and (nPos == Item.EQUIPPOS_AMULET) then---------Phù  hải tặc cấp 10
			return 14;
		elseif (nLevel == 10) and (pEquip.nRefineLevel == 0) and (nDetail ~= 1) and (nDetail ~= 2) then
			return 14;		
		else
			return 16;					-- 10级可强化16次
		end
