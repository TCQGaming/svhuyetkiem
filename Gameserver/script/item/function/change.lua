...
function tbGift:OnSwitch(pPickItem, pDropItem, nX, nY)
	if pDropItem then
		if Item:CalcChange({pDropItem}) <= 0 then
			me.Msg("M?t hàng kh?ng ???c trao ??i！");
			return 0;
		end
	end	
	return	1;
...
function tbGift:OnUpdate()
	self._szTitle = "Trao ??i";
	local pItem = self:First();
	local tbItem = {}
	if not pItem then
		self._szContent = "Vui lòng ??a vào m?c ?? ???c ??i。"
		return 0;
	end
	while pItem do
	...
	self._szContent = "B?n có th? ??i thành<color=yellow>"..nChangeNum.."个<color>Ng? hành，c?n"..math.ceil(nChangeNum / Item.SPIRITSTONE_STACK_NUM).."hành trang ko ?? ch? tr?ng";
end
...
function tbGift:OnOK(tbParam)
	...
	local tbItem = {}
	if not pItem then
		me.Msg("Vui lòng ??a vào các m?c c?n trao ??i.")
		return 0;
	end
	while pItem do
		table.insert(tbItem, pItem);
	...
	if me.CountFreeBagCell() < nFreeCount then
		me.Msg(string.format("Ba l? c?a b?n kh?ng ?? dung l??ng", nFreeCount));
		return 0;
	end
	Item:Change(tbItem)
end
