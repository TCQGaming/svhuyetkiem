...
if (MODULE_GC_SERVER) then
	...
	function PlayerHonor:OnSchemeUpdateHonorLadder()
		...
		local nWeekDay	= tonumber(os.date("%w", nCurTime));
		    local nThoiGian = tonumber(GetLocalDate("%H%M"))
		...
		if (nWeekDay == 1 and  nCurTimer > 0001) then---------------Làm mới bảng xếp hạng
	...
	function PlayerHonor:UpdateWuLinHonorLadder()
		print("Xep hang Vinh Du Vo Lam bat dau");
		local nType = 0;
		...
		print("Xep hang Vinh Du Vo Lam ket thuc");
	end
	
	-- 财富荣誉
	function PlayerHonor:UpdateMoneyHonorLadder()
		print("Xep hang Vinh Du Tai Phu bat dau");
		local nType = 0;
		...
		print("Xep hang Vinh Du Tai Phu ket thuc");
	end
	
	-- 领袖荣誉
	function PlayerHonor:UpdateLeaderHonorLadder()
		print("Xep hang Vinh Du Thu Linh bat dau");
		local nType = 0;
		...
		self:SetShowLadder(tbShowLadder, nType, tbLadderCfg.szLadderName, szContext, tbLadderCfg.szPlayerContext, tbLadderCfg.szPlayerSimpleInfo);
		print("Xep hang Vinh Du Thu Linh ket thuc");
	end
	...
	function PlayerHonor:UpdateXoyoLadder(nFlag)
		print("Xep hang Vinh Du Tieu Dao Coc bat dau");
		local nType = 0;
		...
		print("Xep hang Vinh Du Tieu Dao Coc ket thuc");
...
function PlayerHonor:SetShowLadder(tbLadderResult, nType, szLadderName, szContext, szPlayerContext, szSimpleInfo)
		if (not tbLadderResult) then
			print("Tai day khong co so lieu xep hang....");
			return;
		end
...
　　
　　Xếp hạng vinh dự của bạn vào 00 giờ 2phút sáng hôm nay là: %s, nội dung chi tiết vào xem ở <link=openwnd:Bảng xếp hạng,UI_LADDER>.
