...
function Tong:PerTongDailyAction_Timer()
	...
	if cTong.GetTestState() ~= 0 then
		if cTong.GetBuildFund() >= self.TEST_PASS_BUILD_FUND then
			...
			KTong.Msg2Tong(nTongId, "Xin chúc mừng, bang đã vượt qua giai đoạn khảo nghiệm!")
		else
			local nLeftTime;
			...
			if nLeftTime < 0 then
				KTong.Msg2Tong(nTongId, "帮会在考验期内建设基金未达到"..self.TEST_PASS_BUILD_FUND.."，帮会解散！")
				Tong:DisbandTong_GC(nTongId);
				cTong = nil;	-- 对象失效，赋予nil值
			else
				KTong.Msg2Tong(nTongId, "帮会离考验期结束还有<color=red>"..math.ceil(nLeftTime / 24 / 3600)..
				"<color>天，必须在考验期内使建设基金达到"..self.TEST_PASS_BUILD_FUND.."，否则帮会将解散！")
			end
		end
	end
	if cTong then
...
function Tong:PerTongWeeklyAction_Timer()
	...
	if not self.PerTongWeekly_cNextTong then
		...
		if (Kin.bIsKinWeeklyActionOver and 1 == Kin.bIsKinWeeklyActionOver) then
			Kin:RecordWeeklyActionNo();	-- 记录流水号
			Kin:LogWeeklyTaskLevel();	-- 在家族和帮会都维护完成之后把周目标的log写进数据库
...
function Tong:DailyPresidentConfirm_Timer()
	...
	--Ham nay mo ra thi update theo ngay - ADMIN up dc danh cho viec bang chu quen set quan ham
	-- self:PresidentConfirm_GC(nTongId, 1)
...
function Tong:DailyPresidentConfirm()
	...
	if tonumber(os.date("%w", GetTime())) == self.PRESDIENT_CONFIRM_WDATA then				-- Dong ham if nay lai de fix cho bang chu quen set quan ham
