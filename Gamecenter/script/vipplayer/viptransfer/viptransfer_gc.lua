...
-- 增加转出数据项
function tbVipTransfer:AddApplyOut_GC(szPlayerName, tbInfo)
...
function tbVipTransfer:GetApplyOut_GC(szPlayerName)
	...
	if tbApplyOut then
		...
		local tbData = 
		{
			...
			tbApplyOut.nNewGateId, 
			tbApplyOut.nBindValue * 90,
			tbApplyOut.nBindValue * 2000,
...
-- 平台导入申请数据(字符串)
function tbVipTransfer:SetApplyIn_GC(szData)
	
	-- 验证数据列数
	local tbData = Lib:SplitStr(szData, ",");
	if Lib:CountTB(tbData) ~= 6 then
		return "数据格式错误，导入失败";
	end
	
	-- 格式化字符串
	local tbInfo = {};
	local szAccount = tbData[1];
	tbInfo.nNewGateId = tonumber(tbData[2]);
	...
	local tbMap = Lib:SplitStr(tbData[6], "|");
	...
	-- 验证网关
	local nGateId = tonumber(string.sub(GetGatewayName(), 5, 8));
	if tbInfo.nNewGateId ~= nGateId then
		return "网关不匹配，导入失败";
	end
...
function tbVipTransfer:StartEvent()
	...
	--载入global buffer
	local tbBuffer = GetGblIntBuf(GBLINTBUF_VIP_TRANSFER, 0, 1);
	if tbBuffer and type(tbBuffer) == "table" then
		self.tbGlobalBuffer = tbBuffer;
	end
	
	-- 计划任务
	local nTaskId = KScheduleTask.AddTask("Vip Qua Thoi han Thanh Tru So Lieu", "VipPlayer", "ClearExpireData");
