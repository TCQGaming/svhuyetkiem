...
function Wldh:RegisterScheduleTask()
	...
	local nTaskId = KScheduleTask.AddTask("Dai Hoi Vo Lam Doan The Vong Loai Don Dau", "Wldh", "ScheduleCallOut_PreGameStart");
	assert(nTaskId > 0);
	for i, nDate in ipairs(self.STATE1_PRE_TIME) do
		KScheduleTask.RegisterTimeTask(nTaskId, nDate, i);
	end
	
	nTaskId = KScheduleTask.AddTask("Dai Hoi Vo Lam Doan The Xac Dinh DS Chung Ket", "Wldh", "ScheduleCallOut_PreGameFinalList");
	for i, nDate in ipairs(self.STATE1_PRE_FINIAL_TIME) do
		KScheduleTask.RegisterTimeTask(nTaskId, nDate, i);
	end
	
	nTaskId = KScheduleTask.AddTask("Dai Hoi Vo Lam Doan The Mo Tran Chung Ket", "Wldh", "ScheduleCallOut_FinalGameStart");
	for i, nDate in ipairs(self.STATE2_PRE_FINIAL_TIME) do
		KScheduleTask.RegisterTimeTask(nTaskId, nDate, i);
	end
	
	nTaskId = KScheduleTask.AddTask("Dai Hoi Vo Lam Doan The Giai Doan Chuyen Doi", "Wldh", "ScheduleCallOut_State");
