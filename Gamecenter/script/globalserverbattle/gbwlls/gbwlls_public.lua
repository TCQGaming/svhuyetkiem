			...
			if (tbInfo) then
				...
				for j, tbLeagueInfo in ipairs(tbInfo) do
					...
					if (nGateWay == nNowGateWay and nNowGateWay > 0) then
						...
						szInfo = string.format("Ten Chien Doi  ：%s\n参赛门派：%s；\n战队成员：%s\n名次    ：%s", szLeagueName, szFaction, szMemberMsg, nRank);
						szInfo2 = string.format("Ten Chien Doi：%s；参赛门派：%s；战队成员：%s；名次：%s。", szLeagueName, szFaction, szMemberMsg, nRank);
						szMsg = szMsg .. szInfo .. "\n\n";
...
--					szInfo = string.format("    所在服务器：%s；Ten Chien Doi：%s；参赛门派：%s；战队成员：%s；名次：%s。\n", szServerName, szLeagueName, szFaction, szMemberMsg, nRank);
					szMsg = szMsg .. szInfo;
					nFlagCount = nFlagCount + 1;
		...
		for nFaction, tbInfo in pairs(tb8RankInfo.tbInfo) do
			if (tbInfo) then
				...
				for j, tbLeagueInfo in ipairs(tbInfo) do
					...
					szInfo = string.format("<color=yellow>名次：%s  服务器：%s  Ten Chien Doi：%s  成员：%s。<color>\n", nRank, szServerName, szLeagueName, szMemberMsg);
					szMsg = szMsg .. szInfo;
					if (not tbLeagueInfo.nGuessCount) then
						tbLeagueInfo.nGuessCount = 0;
					end
