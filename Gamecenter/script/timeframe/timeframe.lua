...
function TimeFrame:SetStartServerTime(nDate)
	if string.len(nDate) ~= 8 then
		print("Thoi gian truc thiet tri khai phuc thoi gian cach thuc pham sai lam(YYYYmmdd)", nDate)
		return
	end
...
function TimeFrame:GetStartServerTime(pPlayer)
	...
	if nSec == 0 then
		print("Con khong co thiet tri khai phuc thoi gian");
		if pPlayer then
			pPlayer.Msg("Còn kh?ng có thi?t trí khai ph?c th?i gian");
		end
		return 0;
	end
