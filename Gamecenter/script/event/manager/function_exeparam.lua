...
function tbFun:ExeMinusKinRepute(szParam)
	...
	if nDecreaseRepute > 0 then	
		...
		local szLog = string.format("Giảm uy tín %d, uy tín giảm từ %s xuống %s", nDecreaseRepute, nOldReput, nNewPrestige);
		EventManager:WriteLog(szLog, me);
		me.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, string.format("[Hoạt động] %s", szLog));
	end
	return 0;
end
