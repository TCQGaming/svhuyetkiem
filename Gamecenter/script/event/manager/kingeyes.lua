...
function tbEyes:LoadFile(szPath, szQuestPlayerListPath)
	...
	if not tbFile then
		print("EventManager.KingEyes Loi load file van ban txt~~~", szPath);
		return;
	end
	local nQuestType = 0;
	local nTmpEventId;
	...
	for nId, tbParam in ipairs(tbFile) do	
		...
		if not nEndDate then
			print("EventManager.KingEyes【 Hoat dong he thong pham sai lam 】 Thoi gian cach thuc pham sai lam");
			return;
		end	
