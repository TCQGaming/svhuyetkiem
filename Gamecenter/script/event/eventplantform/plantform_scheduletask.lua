...
function EPlatForm:RegisterScheduleTask()
	...
	local nTaskId = KScheduleTask.AddTask("Hoat Dong Thi Dau The Thao Mo Ra", "EPlatForm", "ScheduleCallOut_TimerFrame");
	assert(nTaskId > 0);
	...
	nTaskId = KScheduleTask.AddTask("Hoat Dong The Thao [Hang Ngay]", "EPlatForm", "ScheduleCallOut_Common");
	assert(nTaskId > 0);
	for nTaskSeriel, nTimeState in pairs(tbCommon) do
		KScheduleTask.RegisterTimeTask(nTaskId, nTimeState, nTaskSeriel);
	end
	
	nTaskId = KScheduleTask.AddTask("Hoat Dong The Thao Giai Doan 2[Hang Ngay]", "EPlatForm", "ScheduleCallOut_Common_Adv");
	assert(nTaskId > 0);
	for nTaskSeriel, nTimeState in pairs(tbCommon_Adv) do
		KScheduleTask.RegisterTimeTask(nTaskId, nTimeState, nTaskSeriel);
	end
	
	nTaskId = KScheduleTask.AddTask("Hoat Dong The Thao[Bat Cuong]", "EPlatForm", "ScheduleCallOut_AdvMatch");
	assert(nTaskId > 0);
