...
function Compensate:LoadFile(szPath,nTaskId)
	...
	local szReturn = "";
	for i = 1,#tbbuf do 
		szReturn = self:AddOnNpc(tbbuf[i].szGate, tbbuf[i].szAccount, tbbuf[i].szName, tbbuf[i].nSDate, tbbuf[i].nEDate, tbbuf[i].tbAward, nTaskId);
	end
	--如果是单个补偿，马上返回错误信息
...
function Compensate:ExLoadFile(szPath)
	...
	if not tbFile then
		print("Loi load file van ban txt~~~");
		return;
	end
	local tbBuf = {};
	for nId, tbParam in ipairs(tbFile) do
		...
		for szKey, tbKey in pairs(tbKingEyes.ExeFunList) do
			if tbParam[szKey] then
				...
				if tbKey[4] == 1 then
					nNeedBag = nNeedBag + (tonumber(tbParamT[2]) or 0);
				end
				if tbKey[4] == 2 then
...
--增加补偿存档
function Compensate:AddGblBufOnLine(szGate, szAccount, szName, nSDate, nEDate, szScript)
	...
	Dbg:WriteLog("CompensateGM", "AddOnLine", szGate, szAccount, szName, nSDate, nEDate, nLogId, szScript);
	self:SetGblBuf(tbBuf);
	if (MODULE_GC_SERVER) then
		GlobalExcute({"SpecialEvent.CompensateGM:AddGblBufOnLine", szGate, szAccount, szName, nSDate, nEDate, szScript});	
	end
	return nLogId;
end

--增加补偿存档,通过npc
function Compensate:AddGblBufOnNpc(szGate, szAccount, szName, nSDate, nEDate, tbAward, nTaskId)
	local tbBuf = self:GetGblBuf();
	if not tbBuf then
		tbBuf = {};
	end
...
function Compensate:StartEvent()
	self.tbGblBuf = {};
	local tbBuf = GetGblIntBuf(GBLINTBUF_COMPENSATE_GM, 0, 1);
...
function Compensate:ClearDateOut()
	...
	print("Den Bu Online qua thoi gian thanh tru","So Luong thanh tru:", #tbOnLineDels + #tbOnNpcDels);
...
--在线补偿
function Compensate:AddOnLine(szGate, szAccount, szName, nSDate, nEDate, szScript)
	...
	local nLogId = self:AddGblBufOnLine(szGate, szAccount, szName, tonumber(nSDate), tonumber(nEDate), szScript);
	GlobalExcute({"SpecialEvent.CompensateGM:AddOnLine", szGate, szAccount, szName});
	return nLogId;
end

--补偿通过npc
function Compensate:AddOnNpc(szGate, szAccount, szName, nSDate, nEDate, tbAward, nTaskId)
	...
	local nPlayerId = KGCPlayer.GetPlayerIdByName(szName);
	if not nPlayerId or nPlayerId <= 0 then
		Dbg:WriteLog("CompensateGM", "AddOnNpcFail", "role not exist:"..szName);
		return "role not exist:"..szName;
	end
	if tonumber(nEDate) > 0 then
		if string.len(nEDate) == 8 then
	...
	local nLogId = self:AddGblBufOnNpc(szGate, szAccount, szName, tonumber(nSDate), tonumber(nEDate), tbAward, nTaskId);
	return nLogId;
end
...
function Compensate:AddOnLine(szGate, szAccount, szName)
	...
	if not nPlayerId or nPlayerId <= 0 then
		...
		Dbg:WriteLog("CompensateGM", "AddOnLineFail", "role not exist:"..szName);
		return "role not exist:"..szName;
	end
	...
	for nLogId, tbAward in pairs(tbAwards) do
		...
		if nCurDate >= tbAward.nSDate and nCurDate < tbAward.nEDate then
			if (not tbAward.szAccount or tbAward.szAccount == "" or szAccount == tbAward.szAccount) then
				...
				--nNeedBag 		= nNeedBag + (tonumber(tbAward.tbAward.nNeedBag) or 0);
