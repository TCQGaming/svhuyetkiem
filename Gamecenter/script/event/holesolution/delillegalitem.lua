...
--Tải tệp danh sách đen và xử lý người chơi theo dữ liệu tệp
------------------------------------------------------
...
function HoleSolution:LoadBlackListToDataBase(szPath, szIndex)
	...
	if not tbFile then
		return "Loi load file van ban txt";
	end
...
function HoleSolution:LoadDataFromDataBase_GC(bUpdateData)
	...
	if bUpdateData ~= 1 then
		...
		self.tbBlackList = GetGblIntBuf(GBLINTBUF_BLACKLIST, 0, 1);
		if not self.tbBlackList then
			print("Khong Co So Lieu ,Tu Tra Ve");
			return;
		end
	end

	if not self.tbBlackList then
		print("So den khong co ban dau hoa, truc tiep tro ve");
		return;
	end
...
function HoleSolution:DeductIlleageItem(tbItemTag, tbSingleRecord)
	...
	--Sau đó trừ uy tín và uy tín được chỉ định trên bảng cấu hình
	if tbItemTag["Ext"] then
		while tbItemTag["Ext"][nTaskCount] do
			...
			if nGoupId == 0 and nSubId == 0 then   -- Giang Hồ uy tín không tồn tại trong biến nhiệm vụ và cần xử lý đặc biệt
				local nOrgPrestige = KGCPlayer.GetPlayerPrestige(me.nId);
				local nNewPrestige = nOrgPrestige - tbSingleRecord["Ext"][nTaskCount];
				-- Nếu có tình trạng trích không đủ thì giảm về 0 ngay, sau này nếu có ý tưởng quy đổi ra giá trị thì sẽ bị xử lý.
				nNewPrestige = nNewPrestige > 0 and nNewPrestige or 0;
				KGCPlayer.SetPlayerPrestige(me.nId, nNewPrestige);
			else	-- Đó là nhiều loại uy tín khác nhau, tất cả đều được lưu trữ trong các biến nhiệm vụ
...
--Tham số đầu tiên cho biết giá trị nợ, tham số thứ hai cho biết phương thức bù và tham số thứ ba cho biết chỉ mục của mảng nhiệm vụ được cung cấp để sử dụng
...
--Xác định xem phương thức bù của loại hiện tại có khả dụng hay không và kiểm tra xem có bất kỳ khoảng trống nào trong mảng tbSubTaskGroup hay không
-- (nếu có, hãy trả về chỉ mục của khoảng trống, nếu không thì trả về 0)
--Đầu tiên đánh giá xem loại này có thể được đặt chồng lên một không gian đã được sử dụng hay không (có cùng giá trị);
-- sau đó đánh giá xem có dung lượng trống hay không (giá trị là 0); nếu không thì không thể sử dụng được
function HoleSolution:CanSetValue(nTaskVar)
	local nZeroIndex = 0;  --Ghi lại chỉ mục của nhóm đầu tiên trong mảng không có giá trị đã đặt và trả về chỉ mục đó để sử dụng nếu không thể xếp chồng.
	for nIndex, tbGroup in pairs(self.tbSubTaskGroup) do
		...
		--Lấy chỉ mục của tập hợp không gian chưa sử dụng đầu tiên trong mảng
		nZeroIndex = (nZeroIndex == 0 and nPreValue == 0) and nIndex or nZeroIndex;
	end
	
	return nZeroIndex;  --Trả về 0 nếu không có chỗ trống.
end
