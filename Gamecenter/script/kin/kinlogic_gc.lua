...
function Kin:MemberIntroduce_GC(nKinId, nExcutorId, nPlayerId, nPrestige)
	...
	--Nếu đề xuất đã được bắt đầu, hãy quay lại
...
function Kin:MemberDel_GC(nKinId, nMemberId, nMethod)
	...
	cKin.SetKinDataVer(self.nJourNum)
	-- Add Kin Log
	local szMsg = string.format("%s, %s", KGCPlayer.GetPlayerName(nPlayerId), (nMethod == 0 and "Phản Bội Gia Tộc" or "Bị Trục Xuất Khỏi Gia Tộc"));
	_G.KinLog(cKin.GetName(),  Log.emKKIN_LOG_TYPE_KINSTRUCTURE, szMsg);
...
function Kin:XoaUyDanhTuan(nPlayerId)
		local nWeekDay	= tonumber(GetLocalDate("%W"));
				if (nWeekDay == 6) then
				local nOrgPrestige = KGCPlayer.GetPlayerPrestige(nPlayerId);
				local nNewPrestige = 0
				nNewPrestige = nNewPrestige > 0 and nNewPrestige or 0;
				KGCPlayer.SetPlayerPrestige(nPlayerId, nNewPrestige);
				end
end
...
function Kin:ChangeKinExpState_GC(nPlayerId, nKinId, nMemberId)
	...
	if nRet ~= 1 then
		Dbg:WriteLog("Kin", "Không đủ quyền khi sửa đổi mã thông báo gia tộc", "Tên người chơi："..KGCPlayer.GetPlayerName(nPlayerId));
		return 0;
	end
	...
	if cKin.GetGainExpState() == nNowDay then
		Dbg:WriteLog("Kin", "Khi sửa đổi mã thông báo gia tộc, tôi thấy rằng mã thông báo gia tộc đã được nhận vào ngày hôm nay", "Tên người chơi："..KGCPlayer.GetPlayerName(nPlayerId));
		return 0;
	end
	
	-- 如果已经给过钱了还没设时间或者是第一次领令牌
	if cKin.GetKinBuildFlagOrderTime() == 0 then
		Dbg:WriteLog("Kin", "Khi sửa mã thông báo gia tộc, người ta thấy rằng tiền đã được đưa ra và thời gian chưa được đặt hoặc đây là lần đầu tiên nhận mã thông báo", "Tên người chơi："..KGCPlayer.GetPlayerName(nPlayerId));
		return 0;
	end
...
-- 1. Bạch Hổ Sảnh 2. Tống Kim 3. Truy nã 4. Thung lũng hạnh phúc 5. Doanh trại
Kin.tbWeeklyAction = {
	{1, 2, 3,},	-- Nhiệm vụ mục tiêu hàng tuần cấp 50
	{1, 2, 3, 4,},	-- Nhiệm vụ mục tiêu hàng tuần cấp 80 级的周目标任务
	{1, 2, 3, 4, 5,},	-- Nhiệm vụ mục tiêu hàng tuần cấp 90级的周目标任务
	};
...
function Kin:PerKinWeeklyTask_GC(cKin, nKinId)
	...
	if (cKin.GetBelongTong() ~= 0) then
		return 0;	-- Gia đình có băng nhóm không đối phó với gia tộc
	end
	Kin:StatisticsWeeklyTaskLevel(cKin);
	--- Nhiệm vụ hàng tuần ngẫu nhiên cho gia đình và ghi lại các nhiệm vụ hàng tuần trong gia đình
