...
function Dialog:Gift(szTable, ...)
	...
	if (not tbGift) or (type(tbGift) ~= "table") then
		print("Giao dien doi tuong vo hieu: "..tostring(szTable));
		return;
	end
	if (not Lib:IsDerived(tbGift, Gift)) then
		print("Giao dien doi tuong can thong qua Gift: "..tostring(szTable));
		return;
	end
	local tbPlayerDialog = self:GetMyDialog();
	local tbCallBacks =
...
function Dialog:OnSelect(nSelect)
	...
	if MODULE_GAMESERVER then
		...
		local oldme = me;
		local oldhim = him;
		me = KPlayer.GetPlayerObjById(tbGRoleArgs.playerId)
		him = KNpc.GetById(tbGRoleArgs.npcId);
