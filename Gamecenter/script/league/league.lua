...
function League:RepairMemberLeague(nType, szLeagueName, szMemberName)
	...
	if (MODULE_GAMESERVER) then
		...
		if (pPlayer) then
			pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, string.format("战队数据错误修复 Ten Chien Doi：%s, 玩家名：%s，类型：%d", szLeagueName, szMemberName, nType));
		end
...
function League:GetFileHistoryInforByLeague(nLgType, nDate, szLeagueName)
	...
	local tbFileTitle = {["LeagueName"]="Ten Chien Doi",["LeagueMembers"]="成员列表",["Date"]="日期"};
