...
function DragonBoat:AddHonor(pPlayer, nHonor)
	...
	pPlayer.Msg(string.format("你獲得了<color=yellow>%s點龍舟賽榮譽點<color>", nHonor));
...
function DragonBoat:OnPlayerType3(nGroup, nPoint)
	...
	if nGiveRank == 0 then
		me.Msg("賽道中已沒有可遭受攻擊的對手了！");
		Dialog:SendBlackBoardMsg(me, "賽道中已沒有可遭受攻擊的對手了！");
	else
...
function DragonBoat:OnPlayerBaseType1(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("你在河道中驚喜的發現了龍舟秘術%s！", KFightSkill.GetSkillName(tbSkill[1])));
	me.Msg(string.format("<color=blue>你在河道中驚喜的發現了龍舟秘術%s！<color>", KFightSkill.GetSkillName(tbSkill[1])));	
end
...
function DragonBoat:OnPlayerBaseType2(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("風雲突變，你在河道中的龍舟遭受到秘術%s的干擾！", KFightSkill.GetSkillName(tbSkill[1])));
	me.Msg(string.format("<color=blue>風雲突變，你在河道中的龍舟遭受到秘術%s的干擾！<color>", KFightSkill.GetSkillName(tbSkill[1])));	
end
...
function DragonBoat:OnPlayerBaseType3(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("你從河道中意外的打撈到%s！", KItem.GetNameById(tbItem[1], tbItem[2], tbItem[3], tbItem[4])));
	me.Msg(string.format("<color=blue>你從河道中意外的打撈到%s！<color>", KItem.GetNameById(tbItem[1], tbItem[2], tbItem[3], tbItem[4])));		
end
...
function DragonBoat:OnPlayerBaseType4(pCastPlayer, tbPlayerInfo)
	...
	if pCastPlayer.nId == pPlayer.nId then
		local szMsg = string.format("真倒霉，你遭受到了自己釋放的天罰%s的攻擊。", KFightSkill.GetSkillName(tbSkill[1]));
		pPlayer.Msg(string.format("<color=blue>%s<color>", szMsg));
		Dialog:SendBlackBoardMsg(pPlayer, szMsg);
		return 0;
	end
	
	local szMsg1 = string.format("真可惡，你遭受到了%s釋放的天罰攻擊%s！", pCastPlayer.szName, KFightSkill.GetSkillName(tbSkill[1]));	
	local szMsg2 = string.format("嘿，你對%s釋放的天罰攻擊%s！", pPlayer.szName, KFightSkill.GetSkillName(tbSkill[1]));	
