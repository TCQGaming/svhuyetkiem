...
function Wlls:CreateLeague(tbMemberList, szLeagueName, nMacthLevel, nExParam)
	...
	for nId, tbPlayer in ipairs(tbMemberList) do
		...
		if (GLOBAL_AGENT and MODULE_GC_SERVER) then
			Dbg:WriteLogEx(Dbg.LOG_INFO, "GbWlls_JoinLeague", tbPlayer.szName, szLeagueName, tbPlayer.nLevel, os.date("%Y-%m-%d",nTime));
		end
	end
...
function Wlls:LeaveLeague(szMemberName, nGameLevel, nQLeave)
	...
	if nGameLevel == Wlls.MACTH_ADV then
		...
		if Wlls:GetGameLevelForRank(szMemberName) == Wlls.MACTH_ADV then
			if #tbAdv <= 1 then
...
function Wlls:SetRankData(tbData, nMacthType, szTitle, nRankParam1, nRankParam2, nRankParam3, nRankParam4)
	...
	for nRank, tbLeague in ipairs(tbData) do
		...
		local tbMemberInfo = {
			...
			szTxt2 = string.format("%s", Lib:Transfer4LenDigit2CnNum(tbLeague.nSession)),
			szTxt3 = Wlls.SEASON_TB[tbLeague.nSession][4],
			szTxt4 = string.format("Thắng lợi: %s  Hòa: %s  Thua: %s", tbLeague.nWin, tbLeague.nTie, (tbLeague.nTotal - tbLeague.nWin - tbLeague.nTie) ),
...
function Wlls:LeagueRank(nFinalType, nUpdateNews)
	...
	if tbMacthCfg.nMapLinkType == Wlls.MAP_LINK_TYPE_RANDOM then
		...
		if (MODULE_GC_SERVER) then	
			--当届前10名入榜.
			Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_PRIM], tbMacthCfg.nMapLinkType, "Võ Lâm Liên Đấu Sơ", 0, Ladder.LADDER_CLASS_WLLS, 1, 0);
			Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_ADV], tbMacthCfg.nMapLinkType, "Võ Lâm Liên Đấu Cao", 0, Ladder.LADDER_CLASS_WLLS, 2, 0);
	...
	if tbMacthCfg.nMapLinkType == Wlls.MAP_LINK_TYPE_SERIES then
		...
		if (MODULE_GC_SERVER) then	
			...
			for nSereis in pairs(self.RankLeagueList[Wlls.MACTH_PRIM]) do
				Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_PRIM][nSereis], tbMacthCfg.nMapLinkType, "Võ Lâm Liên Đấu Sơ", 0, Ladder.LADDER_CLASS_WLLS, 1, nSereis);
			end
			
			for nSereis in pairs(self.RankLeagueList[Wlls.MACTH_ADV]) do
			    Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_ADV][nSereis], tbMacthCfg.nMapLinkType, "Võ Lâm Liên Đấu Cao", 0, Ladder.LADDER_CLASS_WLLS, 2, nSereis);
	...
	if tbMacthCfg.nMapLinkType == Wlls.MAP_LINK_TYPE_FACTION then
		...
		if (MODULE_GC_SERVER) then	
			...
			for nFaction in pairs(self.RankLeagueList[Wlls.MACTH_PRIM]) do
				Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_PRIM][nFaction], tbMacthCfg.nMapLinkType, "Võ Lâm Liên Đấu Sơ", 0, Ladder.LADDER_CLASS_WLLS, 1, nFaction);
			end
			
			for nFaction in pairs(self.RankLeagueList[Wlls.MACTH_ADV]) do
			    Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_ADV][nFaction], tbMacthCfg.nMapLinkType, "Võ Lâm Liên Đấu Cao", 0, Ladder.LADDER_CLASS_WLLS, 2, nFaction);
				local szFaction = Wlls.LADDER_FACTIONNAME[nFaction];
				-- Player:GetFactionRouteName(nFaction);
			end
			GlobalExcute{"Ladder:RefreshLadderName"};
...
function Wlls:LeagueRankFrame(nGameLevel, nType, nFinalType)
	...
	if nType == self.MAP_LINK_TYPE_RANDOM then
		...
		for nRank = self.RankLeagueId[nGameLevel], #self.RankLeagueList[nGameLevel] do
			...
			if (GLOBAL_AGENT and MODULE_GC_SERVER) then
				self:SetTeamPlayerRank(self.RankLeagueList[nGameLevel][nRank].szName, nRank);
			end
	...
	if nType == self.MAP_LINK_TYPE_SERIES then
		...
		for nSeries, tbLeagueList in pairs(self.RankLeagueList[nGameLevel]) do
			for nRank, tbLeagueInfo in ipairs(tbLeagueList) do
				...
				if (GLOBAL_AGENT and MODULE_GC_SERVER) then
					self:SetTeamPlayerRank(tbLeagueInfo.szName, nRank);
				end				
				if (MODULE_GC_SERVER) and nFinalType == 1 then
					--增加荣誉值.
	...
	if (MODULE_GC_SERVER) and nFinalType == 1 then
		...
		if (Wlls.MACTH_PRIM == nGameLevel) then
			nNowLadderType		= 1;
			nLastLadderType		= 4;
			szTitle = "Võ Lâm Liên Đấu Sơ";
		else
			nNowLadderType		= 2;
			nLastLadderType		= 5;
			szTitle = "Võ Lâm Liên Đấu Cao";		
		end
		--清除上届排行榜
		for i=0, 12 do
