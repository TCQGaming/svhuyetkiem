...
function Wlls:RegisterScheduleTask()
	...
	local nTaskId = KScheduleTask.AddTask("Dai Hoi Vo Lam Thi Dau Vong Tron", "Wlls", "ScheduleCallOut_TimerFrame");
	assert(nTaskId > 0);
	...
	nTaskId = KScheduleTask.AddTask("Dai Hoi Vo Lam Thi Dau Vong Tron [Hang Tuan]", "Wlls", "ScheduleCallOut_Weekend");
	assert(nTaskId > 0);
	for nTaskSeriel, nTimeState in pairs(tbWeekend) do
		KScheduleTask.RegisterTimeTask(nTaskId, nTimeState, nTaskSeriel);
	end
	
	nTaskId = KScheduleTask.AddTask("Dai Hoi Vo Lam Thi Dau Vong Tron [Hang Ngay]", "Wlls", "ScheduleCallOut_Common");
	assert(nTaskId > 0);
	for nTaskSeriel, nTimeState in pairs(tbCommon) do
		KScheduleTask.RegisterTimeTask(nTaskId, nTimeState, nTaskSeriel);
	end
	
	nTaskId = KScheduleTask.AddTask("Dai Hoi Vo Lam Thi Dau Vong Tron [Bat Cuong]", "Wlls", "ScheduleCallOut_AdvMatch");
	assert(nTaskId > 0);
...
function Wlls:GamePkStart(nTaskId)
	...
	if (GLOBAL_AGENT) then
		...
		if (nTaskId == 0) then
			local szAnncone = string.format("Giải đấu võ hiệp xuyên máy chủ %s bắt đầu chấp nhận đăng ký. Người chơi có thể đến gặp nhân viên đăng ký liên đấu máy chủ Lâm An để vào Đảo Anh Hùng, sau đó gửi đăng ký tham gia thông qua nhân viên phụ trách liên minh liên máy chủ của Đảo Anh Hùng.", szName);
			-- 表示八强赛
...
function Wlls:Send8PkMsg(nType)
	if nType == self.MAP_LINK_TYPE_FACTION then
		--未开发
		local szFormat = "<color=green>%s<color> Đại diện khu vực <color=green>%s<color> Đội phục vụ <color=green>%s<color>";
		for nReadyId, tbLeague in pairs(Wlls.AdvMatchLists) do
			local szMsg = "";
			if #tbLeague[2] > 0 then
				local nRank = 1;
				local nVsRank = 2;
				local szMsg  	= "<color=gray>Không có đội tham gia<color>";
				local szVsMsg	= "<color=gray>Không có đội tham gia<color>";
				local nFlag		= 0;
				if tbLeague[2][nRank] then
					local tbInfo = self:GetGateWayInfo(tbLeague[2][nRank].szName);
					local szServerName	= "không có"
					local szZoneName	= "không có"
					if (tbInfo) then
				...
				szMsg = string.format("%s sắp đi đến <color=red> <color> cuối cùng với %s, chúng ta hãy chờ xem", szMsg, szVsMsg);
				if (nFlag > 0) then
			...
			elseif #tbLeague[4] > 0 then
				for nRank=1, 2 do
					...
					szMsg = string.format("%s sắp đi đến <color=red> trận bán kết <color> với %s, chúng ta hãy chờ xem", szMsg, szVsMsg);
					if (nFlag > 0) then
						Dialog:GlobalNewsMsg_Center(szMsg);
						Dialog:GlobalNewsMsg_GC(szMsg);
					end
			...
			elseif #tbLeague[8] > 0 then
				for nRank=1, 4 do
					...
					szMsg = string.format("%s sắp cạnh tranh với %s trong cuộc thi <color=red> top 4 <color>, chúng ta hãy chờ xem", szMsg, szVsMsg);
					if (nFlag > 0) then
						Dialog:GlobalNewsMsg_Center(szMsg);
						Dialog:GlobalNewsMsg_GC(szMsg);
					end
