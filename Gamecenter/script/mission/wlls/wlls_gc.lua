...
function Wlls:UpdateHelpNews(nSession, nFinalType)
	...
	local szTitle	= "Võ Lâm Liên Đấu";
	if nFinalType == 1 then
		szTitle	= "Chung Kết Võ Lâm Liên Đấu";
	end
	local nAddTime = GetTime();
	local nEndTime = nAddTime + 3600 * 24 * 7;	
	local szMsg	= "";
	local szPrimWllsName	= "<color=yellow>Võ Lâm Liên Đấu Sơ<color>";
	local szAdvWllsName		= "<color=yellow>Võ Lâm Liên Đấu Cao<color>";
	if (GLOBAL_AGENT) then
		szPrimWllsName	= "<color=yellow>Võ Lâm Liên Đấu Sơ<color>";
		szAdvWllsName	= "<color=yellow>Võ Lâm Liên Đấu Cao<color>";
		szTitle			= "Máy chủ " .. szTitle
	end
...
function Wlls:GetHelpFactionNews(tbLeagueList, nLevel, szTitle)
	...
	for nFaction, tbLeague in pairs(tbLeagueList) do
		local szLeagueName = (tbLeague[1] and tbLeague[1].szName) or "Không có sẵn";
		szMsg = szMsg .. string.format("    <color=yellow>Nhánh môn phái:  %s<color>\n", Player:GetFactionRouteName(nFaction));
		szMsg = szMsg .. string.format("    Chiến đội： <color=green>%s<color>\n", szLeagueName);
		szMsg = szMsg .. string.format("    Thành viên của nhóm： <color=pink>");
		...
		if tbLeague[1] then
			...
			szMsg = szMsg .. string.format("Không có sẵn");
		end
...
function Wlls:GetHelpSeriesNews(tbLeagueList, nLevel, szTitle)
	...
	for nSereis, tbLeague in pairs(tbLeagueList) do
		local szLeagueName = (tbLeague[1] and tbLeague[1].szName) or "Không có sẵn";
		szMsg = szMsg .. string.format("    <color=yellow>Hệ %s<color>\n", string.format(Wlls.SERIES_COLOR[nSereis], Env.SERIES_NAME[nSereis]));
		szMsg = szMsg .. string.format("    Chiến đội： <color=green>%s<color>\n", szLeagueName);
		szMsg = szMsg .. string.format("    Thành viên nhóm： <color=pink>");
...
function Wlls:UpdateAdvHelpNews()
	...
	local szTitle	= "Võ Lâm Liên Đấu Tứ Kết";
	if nFinalType == 1 then
		szTitle	= "Võ Lâm Liên Đấu Tứ Kết";
	end
	local nAddTime = GetTime();
...
function Wlls:GetAdvHelpNewsInfor(tbLeague, nType, nReadyId)
	...
	if nType == self.MAP_LINK_TYPE_RANDOM then
		if Wlls.AdvMatchState == 5 and tbLeague[2][1] and #tbLeague[2][1].tbResult >= 3 then
			if tbLeague[1] and tbLeague[1][1] then
				szMsg = szMsg .. "\n\n<color=red>Chung kết giải vô địch：".. tbLeague[1][1].szName .. "<color>\n";
			else
				szMsg = szMsg .. "\n\n<color=red>Chung kết giải vô địch： Không có chức vô địch do một trận hòa, và cả hai đội đều đứng thứ hai.<color>\n";
			end
		end
		
		if #tbLeague[2] > 0 then
			szMsg = szMsg .. "\n\n<color=yellow>Bảng đấu cuối cùng<color>\n\n";
			local nRank = 1;
			local nVsRank = 2;
			local szName  = "<color=gray>Không có đội tham gia<color>";
			local szVsName  = "<color=gray>Không có đội tham gia<color>";
			...
			if tbLeague[2][nVsRank] then
				...
				szMsg = szMsg .. Lib:StrFillR(szName, 37) .. Lib:StrFillC("Chống lại", 8) .. szVsName .. "\n";
		end
			
		if #tbLeague[4] > 0 then
			szMsg = szMsg .. "\n\n<color=yellow>Tứ Kết<color>\n\n";
			for nRank=1, 2 do
	...
	if nType == self.MAP_LINK_TYPE_SERIES then
		...
		if #tbLeague[2] > 0 then
			szMsg = szMsg .. "\n\n<color=yellow>" .. szSereis .. "Bảng đấu cuối cùng của Ngũ hành<color>\n\n";
			local nRank = 1;
			local nVsRank = 2;
			local szName  = "<color=gray>Không có đội tham gia<color>";
			local szVsName  = "<color=gray>Không có đội tham gia<color>";
		...
		if #tbLeague[4] > 0 then
			szMsg = szMsg .. "\n\n<color=yellow>" .. szSereis .. "Ngũ hành Tứ Kết<color>\n\n";
			for nRank=1, 2 do
				local nVsRank = nRank + 2;
				local szName  = "<color=gray>Không có đội tham gia<color>";
				local szVsName  = "<color=gray>Không có đội tham gia<color>";
		...
		if #tbLeague[8] > 0 then
			szMsg = szMsg .. "\n\n<color=yellow>" .. szSereis .. "8 Cường<color>\n\n";
			for nRank=1, 4 do
				local nVsRank = 9 - nRank;
				local szName  = "<color=gray>Không có đội tham gia<color>";
				local szVsName  = "<color=gray>Không có đội tham gia<color>";
	...
	if nType == self.MAP_LINK_TYPE_FACTION then
		...
		if Wlls.AdvMatchState == 5 then
			if tbLeague[2][1] and #tbLeague[2][1].tbResult >= 3 then
				if tbLeague[1] and tbLeague[1][1] then
					szMsg = szMsg .. "\n\n<color=red>" .. szFaction .. "Võ thuật Chung kết giải vô địch：".. tbLeague[1][1].szName .. "<color>\n";
				else
					szMsg = szMsg .. "\n\n<color=red>" .. szFaction .. "Võ thuật Chung kết giải vô địch：Không có chức vô địch do một trận hòa, và cả hai đội đều đứng thứ hai.<color>\n";
				end
			else
				szMsg = szMsg .. "\n\n<color=red>" .. szFaction .. "Võ thuật Không có sẵn<color>\n";
			end
			szMsg = szMsg .. "\n\n";
			return szMsg;
		end
		...
		if #tbLeague[2] > 0 then
			szMsg = szMsg .. "\n\n<color=yellow>" .. szFaction .. "Võ thuật Bảng đấu cuối cùng<color>\n\n";
			local nRank = 1;
			local nVsRank = 2;
			local szName  = "<color=gray>Không có đội tham gia<color>";
			local szVsName  = "<color=gray>Không có đội tham gia<color>";
		...
		if #tbLeague[4] > 0 then
			szMsg = szMsg .. "\n\n<color=yellow>" .. szFaction .. "Võ thuật Tứ Kết<color>\n\n";
			for nRank=1, 2 do
				local nVsRank = nRank + 2;
				local szName  = "<color=gray>Không có đội tham gia<color>";
				local szVsName  = "<color=gray>Không có đội tham gia<color>";
		...
		if #tbLeague[8] > 0 then
			szMsg = szMsg .. "\n\n<color=yellow>" .. szFaction .. " 8 Cường<color>\n\n";
			for nRank=1, 4 do
				local nVsRank = 9 - nRank;
				local szName  = "<color=gray>Không có đội tham gia<color>";
				local szVsName  = "<color=gray>Không có đội tham gia<color>";
