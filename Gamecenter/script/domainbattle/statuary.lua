...
function tbStatuary:DelNpc(tbInfo)
	...
	if (not tbInfo.tbNpcInfo.nId) then
		return tbInfo;
	end
...
function tbStatuary:DelStatuary(szName, nEventType, nSaveFlag)
	...
	if (MODULE_GAMESERVER) then
		...
		KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, string.format("%s tượng biến mất", szName));
	end
...
function tbStatuary:AddHelpNews_Statuary(szName, nEventType)
	if (self.TYPE_EVENT_NORMAL == nEventType) then
		local szTitle	= string.format("%s dựng tượng!", szName);
		local szMsg		= string.format("Hoàng thượng biểu dương %s có cống hiến xuất sắc, đặc biệt dựng tượng.\n", szName);
		szMsg = szMsg .. [[    Pho tượng tạo hậu, còn nghĩ khảo nghiệm kỳ tại quảng đại dân chúng trung đích danh tiếng hòa lực ảnh hưởng. Quảng đại dân chúng khả dĩ đi trước <link=npcpos: pho tượng,0,4470>, đối pho tượng cúng bái hoặc phỉ nhổ.
	    Từng vai mỗi ngày chỉ có thể đối pho tượng cúng bái hoặc phỉ nhổ một lần.
	    Cúng bái lúc, hội thu được thưởng cho, song song đề cao pho tượng đích sùng kính độ, rơi chậm lại pho tượng đích bền độ.
	    Phỉ nhổ lúc, hội rơi chậm lại pho tượng đích bền độ.]];
...
function tbStatuary:AddHelpNews_Result(tbTongInfo)
	local szTitle	= "Bá chủ của trận chiến đấu cuối cùng công bố!";
	local szName	= KGblTask.SCGetDbTaskStr(DBTASK_BAZHUZHIYIN_MAX);
	local szMsg		= string.format("    Bá chủ của trận chiến đấu cuối cùng công bố!！\n    Thu thập Bá chủ ấn: %s\n    Ghi nhận %s đóng góp nổi bật, Hoàng thượng thiết hạ yến tiệc, ban phép cho bách tính muôn dân vào Lâm An Thành hành hương dự lễ trước cửa hoàng cung.\n    Mời %s và bạn bè đến Lâm An Phủ <link=npcpos:Lễ Bộ Thị Lang,0,4470>, tham gia nghi lễ\nThu thập Bá chủ ấn tối đa 10 bang hội: \n", szName, szName, szName);
	
	for i, tbInfo in ipairs(tbTongInfo) do
		local szRank		= string.format(" %d tên", i);
